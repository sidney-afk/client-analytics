...
- Before opening a PR for Production UI work, run `npm test` and `npm run test:prod-polish`. The latter intentionally combines locked live-read/zero-mutation coverage with a fully mocked native-write lane. No suite may mutate a live backend. F105 keeps the aggregate gate red until legacy picker-open assertions are made authority-aware; a green fast PR job alone is insufficient.
