...
## 2026-09-07 - Frozen anonymous native source composition, uninstalled

Pinned Calendar v48/Samples v49 captures compose with the existing native materialization adapter through five reversible additions per handler. Every other serving byte, including actor/client/event behavior and ordinary tokenless saves, remains unchanged.38 complete-handler/model controls pass with both repository 401 counterexamples retained and pass again after rebase onto d12d761. One bounded owned-disposable HTTP/SQL run passes42 total groups/64 requests/zero external attempts; four directly exercise the exact staged anonymous effects/current-row replay. Broader native negatives remain repository-auth coverage. [Private staged hashes, dependencies and release limits](docs/audits/2026-09-07-frozen-native-composition.md). Product writer files, production SQL, n8n, credentials, flags and JWT settings were untouched; nothing deployed.
...
draft from PR1318. Final integrated `66e823da` passed 12 actual local recovery
groups using SQL `c0d8257b`: exact37-table restore, four gateway/browser creation
envelopes, later human edits, retained raw input, non-UTC hold replay and late-COPY
rollback. Original 56-check SQL evidence and historical 22-check recovery evidence
remain separately pinned. Earlier configuration, CASE syntax and invalid fixture
mode failures are retained as failures. A dedicated disposable CI step is added;
hosted execution is pending publication. No product watcher was activated and no
installed/serving/client-journey claim follows. See the
[recovery audit](docs/audits/2026-09-06-native-card-materialization-recovery.md).
...
## 2026-07-20 — Workload hybrid automatic/manual planning (candidate source only)

- **SMM feedback restored deterministic automatic placement without restoring queue packing.** A
  dated issue without a manual override gets its own automatic work day one working day before its
  Linear deadline, floored to today. The calculation is independent per issue: newly urgent work
  cannot repack or spill existing automatic placements, so any resulting per-editor overload stays
  visible for a person to resolve.
- **Manual intent remains durable and discoverable.** A saved `workload_plan.plan_date`, keyed by the
  stable sub-issue id, wins over the automatic day. Calendar groups and rows distinguish Auto
  planned, Manual planned, and Mixed placement. **Use auto plan** visibly clears the saved override
  and returns the issue to item-local automatic placement; the existing optimistic, exact-one-row,
  revert-and-notify write contract remains unchanged.
- **Deadline and priority carry separate meanings.** Deadline proximity is visible without opening
  a popover (red at one day or less, orange at two to three days, green beyond three). Linear's
  explicit priority uses its native icon/color treatment and is enriched read-only from the
  existing `deliverables.priority` projection by stable Linear UUID. No client-difficulty score is
  introduced, and missing priority data leaves no icon rather than guessing.
- **Boundaries preserved.** This candidate is client-side plus offline guard/documentation work. It
  adds no scheduler service, schema, migration, Edge Function, runtime flag, Linear writer, n8n
