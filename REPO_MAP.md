...
| `migrations/` | Manually-applied Supabase SQL, kept for provenance (no auto-runner). `2026-07-11-b4-linear-outbound.sql` is the additive durable-outbox/switch delta; `2026-07-23-f203-production-issue-create.sql` is F203's source-only additive root/sub-issue create, identity-linkage, and identity-conflict quarantine RPC delta (not live-applied); `2026-07-23-production-comment-thread-lifecycle.sql` is the source-only additive F39/F42/F43 protected-reader, lifecycle, receipt, and two-surface import delta (not live-applied; existing `comment` operation reused); `2026-07-20-f27-team-rollback.sql` is the source-only corrective per-team rollback candidate: generation-fenced final CAS, exact replay echo binder, reserved no-provider drill, and pre-COMMIT TEST enqueue self-probe (not live-applied; #901 records the safe abort); `2026-07-15-pto-tracker.sql` is the locked PTO schema plus default-off flag source; `2026-07-15-pto-cancellation-audit.sql` is the source-only additive cancellation-attribution hardening; `2026-07-19-workload-plan.sql` represents the live, service-role-only sidecar for internal Workload plan dates (effective grants read back; exact revoke-correction artifact provenance remains F147). PTO migrations contain no HR seed data. See `migrations/README.md` for the baseline-plus-deltas layout and source-only-vs-applied distinction. |
...
| `docs/ops/` | Runbooks: new-client onboarding, Linear reconcile safety net, monitoring/rollback coverage map (`docs/ops/MONITORING.md`), the future owner-gated F27 preparatory-deploy/snapshot/install/source-exact-rollback contract (`docs/ops/F27_INSTALL_RUNBOOK.md`) plus its generated drift-checked operator checklist, and pending cleanup checklists. |
...
| F27 team rollback + install toolkit | `.github/workflows/f27-team-rollback-proof.yml`, the F27 job in `calendar-unit-tests.yml`, `scripts/f27-team-rollback-proof.sql`, `scripts/f27-mirror-outbox-snapshot.js`, `scripts/f27-apply-migration.js`, `scripts/f27-private-snapshot-store.js`, `scripts/f27-edge-source-rollback*.js`, `scripts/f27-database-rollback-*.js`, `scripts/f27-inbound-freshness.js`, `scripts/f27-drill-runner.js`, `scripts/f27-install-checklist.js`, and matching `test/f27-*.js` | Disposable PostgreSQL proof of generation-fenced late-insert rejection plus a full reserved no-provider drill, and source-only install operators for a deterministic private queue/definition snapshot, snapshot-bound migration apply, Shared Drive integrity round-trip, source-exact capture/restore/hash-readback rehearsal, generated/executed private database rollback, inbound freshness, live reserved drill, and runbook-derived checklist. Live modes require explicit owner-window confirmations; this toolkit PR invokes none. |
