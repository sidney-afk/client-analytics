...
| `migrations/` | Manually-applied Supabase SQL, kept for provenance (no auto-runner). `2026-07-11-b4-linear-outbound.sql` is the additive durable-outbox/switch delta; `2026-07-15-pto-tracker.sql` is the locked PTO schema plus default-off flag source; `2026-07-15-pto-cancellation-audit.sql` is the source-only additive cancellation-attribution hardening; `2026-07-19-workload-plan.sql` is the applied, service-role-only sidecar for internal Workload plan dates. PTO migrations contain no HR seed data. See `migrations/README.md` for the baseline-plus-deltas layout and source-only-vs-applied distinction. |
