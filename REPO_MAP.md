...
| F42 card-comment import | `scripts/f42-card-comment-import.js`, `scripts/f42-card-comment-apply.js`, `scripts/f42-apply-rehearsal.js`, `docs/independence/F42_CARD_COMMENT_IMPORT_RUNBOOK.md`, `test/production-comment-slice.js`, `test/f42-card-comment-apply.js` | Source-only two-surface Calendar+SXR planner with exact manifest/hash coverage and conflict classification; a gated apply runner that re-derives the plan, refuses anything not complete/conflict-free, applies via the service-only `production_comment_card_import` RPC in parents-before-children order, and verifies applied==planned against an independent DB readback; and a repeatable disposable-PostgreSQL rehearsal that applies every pending migration in order and runs plan+apply against fixtures to gate the live window. |
