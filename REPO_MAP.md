...
| `docs/ops/` | Runbooks: new-client onboarding, Linear reconcile safety net, monitoring/rollback coverage map (`docs/ops/MONITORING.md`), the future owner-gated F27 snapshot/install/exact-rollback contract (`docs/ops/F27_INSTALL_RUNBOOK.md`), and pending cleanup checklists. |
...
| `node qa/overnight_entry.js runner` / `node qa/overnight_entry.js cron` | Protected continuous or bounded unattended QA entry; brokers only TEST-client capability into eligible probes after `posix_session_guard.js` or the Windows `windows_job_worker.js` + Job guard contains the launch target | Local only; see the `/overnight-test` skill. Do not invoke the shell runner directly with ambient credentials; protected POSIX work stays in its assigned process group/session. |
...
| F27 team rollback | `.github/workflows/f27-team-rollback-proof.yml`, the F27 job in `calendar-unit-tests.yml`, `scripts/f27-team-rollback-proof.sql`, `test/f27-team-rollback.js`, `test/f27-linear-inbound-echo.js`, and `test/f27-linear-outbound-replay.js` | Disposable PostgreSQL proof of the generation-fenced late-insert rejection plus the full reserved no-provider drill, exact inbound replay-echo regression, correlated receipts, real-row/flag/fence isolation, permanent audit, and an observer outside n8n. It never targets the live project. |
