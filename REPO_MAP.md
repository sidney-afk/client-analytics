...
| `docs/audits/` | Dated audit evidence (2026-07-03 →). Historical record — do not edit old audits; add new dated files. `2026-07-19-boot-refresh-history-audit.md` preserves the immutable early-frame/refresh/history evidence behind F149–F162, the separately labelled publication source-only read-truth recon behind F163–F169, remediation-phase controlled synthetic-browser Calendar evidence behind F170–F171, the source-only staff Analytics follow-up behind F172, and successive exact-head remediation-review/current-candidate evidence behind F173–F184. For unmerged PR #891 `13c042b`, it distinguishes local green proof from exact-head cloud review `4741233371`, which returned remaining F176/F179 blockers; the separate F184 cloud finding also remains open. Current conclusions belong in `docs/truth/`. |
