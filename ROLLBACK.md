...
|---|---|---|---|
| Calendar writes (upsert/reorder) | n8n webhooks for all real clients. A1 support tables and the `calendar-upsert` Edge Function are deployed, PR #668 is merged, and GitHub Pages deployed merge commit `eecfa4b`. The runtime flag `syncview_runtime_flags.calendar_upsert_ef_clients` is `{"clients":["sidneylaruel"]}`, so only the Sidney TEST client routes calendar upserts to the Edge Function; no real client is enabled. A2 `calendar-reorder` Edge Function is deployed but A2 frontend routing is still draft/unmerged, so live reorders continue to use n8n until the A2 PR is reviewed and merged. | Set `syncview_runtime_flags.calendar_upsert_ef_clients` to `{"clients":[]}` to force all calendar upserts, including `sidneylaruel`, back to n8n. This rollback was rehearsed on TEST and remains the one-step rollback even with the merged code live. If the merged code itself causes trouble, first set the flag empty, then revert merge commit `eecfa4b` if needed. For A2 reorder canary after merge, the same flag remains the single flip point. | 2026-07-04 (A1 live TEST-only flag; A2 reorder EF deployed/parity-passed, unmerged) |
| Samples New (SXR) writes | n8n webhooks remain the live/default path. A2 code for `sample-review-upsert` and `sample-review-reorder` Edge Functions is staged on branch `codex/a2-writer-edge-functions`, both Edge Functions are deployed, and live n8n workflow `MJbMZ789B5ExZz9x` contains fail-safe samples routing through `sample_review_ef_clients`; with the flag empty/missing/unreadable, every client falls back to n8n. The runtime flag `syncview_runtime_flags.sample_review_ef_clients` exists and is `{"clients":[]}`. No client, including Sidney TEST, is currently enabled for samples. | Keep or set `syncview_runtime_flags.sample_review_ef_clients` to `{"clients":[]}`. If the n8n routing edit itself needs rollback, restore/publish the private pre-A2 `Handle Sample Linear Event` node backup. No real client may be added without owner approval. | 2026-07-04 (A2 deployed/parity-passed; flag empty; no real client enabled) |
| Samples Old writes | n8n webhooks (baseline; out of migration scope, D4) | n/a | 2026-07-03 |
| Linear -> app realtime sync | n8n workflow `MJbMZ789B5ExZz9x` active version `405ab03a-12bb-43ca-b70a-14aee3ba7f35`; two Linear webhooks cover both VID and GRA. Its calendar-upsert call is runtime-flagged: only `sidneylaruel` routes calendar upserts to the Edge Function, and all real clients fall back to n8n. Its samples branch now also reads `sample_review_ef_clients` and falls back to n8n for every client because that flag is empty. | Set `calendar_upsert_ef_clients` empty for calendar rollback. Keep/set `sample_review_ef_clients` empty for samples rollback. If the n8n routing edit itself fails, restore/publish the private pre-A2 workflow backup or deactivate the workflow; reconcilers continue healing regardless. | 2026-07-04 (A2 samples routing shim published; sample flag empty) |
