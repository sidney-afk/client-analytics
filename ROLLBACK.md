...
|---|---|---|---|
| Calendar writes (upsert/reorder) | n8n webhooks for all real clients. A1 support tables and the `calendar-upsert` Edge Function are deployed, PR #668 is merged, and GitHub Pages deployed merge commit `eecfa4b`. The runtime flag `syncview_runtime_flags.calendar_upsert_ef_clients` is `{"clients":["sidneylaruel"]}`, so only the Sidney TEST client routes to the Edge Function; no real client is enabled. | Set `syncview_runtime_flags.calendar_upsert_ef_clients` to `{"clients":[]}` to force all calendar upserts, including `sidneylaruel`, back to n8n. This rollback was rehearsed on TEST and remains the one-step rollback even with the merged code live. If the merged code itself causes trouble, first set the flag empty, then revert merge commit `eecfa4b` if needed. | 2026-07-04 (PR #668 merged; Pages deployed; TEST-only flag set) |
...
| Linear -> app realtime sync | n8n workflow `MJbMZ789B5ExZz9x` active version `ece94b2c-cdba-46b3-a1e5-966abda0e8f6`; two Linear webhooks cover both VID and GRA. Its calendar-upsert call is runtime-flagged: only `sidneylaruel` routes to the Edge Function, and all real clients fall back to n8n. | Set `calendar_upsert_ef_clients` empty for immediate rollback. If the n8n routing edit itself fails, publish the backed-up previous version `2fc824c2-1b60-413a-b2c3-e51135f7448a` or deactivate the workflow; reconciler continues healing every 10 min regardless. | 2026-07-04 (TEST-only flag set after Pages deploy) |
