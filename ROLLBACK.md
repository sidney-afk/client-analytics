...
| Linear deliverables reconciler v2 (Track B B3 stage 2) | Draft-stage measuring instrument: `.github/workflows/linear-deliverables-reconcile.yml` runs dry-run by default and `scripts/linear-deliverables-reconcile.js` compares B1 deliverables to Linear across status, title, due date, assignee, parent, archive/delete, engine-tracked comments, card linkage completeness, and Linear webhook enabled/resource state. It writes one summary event row with `source='reconcile'`. Healing is off unless a future manual run sets `apply=true`; when enabled, deliverable healing routes through `deliverable_write`. | Do not merge the workflow to keep it inactive. If merged and noisy, disable the `Linear ⇄ deliverables reconcile v2` workflow in GitHub Actions or revert the Stage-2 PR. Do not change `prod_authority`, `linear_inbound_enabled`, Track A flags, Linear webhooks, or n8n workflows for this rollback. | 2026-07-07 (live dry-run wrote summary event id 1842; dry-run counts: 1 real diff, 14 unknown-assignee repairs, 490 linkage gaps; Linear webhooks checked 2, disabled 0, missing Issue/Comment resources 0) |
