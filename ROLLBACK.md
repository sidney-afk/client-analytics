...
|---|---|---|---|
| Monitoring coverage | GitHub Actions nightlies cover Calendar E2E and Samples E2E on the TEST client; this PR re-enables both workflows and adds the read-only Production preview smoke to the Samples nightly as an independent job. n8n workflow `Tfhc3vebZyG6obOg` is an active Edge Alert Relay to DM Sidney; synthetic relay execution `219237` succeeded. Edge anomaly alerts from `linear-inbound` are coded to post unmapped-state and unknown-assignee anomalies to `SLACK_ALERT_WEBHOOK` with one-hour per-type throttling and identifier-only payloads, but setting that Supabase secret and redeploying the Edge Function is pending merge or explicit owner approval to deploy the draft monitoring branch. n8n workflow `qllIDZPkdNAPRj0b` is an active Monitoring Pager + Reconciler V2 Trigger: every 15 min it dispatches v2 dry-run, checks latest v2/mirror/calendar/samples evidence, and throttles each alert condition to one DM/hour. The coverage map is `docs/MONITORING.md`. | Disable the affected GitHub Actions workflow if a nightly becomes noisy. Disable `Tfhc3vebZyG6obOg` to stop Edge Alert Relay DMs. Disable `qllIDZPkdNAPRj0b` to stop pager DMs and v2 dispatches. For `linear-inbound` anomaly alerts, revert the monitoring PR; alert failures are fail-soft and the inbound mirror kill switch remains `linear_inbound_enabled=false`. | 2026-07-08 (nightlies re-enabled; Calendar run `28907864979` green; Samples run `28909416704` green including Production smoke; Edge Alert Relay execution `219237` green; pager execution `219270` green and dispatched v2 run `28913726658`) |
...
| Status drift healing | **Calendar path:** n8n trigger `AkiFmromoDkmsh39` requests `Linear ⇄ SyncView status reconcile` every 10 min; latest observed runs are green. **Samples path:** GitHub schedule for `Samples ⇄ Linear status reconcile` is restored and green but may deliver roughly hourly. The Monitoring Pager `qllIDZPkdNAPRj0b` checks both paths every 15 min and alerts if the latest completed run is older than 2h or not green. | Calendar reconciler must stay ACTIVE until Track B5 — this is the global safety net. If the samples reconciler misbehaves, disable the `Samples ⇄ Linear status reconcile` schedule in GitHub Actions or revert the cron-only PR; Track-A sample writes still roll back per client through `sample_review_ef_clients`. Disable pager workflow `qllIDZPkdNAPRj0b` if the staleness checks become noisy. | 2026-07-08 (Calendar run `28912777531` green; Samples run `28910037942` green; pager workflow active) |
...
| Linear deliverables reconciler v2 (Track B B3 stage 2/4) | LIVE measuring instrument: `.github/workflows/linear-deliverables-reconcile.yml` runs dry-run by default and `scripts/linear-deliverables-reconcile.js` compares B1 deliverables to Linear across status, title, due date, assignee, parent, archive/delete, engine-tracked comments, card linkage completeness, and Linear webhook enabled/resource state. It writes one summary event row with `source='reconcile'`. GitHub cron remains requested every 10 min but may deliver roughly hourly; n8n pager workflow `qllIDZPkdNAPRj0b` dispatches v2 dry-run every 15 min and alerts if the latest summary is older than 90 min or any of `diff_count`, `repair_list_size`, or `linkage_actionable` is non-zero. The B4 gate watches `linkage_actionable`, not raw classified residue. Healing remains off unless `apply=true`; when enabled, deliverable healing routes through `deliverable_write`. | If noisy, disable the `Linear ⇄ deliverables reconcile v2` workflow in GitHub Actions or disable n8n pager workflow `qllIDZPkdNAPRj0b` to stop the extra dispatches/alerts. Data repair rollback is targeted only: reverse the specific private-artifact card-slot fills / inactive attribution rows / `deliverable_write` heals if owner asks. Do not change `prod_authority`, `linear_inbound_enabled`, Track A flags, Linear webhooks, or n8n workflows for this rollback. | 2026-07-08 (parent-field drift healed through `deliverable_write`: apply summary event `2407` attempted 4/skipped 0; dry-run summary event `2408` checked 1,058 with diff_count 0, repair_list_size 0, linkage_actionable 0) |
