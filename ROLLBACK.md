...
|---|---|---|---|
| Monitoring coverage | GitHub Actions nightlies cover Calendar E2E and Samples E2E on the TEST client; this PR re-enables both workflows and adds the read-only Production preview smoke to the Samples nightly as an independent job. Edge anomaly alerts from `linear-inbound` are coded to post unmapped-state and unknown-assignee anomalies to `SLACK_ALERT_WEBHOOK` with one-hour per-type throttling and identifier-only payloads; Supabase metadata check found that secret is not yet present, so live Slack delivery requires adding it before deploying the alert code. The coverage map is `docs/MONITORING.md`. | Disable the affected GitHub Actions workflow if a nightly becomes noisy. For `linear-inbound` anomaly alerts, revert the monitoring PR; alert failures are fail-soft and the inbound mirror kill switch remains `linear_inbound_enabled=false`. | 2026-07-07 (nightlies re-enabled; Calendar run `28907864979` green; Samples run `28909416704` green including Production smoke) |
