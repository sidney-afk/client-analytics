...
| Production polish automation (2026-07-09) | Frontend-only automation layer for future `?prod=1` polish: `npm run test:prod-polish` packages boot/loading, structure, interaction inventory, accessibility/focus, layout clipping, wired behavior, and pixel parity checks; GitHub Actions runs it on Production UI PRs; `npm run test:prod-review` generates a compact screenshot packet, browsable gallery, Markdown manifest, and machine-readable JSON manifest; the workflow prints the Markdown manifest in the job summary and uploads the artifact; optional Argos upload is guarded by `ARGOS_TOKEN`; the owner-feedback issue template, PR checklist, agent instructions, and automation doc make rough polish feedback AI-ready. The only app behavior change is keyboard accessibility for existing Production controls: focused buttons keep native Enter/Space activation and Filter/Display icon buttons have accessible names. | Revert the automation PR/commit to remove the workflow, scripts, issue template, PR template, review-packet artifact generator, Argos CLI dev dependency/hook, and keyboard/accessibility polish. It changed only frontend Production behavior, test/docs/GitHub metadata, and dev dependencies; no Supabase data, runtime flags, n8n workflows, Linear webhooks, or backend write paths changed. | 2026-07-09 (verified `prod-boot-budget.js`, `prod-a11y-focus.js`, `prod-layout-polish.js`, `prod-review-packet.js` with 10 screenshots, `prod-structure-subset.js`, `prod-interaction-inventory.js`, `behav-wired.js` 158/158 guard mode, `pixel-wired.js` light/dark, and full `npm run test:prod-polish` in 315.5s) |
