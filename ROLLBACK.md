...
| Linear inbound engine (Track B B3 stage 1) | DARK. New `linear-inbound` Edge Function is deployed for B3 stage 1 with HMAC/replay verification, but no Linear webhook points at it and `syncview_runtime_flags.linear_inbound_enabled` remains `{"enabled":false}`. With the flag false, signed deliveries are acknowledged and logged only; production Linear-to-SyncView flow still runs through the existing n8n workflow `MJbMZ789B5ExZz9x` plus reconcilers. | Primary kill switch: keep or set `syncview_runtime_flags.linear_inbound_enabled` to `{"enabled":false}`. Hard stop once webhooks exist: disable the new Linear webhook(s) in Linear. Code rollback is reverting the `linear-inbound` PR and redeploying/removing the function; no existing n8n workflow or Track A flag is part of this rollback. | 2026-07-07 (B3 stage 1 dark deploy; unsigned request rejected, signed flag-false dummy acknowledged with no data change) |
| Linear deliverables reconciler v2 (Track B B3 stage 2/3) | Draft-stage measuring instrument: `.github/workflows/linear-deliverables-reconcile.yml` runs dry-run by default and `scripts/linear-deliverables-reconcile.js` compares B1 deliverables to Linear across status, title, due date, assignee, parent, archive/delete, engine-tracked comments, card linkage completeness, and Linear webhook enabled/resource state. It writes one summary event row with `source='reconcile'`. Healing is off unless a future manual run sets `apply=true`; when enabled, deliverable healing routes through `deliverable_write`. Stage 3 filled all resolvable card linkage slots and repaired assignee attribution; final raw linkage residue is explained as archive-only / duplicate-link / unresolved historical residue, with 0 resolvable writes left. | Do not merge the workflow to keep it inactive. If merged and noisy, disable the `Linear ⇄ deliverables reconcile v2` workflow in GitHub Actions or revert the Stage-2/3 PR. Dormant data repair rollback is targeted only: reverse the specific private-artifact card-slot fills / inactive attribution rows / `deliverable_write` heals if owner asks. Do not change `prod_authority`, `linear_inbound_enabled`, Track A flags, Linear webhooks, or n8n workflows for this rollback. | 2026-07-07 (final dry-run summary event id 1867: 1,058 checked, 0 real diffs, 0 unknown-assignee repairs, 212 explained linkage residue, 0 resolvable linkage writes; Linear webhooks checked 2, disabled 0, missing Issue/Comment resources 0) |
