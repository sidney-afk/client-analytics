...
| Templates / caption prompts | Production `main` still uses Google Sheets via n8n. A4 Supabase tables (`templates`, `caption_prompts`) exist, were backfilled from the live Sheet reads, and A4 write Edge Functions (`templates-save`, `caption-prompts-save`) are deployed, but the A4 frontend routing is staged only on branch `codex/a4-settings-edge-functions` and unmerged. The runtime flag `syncview_runtime_flags.settings_ef_clients` exists and is `{"clients":[]}`, so every client remains on n8n for settings reads and writes. The staged frontend reads n8n as the base store and overlays Supabase rows only for clients listed in `settings_ef_clients`, using the same flag as writes; Supabase read/flag failures fall back to the n8n base. Caption generation and filming-plan tabs remain on n8n and are out of A4 scope. | Keep or set `syncview_runtime_flags.settings_ef_clients` to `{"clients":[]}` to force all template and caption-prompt reads/writes back to n8n. If the staged frontend code itself causes trouble after a future merge, first set the flag empty, then revert the A4 merge commit if needed. No real client may be added without owner approval. | 2026-07-04 (A4 deployed/backfilled/parity-passed; read/write routing gated by same flag; flag empty; unmerged) |
