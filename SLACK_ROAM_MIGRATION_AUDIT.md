...
| CI (`.github/workflows/*`, 4 files) | **None today** | No Slack/notify step exists. `HEADLESS_TESTING_EVAL_2026-06-26.md` *recommends* adding failure alerting — if built, target **Roam**, not a Slack action. The README's `DISCORD_WEBHOOK_URL` is **defunct/unrelated** (dead IG scraper) — **decision item:** remove it during cutover so no one wires alerts to a dead endpoint |
...
| Docs (`NEW_CLIENT_ONBOARDING.md`, `ONBOARDING_FORM.md`, `LINEAR_SYNC_RECONCILE.md`, `SAMPLES_V2_PLAN.md`, `qa/PARITY_LEDGER.md`, `qa/DIVERGENCE_REPORT.md`) | Describe flows | Update Slack→Roam references; planned/future routes (`_sxrSendUrgentSlack` samples ping, post-call Fathom→Slack post, reconciler Slack alert) should target Roam |
