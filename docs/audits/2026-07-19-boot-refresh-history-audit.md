...
| RR-04 | P1 | At `02105e9`, both nightlies exported the resolved TEST token through `GITHUB_ENV`, making it job-wide. The later candidate replaces that export, but exact-head cloud source review at `59022d` found `run-probes` reconstructing `SYNCVIEW_STAFF_KEY` into every manually selected probe child, including non-client probes that do not need client-entry capability. The issuer credential therefore remains broader than the exact in-process resolver that needs it. No token or staff-key value was resolved, read or executed; the `93fc297` candidate remediation is in progress and unmerged. | Unmerged `02105e9`: `qa/test-client-entry.js:92-108` and both nightly workflows; unmerged `59022d`/`93fc297`: `qa/run-probes.js:109-115` | F176 |
