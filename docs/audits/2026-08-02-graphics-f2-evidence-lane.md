...
| Leave an accessible cast backed by a revoked-direct `SECURITY DEFINER` function | `FAIL` with `postgres_role_not_read_only` |
...
accessible application operators and casts backed by a `SECURITY DEFINER` implementation, plus all
