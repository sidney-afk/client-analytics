...
| Created or exact replay | 200; `ok:true`, `conserved:true`, complete current `post` or `sample` | SQL supplied its receipt and complete known card schema, exact client/card and original nonempty native child IDs. Nullable columns remain nullable; later human content is returned unchanged. |
