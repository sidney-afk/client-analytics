...
| Submit (`#linear`) | Staff form that creates a new card (the SyncLinear view itself is Production, above). | 200 | writes through `production-write` (`intake_create`), counted in the Production row | Live |
