...
|---|---|---|---|---|---|
| 2026-09-05 | **Review-gate sweep: every writer, any prefix** (branch `claude/gate-caller-sweep`, ledger 154) | `_kasperApproveComp` (Kasper calendar approve) · `_writeUiRetryCardCommentResolve` (shared journal retry) | **Yes — mirrored** | `_sxrKasperApproveComp` now refuses at the handler when the component is empty, reason shown via `_sxrKasperState.errors`, mirroring `_kasperApproveComp`. The journal retry is one shared function that branches on surface, so both twins are covered by one guard. `test/cal-review-needs-content.js` §9 derives a prefix-free writer roster over BOTH surfaces and classifies every entry; `_sxrKasperUndoApprove` is an explicit exemption (restores a state that existed). | Done |
| 2026-09-05 | **Review-gate escape hatch + the two callers that ignored the refusal** (branch `claude/review-gate-escape-hatch`) | `_calResolveLastTweak` / `_calApplyAutoStatus` (the M3b review state machine row below) · `_calStatusToggleMenu` (per-component status menu) | **Yes — mirrored** | Follow-up to the 2026-09-05 gate row above, after the owner asked whether the gate broke anything. It had: `_sxrApplyAutoStatus` returns false for an empty component and `_sxrResolveLastTweak` ignores the return, so resolving the change-request first closed the thread and moved nothing, silently. Now refuses BEFORE resolving, with the reason shown. NEW `_sxrAutoResolveDestStatus` (the chooser's route, which defaults to Client Approval — deliberately NOT `_sxrSmmApproveTo`, whose default is Kasper Approval; sharing one would make each guard protect the wrong move). `_sxrStatusToggleMenu` now renders an unreachable review status disabled with the reason on hover. **DELIBERATE DIVERGENCE:** the calendar's note offers N/A as the way out for a lane a post will never have; `SXR_STATUSES` has no N/A (samples carry exactly two components, both expected to exist), so the samples note stops at naming what is missing. `test/cal-review-needs-content.js` covers both twins, mutation-proven. | Done |
| 2026-09-05 | **Review needs something to review** (branch `claude/block-empty-review-status`, PR #1272) | `_calStatusPick` / `_calSetAllStatus` / `_calApplyAutoStatus` / `_calReviewApprove`+`_calReviewApplyApprove` (the M3b review state machine row below) | **Yes — mirrored** | A component can no longer be moved to For SMM Approval / Kasper Approval / Client Approval while the thing being reviewed is empty. Owner, 2026-09-05: *"if the caption is empty or if there is no thumbnail URL or no video URL then it shouldn't go to Kasper Approval."* Samples carries the same gate over its two components: NEW `_sxrReviewBlockReason` / `_sxrStatusNeedsContent` / `_sxrCompHasContent` / `SXR_REVIEW_STATUSES` / `SXR_COMP_LABEL` / `_sxrSmmApproveTo`, consulted by `_sxrStatusPick` (both branches), `_sxrSetAllStatus`, `_sxrApplyAutoStatus` and `_sxrReviewApplyApprove` (guarded BEFORE the `kasper_seen` stamp), plus the pre-resolve check in `_sxrReviewApprove` so a refused approve cannot mark the ticked change-requests done. **REUSED rather than forked:** the content predicate is `_kasperCompReviewable` on both surfaces — samples store the media in the same `asset_url` / `thumbnail_url` columns, so a samples-local copy would be pure drift with nothing to gain. **DELIBERATE DIVERGENCE:** samples has no caption and no title, so only the video and thumbnail rules exist here; and Tweaks Needed plus the terminal states are ungated on both surfaces (rejection, and legacy cards completed before either surface carried media). `test/cal-review-needs-content.js` executes the samples gate and derives the samples writer roster from the source, so this row is proved rather than promised. | Done |
| 2026-06-26 | Samples FE — **CLIENT-REVIEW + NOTES + REALTIME parity** (branch `claude/samples-review-parity-plan-ldyzh8`) | `renderCalReview`/`_calReviewCardHtml`/`_calReviewPendingLabel`/`_calReviewToggleCard` (client surface) · `_calRenderCommentsModal`/`_calComposerHtml`/`_calCommentActionsHtml` + global `.cal-comments-*`/`.cal-cm-*` CSS (notes) · `_calRefreshOnReturn` (tab-return refetch) | **Rebuilt / re-skinned / fixed (FE-only)** | Three more surfaces that still diverged from the calendar: **(1) Client review** — the client share link rendered each sample as a wide SMM strip card with the review crammed inside (two cramped columns). Rebuilt the `_isClientLink` branch of `_sxrRenderBody` as a clone of `renderCalReview`: a vertical list of COLLAPSIBLE `kcard.cal-review-card` review cards (strip = thumb + name + plain-English "what to do" line + read-only pills; expand → the roomy per-component review panels + client-visible creative-direction). NEW `_sxrReviewPendingLabel`/`_sxrReviewCardHtml`/`_sxrReviewToggleCard`/`_sxrReviewRepaintCard`. **(2) Notes modal** — same structure but its own `sxr-cm-*` CSS made it look different; re-skinned `_sxrRenderCommentsModal`/`_sxrComposerHtml`/`_sxrCommentRowHtml` (+ the shell + the in-place toggle handlers) to emit the calendar's `cal-comments-*`/`cal-cm-*` markup, reusing the global CSS (gradient avatars, "Tweak #N"/"Resolved" tags, pill-segment composer). **(3) Realtime** — Supabase realtime DOES deliver `sample_reviews` changes (verified directly) and the FE subscribes, but samples lacked the calendar's tab-return catch-up, so a backgrounded tab (whose socket suspended; Supabase never replays missed rows) went stale. Added `_sxrRefreshOnReturn` (clone of `_calRefreshOnReturn`: visibilitychange + focus + pageshow → one throttled background reload); and `_sxrCopyShareLink` now carries `?sxr=1` (samples is default-OFF — without it a fresh client never mounts the surface or activates realtime). All `qa/probes/sxr_*` green incl. new `sxr_client_visual` / `sxr_notes_visual` / `sxr_return_refresh`. FE-only; webhooks + inbound Linear branch UNTOUCHED. | Done |
| 2026-06-26 | Samples FE — **CARD RENDER REBUILT as a faithful calendar clone** (branch `claude/samples-review-parity-plan-ldyzh8`) | `_calRenderInlineCard` / `renderCalOrganizer` / `_calLinkFieldHtml` / `_calTitleRowHtml` / `_calLinearPileHtml`+`_calLinearSlotHtml` / `_calStatusToggleMenu` / `_calOpenSetAllMenu`+`_calSetAllStatus` + the global `.cal-card*` / `.cal-link-field` / `.cal-card-substatus-row` / `.cal-fld-setall` CSS | **Rebuilt (FE-only)** | The earlier samples card was a leaner re-implementation that did NOT look like the calendar card and had no working status-change / Set-all. Rebuilt `_sxrRenderCard` (SMM branch) to clone `_calRenderInlineCard`'s markup, reusing the calendar's GLOBAL `cal-*` CSS so it renders pixel-identical: switched `_sxrRenderBody` from a `sxr-grid` of tiles to the calendar's horizontal `cal-organizer-strip` of wide cards (restyled `.sxr-grid`/`.sxr-card`/`.sxr-card-thumb` to the calendar's `--cal-card-w`/`--cal-thumb-h` floating-thumbnail look). NEW `_sxr*` clones: `_sxrLinkFieldHtml`+`_sxrOnLinkInput`/`_sxrOnLinkBlur`/`_sxrOpenLink`/`_sxrEditLink` (collapsed media link-pills), `_sxrLinearPileHtml`/`_sxrLinearSlotHtml` (Linear pile on the thumbnail; Linear EDIT now swaps the title row), `_sxrTitleRowHtml`, `_sxrStatusRowHtml` (bottom per-component sub-status triggers) + `_sxrOpenSetAllMenu`/`_sxrSetAllStatus`/`_sxrSetAllSettable` ("Set all to…"); rewrote `_sxrOpenStatusMenuFor` to the colour-coded `cal-fld-status-item` menu and `_sxrForceThumbRefresh` to PRESERVE the Linear-pile/warn overlays (clone of `_calSetThumbMedia` — without this a media-link edit wiped the pile). **EXCLUDED (per spec):** caption (+CTA), date/scheduling, platforms strip, colour tag — so the card is shorter. **DELIBERATE DIVERGENCE:** the calendar locks an unlinked video/graphic trigger because caption is its always-settable escape hatch; samples has no caption, so the triggers stay ALWAYS-ACTIONABLE (the Linear push no-ops until linked) — otherwise a not-yet-linked sample would have nothing settable. All 25 `qa/probes/sxr_*` green incl. `sxr_f1` isolation + the new `sxr_card_visual`. FE-only; the `sample-review-upsert` webhook + the inbound Linear branch are UNTOUCHED. | Done |
