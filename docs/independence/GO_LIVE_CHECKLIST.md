...
These are a **design**, not installed or tested watchers. Reuse existing GitHub Actions, event ledgers, private Drive and relay where suitable; add the named tests/instrumentation below. Suggested default recipient: owner Slack DM with Kasper as client-incident backup; the owner must resolve the actual private destinations and acknowledge drills before G1/G7. Independent fallback: GitHub failed-run email to the owner, tested while n8n is unavailable. No channel is counted as working from configuration alone. Public logs contain only gate, function/lane, anonymized run ID, count, age, status, source hash and incident code.
