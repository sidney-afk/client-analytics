...
### Current operational scope and finite blockers - September 7, 2026

The owner's September 7 instruction supersedes broader historical-export prerequisites: finish **operational independence for current client, Kasper and SMM website workflows**. Preserve current work and anything reachable through offered product views, including needed historical comments/files. Comprehensive preservation of unused Linear history is optional, separate work. No deletion, hidden removal of an offered feature, or acceptance of content loss is authorized. A terminal provider status alone does not establish that an item is unused.

Current runtime candidate is draft [PR1326](https://github.com/sidney-afk/client-analytics/pull/1326), exact `d12d761d309cc0bc98bbefb597afa3c62516b54e`, combining d144 with refused-intake retention, owned-native Submit routing and retained Workload cutoff admission. Named post-publication source review passed. Focused combined checks59/16/72/58, map507 and truth542 pass. The historical459-suite hosted result belongs to d144; all seven applicable new-head hosted checks now pass, with two optional polish jobs skipped. Separate staged operations PR1332 cfb042aca prepares the tokenless composition and independent receipt/heartbeat consumer; source review and focused checks pass, but nothing is installed. This is source/disposable proof, not a live release. [Completion accounting](../audits/2026-09-05-linear-exit-completion-accounting.md) owns dated evidence; older checkpoints below are historical even where they say "current". PR1327 and unpushed transport/privacy work remain optional and paused.

| Remaining operational blocker | Already prepared / smallest remaining work | Owner decision or action still needed |
|---|---|---|
| G0/G2/G3: install a compatible replacement and recovery | Native intake, assignments, labels, Workload, card/feedback recovery, v8 schema/data recovery and ordinary-worker cutoff are integrated. Assemble exact migrations, catalog/roster attestation, frozen anonymous serving composition, private recovery custody and function/browser deployment order. Latest installed-state evidence is dated September 6; refresh only at the proposed release boundary. | Later exact release approval and owner-held capture; no merge/install/deploy now. |
| G4/G5: current work, discussions and media stay usable | Reuse saved provider/native inventories. Trace references to actual offered readers and resolve actionable omissions; rescue only the dependency set needed by those views. Existing identity matches and URL counts do not prove rendering or file recovery. | Approve a concrete, private, bounded copy/repair only when its source, destination, readback and inverse are ready. Optional unused history needs no exit decision. |
| G6: old browsers and automation routes | First-click4xx loss and saved-native routing are corrected and combined. Fresh F44 and legacy card n8n routes still need serving compatibility; direct forwarding to repository writers rejects credentialless payloads. Prepare native branches on captured tokenless closures and exact raw-body preservation. Sign-out/actorless debt remains held under existing privacy rules. | Later exact n8n/serving changes and live debt disposition; no invented actor, privileged forwarding key or replacement request identity. |
| G8: every product route stops contacting Linear | Ordinary worker and retained Workload admission corrections are combined. Workload needs the existing cutoff table/grant before serving; current CON/STR work must remain available. Old isolates, pre-admitted requests, published automation roots, inbound and both outbox lanes still need disposition and observed denial. | Later reviewed scheduler/n8n/flag/credential operations; never stop anonymous client saves. |
| G1/G5/G7: complete real workflows and alarms | The editor-to-SMM-to-Kasper route and comments pass8 focused isolated steps on combined runtime59b. Actual reserved TEST client/staff persistence, degraded paths and independent denied-provider observation remain. Owner screenshot acknowledges both September5 drills; watchers are INACTIVE and independent fallback is unproved. | Later TEST/monitor activation approval. Prepare the independent backup setup for review; the owner does not know of an existing host and should not be asked to design infrastructure. |
| G0/G9/G10: safe observation time | Neither clock has started. Retain72 hours after product-access removal before Decision A, then at least14 days of controlled retention. September15 cancellation remains scheduled. Owner now selects preparation of a short access extension. Public billing documentation was verified September7; actual account renewal/cost remains unverified. | Prepare one monthly renewal through October15 for review, conditional on account billing details and completing the gates in time. No billing, cancellation or credential change authorized. |

**Completed corrections:** PR1328 b60 refusal retention is now integrated in PR1326. Its six applicable hosted checks passed; combined source checks also passed. The additional Submit selector and Workload fixes preserve accepted work and healthy open-control behavior. Their exact remaining route/install limits are in the candidate's legacy compatibility and Workload cutoff runbooks. None is served.

**Earlier bounded browser proof (retained):** on exact b60, the unchanged isolated harness passed its video/graphics review journeys and retained two previous failing cases. A separate four-case run passed rejected save, lost response, stale version and transport negative controls. Draft [PR1329](https://github.com/sidney-afk/client-analytics/pull/1329), tooling `17bf8cc2d14edf2fe9cc37e73f843846e548b496`, repairs the fixture's omitted Feedback response by invoking the actual source reader over synthetic rows; the comments cell then passes four steps with a preserved missing-feedback negative and zero canonical import. Coordinator independently reviewed and reran that cell. The original red receipts remain historical; neither run proves installed persistence or the full matrix.

**Owner workflow now exercised:** Kasper change request -> actual video editor sends For SMM Approval -> separate SMM sends Kasper Approval passes on combined59b, including visible Save failed/Retry, unchanged modeled state and fresh contexts. PR1329 tooling is now `ef0a7d0bf2e14c540b6193fa6d6eba1048f664f2`; named post-publication review passed. This proves the isolated control sequence, not media-edit quality or live authorization. The separate Comments-history Reopen expectation remains an edge contract; no policy change or newly required owner decision.

**Next finite preparation:** finish the legacy adapters, required operational media reader/copy preparation, combined recovery and independent alarm setup. All1167 distinct references from486 active-client native briefs now have private source bytes, across a bounded1166-file collection and one separate large-video read. The owner explicitly allows only that historical Posted video's website restoration to wait; keep its private copy and all original content. Required website media therefore excludes that single privately bound reference, not the other media in its brief. Format validation and independent website delivery remain unproven. The49 linked staff comment rows remain a separate dependency. See the [release preparation packet](LINEAR_EXIT_RELEASE_PACKET_2026-09-07.md) for exact receipts, actions and inverses.

**Concrete release packet still to finish, in G0-G10 order:** (1) pin candidate and all compatible function closures, additive migrations/catalogs and private backup/recovery digests; (2) rehearse installation and current client/staff T1-T6 paths in disposable isolation with denied provider transport, then submit the exact held production/TEST actions for approval; (3) after approval stage backend/recovery before activating browser callers, retaining frozen tokenless writers; (4) verify actual served/installed bytes and run reserved TEST journeys plus acknowledged alert/fallback drills; (5) separately approve and execute inventoried cutoff/debt dispositions, then product credential removal; (6) observe 72 hours before Decision A and retain the isolated account for the later 14-day period. Each stage needs its exact inverse and post-change receipt; this sequence is preparation, not an executable blanket approval.

**Previously chosen rules retained:** Calendar owns new sub-issue creation; Submit remains available; SyncLinear and Submit retain their distinct routes. Anonymous client writers stay tokenless. Accepted native work, request identities and later human edits survive retries and recovery. The owner kept SyncViewbot as the primary alert route on September 7 and requested read-only diagnosis. Comprehensive unused-history export is optional; deletion is never part of this release.
...
**September 6 UTC follow-up:** [PR #1313 reconstruction review](../audits/2026-09-06-recovery-package-independent-review.md) independently reproduced three false-success restores at `8fa163b79475f50474c092eafa6e6db446d2241e`: two lose exact sequence state and one permits an undetected restored-content change. The original 16 groups pass with an explicitly disclosed Windows harness adapter; the extra failures remain red. Runtime privilege checks and committed-target failure recovery also require correction. The [existing-session correction handoff](LINEAR_EXIT_HANDOFF_SCHEMA_RESTORE_CORRECTIONS_2026-09-06.md) is concrete, with no live execution authorized. G3/G10 schema recovery remains held. The owner confirms the other two Claude tasks are still running.

The [completion accounting update](../audits/2026-09-05-linear-exit-completion-accounting.md#september-6-utc-outcome-update) records reviewed drafts #1315 (intake editor projection) and #1316 (dormant label catalog), with exact commits, distinct test evidence and release limits. The catalog's new table is outside the current backup data corpus and requires schema plus staged-data restore integration before installation. These are completed bounded draft slices, not completed G2/G5 production gates. Product watchers are still inactive, no observation clock has started, and Decision A remains NOT READY. Earlier checkpoint statements below retain their original dated evidence.
...
**September 7 scope correction:** completeness here means the current product dependency set: offered active/historical views, current staff work, accepted operations and their required content/configuration/media. An independently complete whole-workspace forensic export is not a prerequisite merely because older F34 tooling asks for one. Keep that tool's existing certification rules intact; prepare a separately reviewed operational manifest if needed. No exclusions may be inferred solely from age, team, terminal status or missing normalized linkage. Prove actual reachability/ownership first and keep unknown required-use cases unresolved.
...
### G10 — Decision B observation, optional forensic preservation, then separate C decisions

**Change / order:** retain the isolated Linear workspace for at least 14 consecutive days after Decision A and through two full 7-day cache intervals plus real staff/client cycles. This is a proposed minimum, not a proven provider retention guarantee. Extend for an operational incident, unresolved product dependency or longer critical cycle. Verify recovery of required native history and assets with product credentials absent. Comprehensive forensic export and its two scratch restores are a separate optional project; unfinished unused-history export does not reopen Decision A. Verify actual account permission and transition behavior rather than assuming a plan tier is read-only. Resolve the already scheduled September 15 transition before relying on this period.
...
**Pre/post tests:** daily T1 read/history samples plus scheduled approved TEST persistence checks; T7 restored product-required native/store/archive browsing, no provider egress, count/hash/audience parity, duplicate replay refusal; T8 liveness/delivery. **Completion:** continuous 14-day green period, complete product-required manifests and checked restore; owner acknowledges account-specific consequences. No automatic cancellation/deletion at timer expiry. **Rollback:** retirement/permissions per verified provider inverse while still available; billing per account terms; permanent workspace deletion has no post-deadline inverse. **Abort:** missing required files/comments/history, broken operational restore, unresolved required ownership, unverified account behavior or a retention deadline arriving before proof. Optional forensic gaps remain recorded separately.
...
These are **release requirements**, not active product watchers. Some runner/observer/drill code is now implemented and locally tested; use the dated checkpoint above for its exact limits. The owner selected a private Slack DM through the existing SyncViewbot. The two specifically approved primary DRILL messages were delivered, independently read back, and are now human-acknowledged by the owner screenshot. No other recipient or recurring message is authorized by that drill. Resolve a separate backup recipient before activation. Independent observation and fallback must operate outside both GitHub Actions and n8n so failure of either cannot suppress its own missing-run alarm; GitHub failed-run email alone cannot close that combined requirement. The earlier hosted missed-ping option was a proposal. New private source preparation remains inactive; complete independent receipt checks, incident acknowledgement/escalation and actual backup delivery must be proved before activation. No product watcher is counted active from code, configuration, a coordinator heartbeat or one delivered alarm alone. Public logs contain only gate, function/lane, anonymized run ID, count, age, status, source hash and incident code.
