...
Current runtime candidate is draft [PR1326](https://github.com/sidney-afk/client-analytics/pull/1326), exact `d1442f65c4da6dc4e5ef8f155e1465f564cafa5e`; its hosted checks were re-read September 7 and remain successful where executed. The recorded exact-head result is 459 unit suites plus the dedicated native recovery checks. Optional skipped jobs remain skipped. This is source/disposable proof, not a working live release. [Completion accounting](../audits/2026-09-05-linear-exit-completion-accounting.md) owns dated evidence; older checkpoints below are historical even where they say "current". PR1327 and unpushed transport/privacy work are checkpointed optional work; do not resume them unless a specific current product dependency requires them.
...
| G6: old browsers and automation routes | Accepted-work retention exists. Remaining source gaps include unheld first-click 4xx request loss, legacy intake/card n8n paths and sign-out recovery under the existing privacy contract. Old browser/client payload compatibility still needs actual route proof. | n8n changes and live legacy-debt disposition remain separately held; no invented actor or replacement request identity. |
| G8: every product route stops contacting Linear | Ordinary worker request fencing is implemented. `workload-linear` metadata still has an unfenced provider read; due-date handling reads provider team before rejecting native authority. Published automation roots, old workers/in-flight requests, inbound and both outbox lanes need exact disposition and denial evidence. | Later reviewed scheduler/n8n/flag/credential operations; never stop anonymous client saves. |
| G1/G5/G7: complete real workflows and alarms | Reuse focused source/SQL/browser evidence. Remaining release proof is populated reserved TEST client/Kasper/SMM journeys, failed reads/lost responses/old tabs, independent deny-provider observation, and second-context persistence. Watchers are INACTIVE. Bot messages are present; human acknowledgement and independent fallback are separate. | Owner confirms TEST execution window and delivery; independent host/receipt transport/fallback route must be concretely selected and reviewed before activation. |
| G0/G9/G10: safe observation time | Neither clock has started. Keep the existing 72 hours before Decision A and later minimum 14 days of controlled retention. The owner reconfirmed September 15 cancellation on September 7. Even an immediate cutoff would exceed that date. Cancellation consequences have not been freshly verified. | Account owner must resolve verified access/retention timing; no cancellation, billing or credential action is authorized here. |

**Completed bounded correction:** [draft PR1328](https://github.com/sidney-afk/client-analytics/pull/1328), exact `b60a9705492002830eed60ece874e0686fc4b538`, retains Calendar/Samples unheld live-click refusals. Independent source review and coordinator rerun of all 59 focused source/model groups passed. It is stacked on PR1326, not integrated or served. The source fix does not solve sign-out/storage loss or old transports. Hosted validation was still running at publication.

**Current bounded browser proof:** on exact b60, the unchanged isolated harness passed its video/graphics review journeys and retained two previous failing cases. A separate four-case run passed rejected save, lost response, stale version and transport negative controls. Draft [PR1329](https://github.com/sidney-afk/client-analytics/pull/1329), tooling `17bf8cc2d14edf2fe9cc37e73f843846e548b496`, repairs the fixture's omitted Feedback response by invoking the actual source reader over synthetic rows; the comments cell then passes four steps with a preserved missing-feedback negative and zero canonical import. Coordinator independently reviewed and reran that cell. The original red receipts remain historical; neither run proves installed persistence or the full matrix.

**Owner workflow clarification:** the normal video route is Kasper change request -> editor fixes -> For SMM Approval -> SMM review -> Kasper Approval. Existing journey cells resolve as SMM and do not prove that complete editor handoff; add the correct role journey before declaring the offered workflow complete. The separate Reopen control exercised by the old test is inside Comments history, not that normal handoff. Its status expectation remains an unresolved edge contract; no reopening-policy change was approved, and it must not be presented as an owner decision blocking the clarified normal route.

**Next finite preparation:** prove the clarified editor-to-SMM journey; prepare the remaining legacy Workload/n8n provider boundary and current-reader media dependency set using saved inventories. Do not repeat the full candidate suite or restart the archive exporter. Source fixes and per-route rehearsals precede a final release request.
