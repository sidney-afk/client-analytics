...
| VERIFIED | **B1 incremental refresh — omitted** | Every ~30 minutes: Linear changed-issues query -> `b1-linear-backfill.js --incremental --apply` -> `batch_write`, `deliverable_write`, `linear_archive`. Current source loads team authority, filters operational batch/deliverable candidates to Linear-authoritative teams, and freshly rechecks authority before each authoritative write. Archive handling remains a separate path; comments are still omitted. | The current operational batch/deliverable apply is direction-gated. Archive disposition and missing comment parity remain explicit B5/migration work rather than proof of full native parity. | **keep-until-B5 / no-action to the current direction guard.** Preserve the fresh pre-write check; settle archive and comment migration separately. `.github/workflows/b1-linear-incremental-refresh.yml`; `scripts/b1-linear-backfill.js:1010-1184`. |
...
| VERIFIED | **Native linkage fields are write-routing-only; card ownership/navigation is absent (F112)** | `video_deliverable_id` / `graphic_deliverable_id` are normalized, saved, rollback/clear-sentinel capable, and Kasper-persisted. The deployed/current-main and latest inspected #813 slot renderers still consume only Linear URLs; neither Calendar nor Samples dereferences `deliverables → team_members`, renders D-20's **View sub-issue** route, or shows the current assignee. | Field transport: **no-action**. Before the first creative flip, make these IDs the authority-aware target for every predicate, identity, route, and ownership projection; preserve the Linear link only for a Linear-authoritative grace leg. | `index.html`; current #813 candidate; second-pass static/deployed-source and test-contract census 2026-07-14. |
