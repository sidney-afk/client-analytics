...
- [x] **Standardize authority vocabulary (F55, 2026-07-28):** every browser, EF, script, reconciler,
  and n8n guard now accepts only canonical `linear`/`syncview`. The backend-only `supabase` alias
  was removed/migrated, malformed/partial/legacy values are rejected consistently, and
  `test/f55-authority-vocabulary-contract.js` is the all-consumer contract test.
