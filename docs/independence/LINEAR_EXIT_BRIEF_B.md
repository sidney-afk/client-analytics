...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [migration] Apply the COMPOSED artifact from scripts/native-intake-named-append-compose.js (composed_sha256 2571a909971f2bc52ef270400b7666c2b529d36ab4f6a56044f00d1b2ecf61ec, **66665 BYTES** / 66,659 characters, one outer transaction) — this is native-only-intake and named-append together. **The brief originally said "66659 bytes" and that number is a CHARACTER count, not a byte count.** Running the composer gives `r.sql.length === 66659` and `Buffer.byteLength(r.sql,'utf8') === 66665`; the SQL holds three non-ASCII characters at 3 bytes each. `wc -c` and every other byte measure report **66665**. Since the rule here is to refuse on a mismatch, publishing the character count as bytes would make an operator refuse the CORRECT artifact, so verify against 66665 bytes or 66659 characters and never against the other pairing. Never apply 2026-09-05-native-only-intake.sql as a standalone step, and never apply it after 2026-09-07-production-intake-append-v8.sql: both `create or replace` the SAME `public.production_intake_append(p_batch_id text, p_expected_updated_at timestamptz, p_rows jsonb, p_events jsonb)` (native-only-intake:268, v8:168), identical argument lists, no overload, so whichever is applied second silently replaces the other's whole body. **VERIFIED BY RUNNING THE COMPOSER** at `origin/integration/linear-exit-candidate-20260906` (= candidate `5bcc03b`), which carries both it and `migrations/2026-09-07-native-intake-named-append.sql`: `fromRepository()` returns the `composed_sha256` quoted above, `contract: native-intake-named-atomic-v1`, `outer_transactions: 1`, `executed: false`. Composition, read out of the script: it prefixes `\set ON_ERROR_STOP on`, splices out the native file's trailing `commit;` and the hybrid file's leading `begin;`, and asserts exactly two transaction statements survive, so the whole install is one transaction that aborts entire. The hybrid half's guard raises `native_intake_named_append_prerequisite_missing` unless `public.production_native_intake_epochs()` exists AND `zz_native_intake_receipt_guard` on `public.mirror_outbox` is enabled (`tgenabled='O'`) — inside the composed file the native half creates both first, so the guard's real job is refusing the hybrid file applied ALONE. `production_intake_append` is `create or replace`d TWICE inside the transaction (native body, then the named body); the LAST one wins. `production_component_fill` is defined once, by the native half, and the hybrid half does not touch it. **[RESTORED 2026-09-08 · sources: scripts/native-intake-named-append-compose.js and migrations/2026-09-07-native-intake-named-append.sql lines 1-11, both @origin/integration/linear-exit-candidate-20260906, composer executed in a worktree at that commit; migrations/2026-09-05-native-only-intake.sql:268; migrations/2026-09-07-production-intake-append-v8.sql:168. Byte/character correction promoted to the headline after Codex review on #1352 (P1).]**
    undo: Not cleanly, and the fragment that stops at "dropping production_" understates it. What is actually owed, in order: (1) re-apply the PRIOR body of every `create or replace`d RPC — `public.production_intake_append(text,timestamptz,jsonb,jsonb)` from `migrations/2026-09-07-production-intake-append-v8.sql:168`, `public.production_intake_root_begin(jsonb,jsonb,jsonb)` from `migrations/2026-09-05-native-intake-root-manifest.sql:28`, and — MISSING FROM THE SURVIVING TEXT — `public.production_component_fill(text,timestamptz,text,jsonb,jsonb)` from `migrations/2026-08-31-production-component-fill.sql:87`, which native-only-intake also replaces (its line 633, same signature). Reverting only the append body leaves the native component-fill body live. (2) `drop trigger zz_native_intake_receipt_guard on public.mirror_outbox;` then `drop function public.production_native_intake_receipt_guard();`, `drop function public.production_intake_epoch_read(text,text,text,text,text,text,jsonb,jsonb);`, `drop function public.production_native_intake_epochs();`. (3) `alter table public.production_intake_manifests drop column native_epochs;` — that is the `production_...` the clipped line was reaching for. (4) The `native_intake_epochs` row inserted into `public.syncview_runtime_flags`: set both teams back to `{"enabled":false,"epoch":null}`. **DO NOT DELETE THE ROW** unless the epoch functions are already gone — `production_native_intake_epochs()` reads it and raises when it is absent or malformed. (5) NOT REVERSIBLE, and this is the part to plan around rather than undo: accepted manifests and terminal native receipts are retained by design (the migration's line 2), and the receipt guard refuses to let them be deleted. Reverse behaviour by flipping the epoch flag off first and only then unwinding structure. **[RESTORED 2026-09-08 · sources: migrations/2026-09-05-native-only-intake.sql lines 1-2, 4-6, 8-10, 14, 38, 83, 133-136, 268, 633, 1013-1019; migrations/2026-08-31-production-component-fill.sql:87; migrations/2026-09-07-production-intake-append-v8.sql:168]**
