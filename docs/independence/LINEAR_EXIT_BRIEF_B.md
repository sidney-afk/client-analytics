...
**This file: 13 executable lines restored, 0 left unrestored.** Two carry
...
replaces, and the B7 label-capture `mitigate:` is superseded by events — the
capture was taken on 2026-09-08 (OPEN_REPAIRS item 170), so that lane is no
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [migration] Apply the COMPOSED artifact from scripts/native-intake-named-append-compose.js (composed_sha256 2571a909971f2bc52ef270400b7666c2b529d36ab4f6a56044f00d1b2ecf61ec, 66659 bytes, one outer transaction) — this is native-only-intake and named-append together. Never apply 2026-09-05-native-only-intake.sql as a standalone step, and never apply it after 2026-09-07-production-intake-append-v8.sql. THE REASON, verified in both files: each of them `create or replace`s the SAME function `public.production_intake_append(p_batch_id text, p_expected_updated_at timestamptz, p_rows jsonb, p_events jsonb)` — native-only-intake at its line 268, v8 at its line 168 — with identical argument lists, so there is no overload and whichever is applied second silently replaces the other's whole body. Applied alone after v8, native-only-intake reverts the ordinal/name predicates; applied before, v8 reverts native admission and the receipt path. Only the composed artifact installs both bodies in one transaction. TWO THINGS THIS SESSION COULD NOT VERIFY AND YOU MUST BEFORE RUNNING IT: (a) neither `scripts/native-intake-named-append-compose.js` nor `migrations/2026-09-07-native-intake-named-append.sql` exists on ANY branch reachable from this repository clone — they are described only in OPEN_REPAIRS item 175, from candidate `5bcc03bd7` — so the composed_sha256 and byte length above are unchecked; recompute them from the artifact you actually hold and refuse to run it if they disagree. (b) Item 175 records that the composed migration opens with a `do $$` block raising `native_intake_named_append_prerequisite_missing` unless `public.production_native_intake_epochs()` exists AND the `zz_native_intake_receipt_guard` trigger on `public.mirror_outbox` is enabled — a wrong install order refuses rather than half-installing — but that guard was read at `5bcc03bd7` and has never been run against a live install. **[RESTORED 2026-09-08 · sources: migrations/2026-09-05-native-only-intake.sql:268 and migrations/2026-09-07-production-intake-append-v8.sql:168 (identical signatures); docs/ops/OPEN_REPAIRS.md item 175]**
...
RISKS
  - THE NAMED-APPEND ORDERING HAZARD. migrations/2026-09-05-native-only-intake.sql line 268 and migrations/2026-09-07-production-intake-append-v8.sql both `create or replace function public.production_intake_append(text,timestamptz,jsonb,jsonb)`. The nat
    mitigate: Do NOT install 2026-09-05-native-only-intake.sql as a standalone step. Install the COMPOSED artifact from scripts/native-intake-named-append-compose.js, which splices native-only-intake and named-append into ONE transaction. **WHAT THE ORIGINAL LINE CLAIMED TO HAVE VERIFIED CANNOT BE RE-VERIFIED HERE, so it is not continued: the composer script and the composed migration are on no branch this repository can reach** (searched: main, the coordinator branch, and every lane branch on origin) — they are attested only by OPEN_REPAIRS item 175, from candidate `5bcc03bd7`. Whoever runs this must read the artifact they actually hold and confirm the splice themselves. What IS independently confirmed, from the two files that do exist: `migrations/2026-09-05-native-only-intake.sql:268` and `migrations/2026-09-07-production-intake-append-v8.sql:168` declare the same `public.production_intake_append(text,timestamptz,jsonb,jsonb)` with identical arguments, so the second one applied wins outright — that is the whole hazard, and it is real. Item 175 additionally records the composed migration's fail-closed preamble (`native_intake_named_append_prerequisite_missing` unless `production_native_intake_epochs()` exists and `zz_native_intake_receipt_guard` is enabled), read at that candidate and never executed. **[RESTORED 2026-09-08 · sources: migrations/2026-09-05-native-only-intake.sql:268; migrations/2026-09-07-production-intake-append-v8.sql:168; docs/ops/OPEN_REPAIRS.md item 175. Gap stated rather than filled: the composer and its output are unreadable from here.]**
