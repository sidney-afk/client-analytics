...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [migration] Apply migrations/2026-09-05-native-intake-reconcile.sql (server-owned completion of accepted native intake + production_card_provenance table and its two card-table triggers).
    undo: drop trigger zz_production_card_provenance on calendar_posts and on sample_reviews; drop the reconcile RPCs; drop table production_card_provenance. Append-only fact-recording triggers that never refuse or reorder a write, so dropping them loses history nothing else records — and the migration's own header says "Rollback retains every accepted row this creates", so prefer revoking execute from service_role and leaving the objects installed. "The reconcile RPCs" is not a list anyone can guess, so here it is in full: `production_intake_reconcile_iso(timestamptz)`, `production_intake_reconcile_reason(text,text)`, `production_intake_reconcile_state(text)`, `production_intake_reconcile_record(public.production_intake_manifests,text,text,text,jsonb)`, `production_intake_reconcile_children(text,text,boolean)`, `production_intake_reconcile_cards(text,text,boolean)`, `production_intake_reconcile_backlog(int,text)`, `production_intake_reconcile_summary()`, plus the three provenance helpers `production_card_signature(text,jsonb)`, `production_card_slot(text)` and `production_card_provenance_record()` (drop the two triggers before that last one). `production_card_provenance` carries its own index `production_card_provenance_card_idx` and drops with the table; it is RLS-enabled with only `select` granted to service_role, so leaving it in place exposes nothing. **[RESTORED 2026-09-08 · source: migrations/2026-09-05-native-intake-reconcile.sql lines 1-9, 48, 61, 85, 239, 272-287, 298, 314, 320, 359-361, 375, 577, 831, 865 (read at feat/native-existing-assignee-20260906)]**
  - [migration] Apply migrations/2026-09-05-native-label-catalog-foundation.sql then migrations/2026-09-06-native-label-writes.sql (the second alters the first's table).
    undo: Both seed mode:provider and refuse to serve until a version is activated, so installed-but-provider is a genuine no-op state. Undo = drop the two triggers on mirror_outbox (zzz_native_label_receipt_guard, zzz_native_label_truncate_guard), then the new RPCs they and the writer depend on: `production_native_label_receipt_guard()`, `production_native_label_truncate_guard()`, `production_labels_write(jsonb,jsonb)`, `production_label_catalog_capability()`, `production_label_catalog_read_attested(uuid,text)`, `production_label_catalog_stage_attested(uuid,jsonb,jsonb)`, and `alter table public.production_label_catalog_versions drop column operator_attestation;` — that is all of 2026-09-06-native-label-writes.sql except its runtime-flag row. Reversing the FOUNDATION as well additionally means dropping `production_label_catalog_stage(uuid,jsonb)`, `_read_version(uuid,text)`, `_validate_selection(uuid,text,jsonb,jsonb)`, `_activate(uuid)`, `_read_active(text)`, `_check_manifest(jsonb)`, the two immutability triggers `production_label_catalog_no_change` / `production_label_catalog_no_truncate`, the function `production_label_catalog_immutable()`, and finally the table `production_label_catalog_versions`. TWO HAZARDS THE ORIGINAL LINE DID NOT REACH. (1) Do not delete the `production_native_label_catalog` row from `syncview_runtime_flags` while the receipt guard is installed: `production_label_catalog_capability()` raises `native_label_catalog_config_invalid` when the row is missing or malformed, and the guard calls it on every insert whose `operation='labels'` and `dedup_key like 'write-ui:labels:deliverable:%'` — so a deleted flag row breaks label writes with a confusing error rather than turning them off. Set `mode` instead. (2) While `zzz_native_label_receipt_guard` exists, any `mirror_outbox` row carrying `_native_label_catalog_version` cannot be deleted (`native_label_receipt_retained`) and `mirror_outbox` cannot be truncated while one exists. Drop the triggers FIRST if the intent is to remove receipts — and read the retention line before deciding you want to. **[RESTORED 2026-09-08 · sources: migrations/2026-09-06-native-label-writes.sql lines 1-8, 14, 44, 57-70, 74-118, 121-131, 134, 157-166; migrations/2026-09-05-native-label-catalog-foundation.sql lines 1-19, 24-36, 40, 237-259]**
  - [migration] Apply migrations/2026-09-06-native-existing-assignment.sql.
    undo: Seeds native_assignment_epochs as mode:provider for both teams, so it is inert on install. Undo = drop triggers zzz_native_assignment_receipt_guard / zzz_native_assignment_truncate_guard and then the functions — of which there are FIVE, not three: `production_native_assignment_receipt_guard()`, `production_native_assignment_truncate_guard()`, `production_assignee_write(jsonb,jsonb)`, `production_assignment_context(jsonb)` and `production_assignment_epoch(text)`. Drop the triggers before their two functions; `production_assignee_write` and `production_assignment_context` both reach `production_assignment_epoch`, so drop that one last. Once an epoch has been enabled and native assignment receipts exist, the structural undo stops being complete: a `mirror_outbox` row carrying `_native_assignment_epoch` cannot be deleted while the guard is installed (`native_assignment_receipt_retained`) and `mirror_outbox` cannot be truncated while one exists, and those receipts are the only record that the write went native rather than to the provider. So the behavioural undo is the one to reach for first — set the team's `mode` back to `"provider"` in the `native_assignment_epochs` flag (or `"hold"`, which raises `assignment_authority_unavailable` on every assignment write for that team) and leave the objects installed. Do not delete the flag row: `production_assignment_epoch(text)` raises `assignment_authority_unavailable` when it is absent or malformed. **[RESTORED 2026-09-08 · source: migrations/2026-09-06-native-existing-assignment.sql lines 1-6, 8-33, 34, 85-120, 121-133, 134, 179-184 (read at feat/native-existing-assignee-20260906)]**
