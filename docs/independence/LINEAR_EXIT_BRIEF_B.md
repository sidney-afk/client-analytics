...
66,665 UTF-8 bytes. Its `composed_sha256` verifies exactly as quoted.
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [migration] Apply the COMPOSED artifact from scripts/native-intake-named-append-compose.js (composed_sha256 2571a909971f2bc52ef270400b7666c2b529d36ab4f6a56044f00d1b2ecf61ec, 66659 bytes, one outer transaction) — this is native-only-intake and named-append together. Never apply 2026-09-05-native-only-intake.sql as a standalone step, and never apply it after 2026-09-07-production-intake-append-v8.sql: both `create or replace` the SAME `public.production_intake_append(p_batch_id text, p_expected_updated_at timestamptz, p_rows jsonb, p_events jsonb)` (native-only-intake:268, v8:168), identical argument lists, no overload, so whichever is applied second silently replaces the other's whole body. **VERIFIED BY RUNNING THE COMPOSER** at `origin/integration/linear-exit-candidate-20260906` (= candidate `5bcc03b`), which carries both it and `migrations/2026-09-07-native-intake-named-append.sql`: `fromRepository()` returns `composed_sha256` exactly as quoted above, `contract: native-intake-named-atomic-v1`, `outer_transactions: 1`, `executed: false`. **ONE CORRECTION: "66659 bytes" is a CHARACTER count, not bytes.** The composed SQL is 66,659 JavaScript characters and **66,665 UTF-8 bytes** (three non-ASCII characters at 3 bytes each). Check whichever you measure and do not let `wc -c` reporting 66665 look like the wrong artifact. Composition, read out of the script: it prefixes `\set ON_ERROR_STOP on`, splices out the native file's trailing `commit;` and the hybrid file's leading `begin;`, and asserts exactly two transaction statements survive, so the whole install is one transaction that aborts entire. The hybrid half's guard (`do $$ ... raise exception 'native_intake_named_append_prerequisite_missing'`) refuses unless `public.production_native_intake_epochs()` exists AND the `zz_native_intake_receipt_guard` trigger on `public.mirror_outbox` is enabled (`tgenabled='O'`) — inside the composed file the native half creates both first, so the guard's real job is refusing the hybrid file applied ALONE. Note `production_intake_append` is `create or replace`d TWICE inside the composed transaction (native body, then the named body); the LAST one wins and is the one that ships. `production_component_fill` is defined once, by the native half, and the hybrid half does not touch it. **[RESTORED 2026-09-08 · sources: scripts/native-intake-named-append-compose.js and migrations/2026-09-07-native-intake-named-append.sql lines 1-11, both @origin/integration/linear-exit-candidate-20260906; composer executed in a worktree at that commit; migrations/2026-09-05-native-only-intake.sql:268; migrations/2026-09-07-production-intake-append-v8.sql:168. Digest and byte count verified after Codex review on #1352; an earlier pass wrongly reported these artifacts as unreachable.]**
