...
**One un-clipped line above is also wrong and was left as written**, because
this pass only edits truncated lines: `- [migration] Apply
migrations/2026-09-07-production-intake-append-v8.sql (still SOURCE ONLY per
EXECUTION_LOG.md:6270)`. It is NOT source-only — `EXECUTION_LOG.md` (2026-09-07,
around line 6333) records it APPLIED by the owner in the SQL Editor, "Success.
No rows returned", before that day's dispatch. Re-applying it is a
`create or replace` and harmless, but do not plan around it as pending.
