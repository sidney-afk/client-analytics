...
DONE WHEN
  ...
  - A Section 4 dispatch of `deploy-f27-section4-closures.yml` completes green with `production-write` at the new digest.
...
WORK ITEMS
  ...
  [B8] (medium risk, mixed) Re-pin the deploy lane and its guard test
     Bump PRODUCTION_WRITE_SOURCE_SHA256 (and PRODUCTION_WRITE_FILE_COUNT to '6' only if you kept the brief-media import) in .github/workflows/deploy-f27-section4-closures.yml, regenerating with `node scripts/ef-fingerprint.js <sha> --slugs=production-write --expected-only`. LEAVE LINEAR_OUTBOUND_SOURCE_SHA256 at 1489a4c2... — the candidate bumps it to 43329cdf... for the outbound-cutoff lane, and lane B must not drag that in; the lane redeploys all four functions regardless, so an unchanged pin just redeploys byte-identical bytes. test/f27-section4-deploy-lane.js runs ef-fingerprint against the repo and asserts the workflow pins match (line ~595 `currentCandidatesMatch`), and asserts docs/ops/EF
     files: .github/workflows/deploy-f27-section4-closures.yml, test/f27-section4-deploy-lane.js, docs/ops/EF_DEPLOY_MANIFEST.md
...
FILES YOU OWN (touch nothing else)
  ...
  .github/workflows/deploy-f27-section4-closures.yml (PRODUCTION_WRITE_* env keys ONLY — LINEAR_OUTBOUND_* belongs to the outbound-cutoff lane)
  test/f27-section4-deploy-lane.js (production-write candidate block only)
  docs/ops/EF_DEPLOY_MANIFEST.md (production-write closure row only, and only if file count moves)
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [edge-function-deploy] Dispatch .github/workflows/deploy-f27-section4-closures.yml with operation=deploy-reviewed-release. Deploys all four of batch-write, deliverable-write, linear-outbound, production-write.
...
RISKS
  ...
  - FILE-COUNT / PIN COLLISION IN THE DEPLOY LANE. The candidate bumps PRODUCTION_WRITE_FILE_COUNT 5 -> 6 (new import `../_shared/native-brief-media.mjs`) AND LINEAR_OUTBOUND_SOURCE_SHA256 1489a4c2 -> 43329cdf in the same workflow file that another lane 
    mitigate: Lane B edits only the PRODUCTION_WRITE_* keys. Drop the native-brief-media import (one import line + one call at index.ts:5306) unless the brief-media lane is shipping in the same release, keeping file count at 5. Regenerate with `node scripts/ef-fin
