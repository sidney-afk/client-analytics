...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [migration] Apply the COMPOSED artifact from scripts/native-intake-named-append-compose.js (composed_sha256 2571a909971f2bc52ef270400b7666c2b529d36ab4f6a56044f00d1b2ecf61ec, **66665 BYTES** / 66,659 characters, one outer transaction) — this is native-only-intake and named-append together. **The brief originally said "66659 bytes" and that number is a CHARACTER count, not a byte count.** Running the composer gives `r.sql.length === 66659` and `Buffer.byteLength(r.sql,'utf8') === 66665`; the SQL holds three non-ASCII characters at 3 bytes each. `wc -c` and every other byte measure report **66665**. Since the rule here is to refuse on a mismatch, publishing the character count as bytes would make an operator refuse the CORRECT artifact, so verify against 66665 bytes or 66659 characters and never against the other pairing. Never apply 2026-09-05-native-only-intake.sql as a standalone step, and never apply it after 2026-09-07-production-intake-append-v8.sql: both `create or replace` the SAME `public.production_intake_append(p_batch_id text, p_expected_updated_at timestamptz, p_rows jsonb, p_events jsonb)` (native-only-intake:268, v8:168), identical argument lists, no overload, so whichever is applied second silently replaces the other's whole body. **VERIFIED BY RUNNING THE COMPOSER** at `origin/integration/linear-exit-candidate-20260906` (= candidate `5bcc03b`), which carries both it and `migrations/2026-09-07-native-intake-named-append.sql`: `fromRepository()` returns the `composed_sha256` quoted above, `contract: native-intake-named-atomic-v1`, `outer_transactions: 1`, `executed: false`. Composition, read out of the script: it prefixes `\set ON_ERROR_STOP on`, splices out the native file's trailing `commit;` and the hybrid file's leading `begin;`, and asserts exactly two transaction statements survive, so the whole install is one transaction that aborts entire. The hybrid half's guard raises `native_intake_named_append_prerequisite_missing` unless `public.production_native_intake_epochs()` exists AND `zz_native_intake_receipt_guard` on `public.mirror_outbox` is enabled (`tgenabled='O'`) — inside the composed file the native half creates both first, so the guard's real job is refusing the hybrid file applied ALONE. `production_intake_append` is `create or replace`d TWICE inside the transaction (native body, then the named body); the LAST one wins. `production_component_fill` is defined once, by the native half, and the hybrid half does not touch it. **[RESTORED 2026-09-08 · sources: scripts/native-intake-named-append-compose.js and migrations/2026-09-07-native-intake-named-append.sql lines 1-11, both @origin/integration/linear-exit-candidate-20260906, composer executed in a worktree at that commit; migrations/2026-09-05-native-only-intake.sql:268; migrations/2026-09-07-production-intake-append-v8.sql:168. Byte/character correction promoted to the headline after Codex review on #1352 (P1).]**
    undo: Not cleanly, and the fragment that stops at "dropping production_" understates it. **ORDER MATTERS AND THE FLAG COMES FIRST** — an earlier version of this line numbered the flag fourth while its closing sentence said to flip it first, which is two contradictory rollback orders in one instruction. The order is: (1) **Stop admission before touching anything.** Set `syncview_runtime_flags.native_intake_epochs` to `{"enabled":false,"epoch":null}` for both teams. Do NOT delete the row — `production_native_intake_epochs()` reads it and raises when it is absent or malformed. Until this is done, new requests are still admitted, and steps 2-4 would be replacing or removing RPCs underneath a gateway that is calling them. (2) Re-apply the PRIOR body of every `create or replace`d RPC — `public.production_intake_append(text,timestamptz,jsonb,jsonb)` from `migrations/2026-09-07-production-intake-append-v8.sql:168`, `public.production_intake_root_begin(jsonb,jsonb,jsonb)` from `migrations/2026-09-05-native-intake-root-manifest.sql:28`, and — MISSING FROM THE SURVIVING TEXT — `public.production_component_fill(text,timestamptz,text,jsonb,jsonb)` from `migrations/2026-08-31-production-component-fill.sql:87`, which native-only-intake also replaces (its line 633, same signature). Reverting only the append body leaves the native component-fill body live. (3) `drop trigger zz_native_intake_receipt_guard on public.mirror_outbox;` then `drop function public.production_native_intake_receipt_guard();`, `drop function public.production_intake_epoch_read(text,text,text,text,text,text,jsonb,jsonb);`, `drop function public.production_native_intake_epochs();` — this one last, since the flag read above depends on it, and only once nothing calls it. (4) `alter table public.production_intake_manifests drop column native_epochs;` — that is the `production_...` the clipped line was reaching for. (5) NOT REVERSIBLE, and this is the part to plan around rather than undo: accepted manifests and terminal native receipts are retained by design (the migration's line 2) and the receipt guard refuses to let them be deleted. If the flag is off, the system is already behaviourally rolled back; steps 2-4 are structural tidying and are the ones that can break a live gateway if taken in the wrong order. **[RESTORED 2026-09-08 · sources: migrations/2026-09-05-native-only-intake.sql lines 1-2, 4-6, 8-10, 14, 38, 83, 133-136, 268, 633, 1013-1019; migrations/2026-08-31-production-component-fill.sql:87; migrations/2026-09-07-production-intake-append-v8.sql:168. Order corrected after Codex review on #1352 (P1): the numbered list and the closing sentence disagreed about when the flag goes off.]**
  ...
  - [migration] Apply migrations/2026-09-05-native-label-catalog-foundation.sql then migrations/2026-09-06-native-label-writes.sql (the second alters the first's table).
    undo: Both seed mode:provider and refuse to serve until a version is activated, so installed-but-provider is a genuine no-op state. **STOP THE CALLERS BEFORE YOU DROP WHAT THEY CALL.** This is a sequence, not a set of inverse statements: every structural step below is safe only once nothing is invoking the objects it removes. **Step 0: set `production_native_label_catalog.mode` to `"hold"`** (new label writes refuse with `native_label_catalog_held`, gateway retained) — never delete the flag row, because `production_label_catalog_capability()` raises `native_label_catalog_config_invalid` when it is missing and the guard calls it on every insert whose `operation='labels'` and `dedup_key like 'write-ui:labels:deliverable:%'`, which fails label writes with a confusing error instead of a clean stop. Then: drop the two triggers on mirror_outbox (zzz_native_label_receipt_guard, zzz_native_label_truncate_guard), then their functions `production_native_label_receipt_guard()` and `production_native_label_truncate_guard()`, then `production_labels_write(jsonb,jsonb)`, `production_label_catalog_read_attested(uuid,text)`, `production_label_catalog_capability()` and `production_label_catalog_stage_attested(uuid,jsonb,jsonb)` — capability last of those four, since the guard and the writer reach it — and `alter table public.production_label_catalog_versions drop column operator_attestation;`. That is all of 2026-09-06-native-label-writes.sql except its runtime-flag row. Reversing the FOUNDATION as well additionally means dropping `production_label_catalog_stage(uuid,jsonb)`, `_read_version(uuid,text)`, `_validate_selection(uuid,text,jsonb,jsonb)`, `_activate(uuid)`, `_read_active(text)`, then `_check_manifest(jsonb)` (the others call it), the two immutability triggers `production_label_catalog_no_change` / `production_label_catalog_no_truncate`, the function `production_label_catalog_immutable()`, and finally the table `production_label_catalog_versions`. ONE HAZARD THE ORIGINAL LINE DID NOT REACH: while `zzz_native_label_receipt_guard` exists, any `mirror_outbox` row carrying `_native_label_catalog_version` cannot be deleted (`native_label_receipt_retained`) and `mirror_outbox` cannot be truncated while one exists. Drop the triggers FIRST if the intent is to remove receipts — and read the retention line before deciding you want to. **[RESTORED 2026-09-08 · sources: migrations/2026-09-06-native-label-writes.sql lines 1-8, 14, 44, 57-70, 74-118, 121-131, 134, 157-166; migrations/2026-09-05-native-label-catalog-foundation.sql lines 1-19, 24-36, 40, 237-259. Stop-first precondition and drop order added in the #1352 sequencing sweep.]**
  ...
  - [runtime-flag] Stage + attest a real label catalog version (production_label_catalog_stage_attested) and set production_native_label_catalog to {schema_version:1, mode:"native", version_id:<uuid>}. REQUIRES a Linear label export taken before Sept 15.
    undo: mode:"hold" (blocks new label writes with 503 native_label_catalog_held, gateway retained) or mode:"provider" — but provider stops working the moment Linear dies, so hold is the real containment. Catalog versions are immutable by trigger and must never be edited, deleted or truncated to fix a bad capture: `production_label_catalog_no_change` (before update or delete, per row) and `production_label_catalog_no_truncate` (before truncate, per statement) both raise `label_catalog_immutable` unconditionally. The way to retire a wrong version is therefore to stage and attest a NEW one and repoint `production_native_label_catalog.version_id` at it, leaving the bad row in place as evidence. Note while you are here that `production_label_catalog_activate(uuid)` and `production_label_catalog_read_active(text)` are held stubs — both raise `label_catalog_activation_held` on every call — so activation and de-activation are the RUNTIME FLAG, not those RPCs; do not go looking for a de-activate call. And do not delete the flag row as a way of switching off: `production_label_catalog_capability()` raises `native_label_catalog_config_invalid` when it is missing, which fails label writes with a misleading error instead of a clean hold. **[RESTORED 2026-09-08 · sources: migrations/2026-09-05-native-label-catalog-foundation.sql lines 24-36, 237-248; migrations/2026-09-06-native-label-writes.sql lines 6-8, 57-70, 92-93]**
