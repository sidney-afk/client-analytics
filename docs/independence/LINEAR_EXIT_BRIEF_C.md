...
in its marker. One carries a correction: the "four Linear-only n8n workflows"
list is SIX workflows, and one of them is F48's still-open
`webhook/editors-week`. The `log-linear-submission` clause, first published
here as an unrecoverable gap, turned out to be recoverable and is now restored
from the touchpoint inventory and the system map.
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [n8n-edit] Edit VIDEO PRODUCTION AUTOMATION (workflow id BrJSe8zCKUccfmIq) to replace the two Submit entry edges so video-form / graphic-form forward to the native gateway instead of creating Linear issues, using scripts/n8n-f44-native-adapter.js against a fresh private capture. The two entries are named in the adapter: `{video: {path:'video-form', previous:'F44 Normalize Video'}, graphics: {path:'graphic-form', previous:'F44 Normalize Graphics'}}` (scripts/n8n-f44-native-adapter.js:11), and it binds `endpointContract: 'legacy_intake_receive-v1'` plus a `VALIDATORS_SHA` over a slice of index.html, refusing with `browser_response_contract_drift` if that slice has moved — so generate and install this edit BEFORE deleting the browser-side legacy Submit code, or re-pin the SHA in the same commit. **The `log-linear-submission` sheet is NOT touched by this edit, and its standing disposition is already written down.** It is a separate branch of the same workflow (`Webhook6 -> Append row in sheet`), called from the browser at index.html:30049-30069, and it appends `{Timestamp, Client Name, Mode, Webhook JSON}` to the master sheet's "Linear Submissions" tab. It does not mutate Linear, so nothing about the Linear exit forces it off. `LINEAR_CUTOVER_TOUCHPOINT_INVENTORY.md:117` dispositions it **keep-until-B5 (or retain as non-Linear telemetry)**, and `SYSTEM_MAP.md:470,540` is explicit that it "remains telemetry, not the durable acceptance receipt" and "post-commit telemetry only" — the native gateway's own receipt is the acceptance record, not this sheet. The open bit, per `docs/audits/2026-07-05-n8n.md:431`, is that this workflow's non-Linear branches (this one and the `add-to-calendar` legacy sheet writer) "need explicit disposition in the spec"; leaving it logging is safe, and retiring it is a separate decision that is not this edit's to take. **[RESTORED 2026-09-08 · beyond the named field set (an `- [n8n-edit]` line) · sources: scripts/n8n-f44-native-adapter.js:7,11,33,50; docs/independence/LINEAR_CUTOVER_TOUCHPOINT_INVENTORY.md:117; docs/independence/SYSTEM_MAP.md:470,540; docs/audits/2026-07-05-n8n.md:186,431; docs/truth/N8N.md:93-94. The log-linear-submission clause was first published as an unrecoverable gap; it was recoverable and this is it.]**
  ...
  - [n8n-edit] Disable (do not delete) the four Linear-only n8n workflows once their browser callers are gone. **THE COUNT IS WRONG AND THE LIST WAS CLIPPED: there are SIX, and they are** — SyncView Calendar - Linear Set Status (`VQqqeY9B2GZbh2Bt`, `webhook/linear-set-status`), SyncView Calendar - Linear Add Comment (`8stSpZUiyG7f2LQX`, `webhook/linear-add-comment`), Calendar - Linear Sub-Issues (`Nk3pwR6Fbl4VAPqH`, `webhook/linear-subissues`), Calendar - Linear Issue Statuses (`GP8CSZDNcy5sGdFr`, `webhook/linear-issue-statuses`), SyncView Workload - Tweak Comments (`d7Dod7OuQsVsl1CN`, `webhook/linear-tweak-comments`), and Editors - Labor Week (`rhDX5VfnmOylc8o7`, `webhook/editors-week`). **Editors - Labor Week is F48 and F48 IS OPEN**: `webhook/editors-week` is still a deployed, unauthenticated workflow returning the confidential metadata F48 tracks, and removing its browser caller did not and cannot close it. Do not record F48 as closed on any diff; it closes when that workflow stops answering. Note also that disabling `linear-set-status` removes the only n8n dueDate writer (+2 days when overdue, on every call), which is a behaviour change to decide on its own — see the separate risk below. **[RESTORED 2026-09-08 · beyond the named field set (an `- [n8n-edit]` line) · sources: docs/audits/2026-07-03-n8n.md lines 68-73; docs/truth/ENDPOINTS.md:36-38; docs/truth/N8N.md:88-89; scripts/write-ui-n8n-authority-gates.js:24-25; docs/ops/OPEN_REPAIRS.md item 178 (F48 not closed)]**
    undo: Re-publish. Unpublishing is the documented reversible step (see the Weekly Slack – Top Reel precedent: 'workflow BTxic5NSaCMtZMh6 unpublished in n8n, definition intact for re-enabling'). Capture each workflow's activeVersionId before unpublishing. **BUT THE REVERSAL IS ONLY MEANINGFUL WHILE LINEAR STILL ANSWERS.** Every one of these six is a Linear bridge: re-publishing them after the account lapses on 2026-09-15, or after lane F revokes `LINEAR_MIRROR_API_KEY` / `LINEAR_API_KEY` / `LINEAR_READ_API_KEY`, restores workflows that fail on every call. The definitions survive; the capability does not. So this undo is a genuine rollback only inside the window before those two events, and after them the honest state is retirement rather than a pause — which is also why unpublishing (not deleting) is still the right forward action: it keeps the definitions as evidence of what the estate used to do. One of the six is the exception that must NOT be treated as retired-by-lapse: `Editors - Labor Week` / `webhook/editors-week` is F48, and F48 closes when that workflow stops answering, not when Linear stops answering it. An unauthenticated webhook that returns confidential metadata keeps doing so after the provider behind it dies. **[RESTORED 2026-09-08 · sources: docs/audits/2026-07-03-n8n.md:68-73; docs/truth/ENDPOINTS.md:36-38; this brief's own disable list and lane F's credential-revocation step; docs/ops/OPEN_REPAIRS.md item 178 (F48 not closed). This line was NOT truncated; the post-revocation boundary added in the round-five sweep.]**
