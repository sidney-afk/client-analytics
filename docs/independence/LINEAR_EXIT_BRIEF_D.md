...
correction: the deploy `undo:` credits `ROLLBACK.md` with recording the
browser-only revert path, and `ROLLBACK.md` has no row for this lane at all —
adding it is part of shipping the lane.
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  - [edge-function-deploy] Dispatch https://github.com/sidney-afk/client-analytics/actions/workflows/deploy-onboarding-edge-functions.yml with `commit_sha` = the exact main tip carrying the lifted `production-comments` closure. This is workflow_dispatch-only with a main-ancestry guard and runs in the `production` GitHub Environment. It is NOT the F27 Section 4 lane, so it needs NO sealed rollback bundle, no Drive upload and no `confirm` string: `commit_sha` is its only input, it must be exactly 40 lowercase hex characters, and it must be an ANCESTOR of `origin/main` — not necessarily main's tip (workflow lines 36-40 and 68-74). That ancestry check runs first, from the trusted default branch, with no production secret in scope and no script from the dispatched commit executed; only after it passes is the dispatched tree checked out. Practically this means lane D's dispatch does NOT impose the Section 4 merge freeze — an ancestor SHA stays valid if main moves — but see the sequencing risk below, because the SHA you choose also decides what version of `linear-outbound` and `production-write` goes live. **[RESTORED 2026-09-08 · source: .github/workflows/deploy-onboarding-edge-functions.yml lines 1-21, 36-40, 44-53, 63-82]**
    undo: Re-dispatch the same workflow with an earlier main SHA. Rolls back all four functions together — see the risk below. The browser-only revert path is the cheaper lever and is genuinely safe here: reverting the index.html diff hides the panel without touching stored data (the whole lane is a pure reader — `production-comments/feedback.mjs` opens "Read-only staff projection" and issues no insert/update/delete, and COMPONENT_FEEDBACK.md states there is no new SQL), so nothing has to be un-written and the two levers can be pulled in either order. **ONE CORRECTION: `ROLLBACK.md` does NOT record this path today.** It has no row for the Feedback panel or for `production-comments`; the two-lever shape the fragment was invoking is the house pattern used by other rows (see the Production-tab creation row, ROLLBACK.md:191, "Two independent levers. **Browser:** … **Server:** …"). Adding the row is part of shipping this lane, not something to cite before it exists — AGENTS.md already requires `ROLLBACK.md` to be kept current for Production work. **[RESTORED 2026-09-08 · sources: supabase/functions/production-comments/feedback.mjs:1-4 @claude/lx-d-feedback; ROLLBACK.md (searched: no production-comments/Feedback row; pattern at :191); AGENTS.md:98]**
