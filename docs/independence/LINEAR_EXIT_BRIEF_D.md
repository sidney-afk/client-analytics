...
**This file: 6 executable lines restored, 0 left unrestored.** The deploy
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  - [edge-function-deploy] Dispatch https://github.com/sidney-afk/client-analytics/actions/workflows/deploy-onboarding-edge-functions.yml with `commit_sha` = the exact main tip carrying the lifted `production-comments` closure. This is workflow_dispatch-only with a main-ancestry guard and runs in the `production` GitHub Environment. It is NOT the F27 Section 4 lane, so it needs NO sealed rollback bundle, no Drive upload and no `confirm` string: `commit_sha` is its only input, it must be exactly 40 lowercase hex characters, and it must be an ANCESTOR of `origin/main` — not necessarily main's tip (workflow lines 36-40 and 68-74). That ancestry check runs first, from the trusted default branch, with no production secret in scope and no script from the dispatched commit executed; only after it passes is the dispatched tree checked out. Practically this means lane D's dispatch does NOT impose the Section 4 merge freeze — an ancestor SHA stays valid if main moves — but see the sequencing risk below, because the SHA you choose also decides what version of `linear-outbound` and `production-write` goes live. **[RESTORED 2026-09-08 · source: .github/workflows/deploy-onboarding-edge-functions.yml lines 1-21, 36-40, 44-53, 63-82]**
    undo: Re-dispatch the same workflow with an earlier main SHA. Rolls back all four functions together — see the risk below. The browser-only revert path is the cheaper lever and is genuinely safe here: reverting the index.html diff hides the panel without touching stored data (the whole lane is a pure reader — `production-comments/feedback.mjs` opens "Read-only staff projection" and issues no insert/update/delete, and COMPONENT_FEEDBACK.md states there is no new SQL). `ROLLBACK.md` DOES record it, on lane D's own branch: `claude/lx-d-feedback` ROLLBACK.md:107, the row "Component feedback reader — SyncLinear 'Feedback & tweaks' panel + Workload Tweak Needed popover (lane D, NOT YET DEPLOYED)". Its rollback column reads **"Browser-only revert (no deploy, no data touched): `git revert` the merge and let Pages redeploy — the panel disappears and the heading returns to 'Comments'. Stored comments and card cells are untouched because this lane only reads."** and then, for the server lever, that the dispatch is NOT per-function: workflow line 138 redeploys `linear-outbound production-write production-comments production-archive` from one `commit_sha` in that order, so rolling back here also rolls back whatever lanes B and C had live at that SHA. Its verification column says NOT VERIFIED LIVE, source-only, offline proof only. That row does not exist on `main` or on the coordinator branch, so it ships with the lane; check it is still present in the merge. **[RESTORED 2026-09-08 · sources: ROLLBACK.md:107 @claude/lx-d-feedback; supabase/functions/production-comments/feedback.mjs:1-4 @claude/lx-d-feedback; AGENTS.md:98. Corrected after Codex review on #1352: an earlier pass searched only the coordinator branch's ROLLBACK.md and wrongly reported the row as missing.]**
...
RISKS
  ...
  - The only deploy path for `production-comments` also redeploys `linear-outbound` and `production-write`. If Lane B/C have merged unreleased changes to those, lane D's dispatch releases them, and a rollback releases lane D's reader back to the old shap
    mitigate: Sequence the dispatch with Lane B/C. Read `.github/workflows/deploy-onboarding-edge-functions.yml` lines 1-20 and 129-165 before dispatch and confirm what those two functions are at the chosen SHA. State the SHA to the owner and DO NOT MERGE ANYTHING to `main` between handing it over and his run going green — not even a docs PR; that exact mistake rejected a dispatch on 2026-09-02 and four merged PRs did it again on 2026-08-08. Two facts to carry into that sequencing. (1) The release ORDER inside the set is `linear-outbound production-write production-comments production-archive` (workflow line ~138), chosen deliberately so the provider and the write gateway land before the readers: a new reader against an old `production-write` can have an edit mis-handled as a create, producing a silent duplicate comment, while new-writer/old-reader stays safe. Do not reorder it and do not deploy the reader alone. (2) Lane D's ancestry guard tolerates main moving, but the SHA still pins what B and C get released, so "an older SHA is still valid" is not a licence to pick a stale one. **[RESTORED 2026-09-08 · sources: .github/workflows/deploy-onboarding-edge-functions.yml lines 1-21 and 125-145; CLAUDE.md (dispatch freeze, the 2026-09-02 and 2026-08-08 rejections)]**
