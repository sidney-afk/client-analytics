...
RISKS
  ...
  - Every UPDATE on `deliverables` fires `track_b_deliverable_ledger_guard_after` (migrations/2026-07-06-b1-linear-data-model.sql:239-290), writing a `deliverable_events` row with `action='update'`, `source='system'`, `payload={"op":"UPDATE","reason":"rp
    mitigate: Accept it — it is an audit trail, not damage, and it is the trigger's designed behavior for a direct write. It does NOT enqueue `mirror_outbox`, so nothing is pushed to Linear. State the expected row count in the OPEN_REPAIRS entry so a later reader finds the number already explained instead of reading a burst of `source='system'` / `reason='rpc_bypass_guard'` events as an incident. The count is exactly one `deliverable_events` row per updated deliverable (`track_b_deliverable_ledger_guard_after`, migrations/2026-07-06-b1-linear-data-model.sql:239-290), so it is knowable before the run — but **key it the way the forward file keys it, not the way the manifest does.** `cmdRewrite` groups occurrences by `${occ.table}:${occ.id}` and emits ONE update per row that actually changed (`if (result.replaced === 0) { untouched += 1; continue; }`, scripts/linear-media-rescue.mjs:415-421), so the expected event count is the number of DISTINCT `deliverables` rows in the generated forward SQL — not manifest rows (which span two tables), not occurrences (a row holding three rescued images is still one update, hence one event), and not rows whose every occurrence was skipped for an unrescued file. Count the `deliverables` statements in `linear-media-rescue-forward.sql` and write THAT number down, then check the two agree afterwards. Comments produce no such rows — `production_comments` has no equivalent trigger — so the deliverables count is the whole of it. **[RESTORED 2026-09-08 · sources: docs/ops/LINEAR_MEDIA_RESCUE.md §3 "Expected side effects of the apply"; migrations/2026-07-06-b1-linear-data-model.sql:219-237 and :239-290; scripts/linear-media-rescue.mjs:406-422. The counting key was tightened after Codex round ten found the same fan-out error in this file's sibling line — AGENTS.md:153-166.]**
  ...
  - `RATE_LIMIT_PER_HOUR = 120` per actor with a reserve-then-count design means a burst upload refuses ITSELF at the boundary (policy.mjs:24, index.ts:257-277).
    mitigate: **BOTH NUMBERS IN THIS LINE WERE WRONG: 30 SECONDS SELF-REFUSES, AND "~150 FILES" IS OFF BY ROUGHLY AN ORDER OF MAGNITUDE.** Use the pace the shipped script already defaults to and do not override it downward: `const spacing = Number(process.env.SYNCVIEW_UPLOAD_SPACING_MS || 40_000)` plus up to 5 s of jitter (scripts/linear-media-rescue.mjs:331-334, 384), with its own comment giving the reason. 30 s sits exactly on the boundary: the window is `since = now - 3600000` with `.gte('created_at', since)`, the count INCLUDES the row the caller just reserved, and the refusal is `> RATE_LIMIT_PER_HOUR` (supabase/functions/description-image-upload/index.ts:183-206), so a run paced at 3600/120 refuses itself at the boundary and the reservation is deleted. **AND THE SIZE OF THE JOB:** '~150 files' is asserted with no basis. This brief's own E1 corpus is 1,166 in-cap brief files plus 75 comment files (1,141 distinct CONTENT hashes — a different key from the path the uploader dedupes on, `byKey` at :324-328), so the real figure is whatever `distinct_files=` the E1 scan prints, and it must come from that scan rather than from an estimate. At that order of magnitude, ~1,200 files at ~42.5 s each is about **14 hours**, not 75 minutes — plan it as a resumable multi-session job, not an unattended lunch break. Resumable is a property the script actually has: the out-map is written after every success and already-uploaded keys are skipped (`if (out[k]) { done += 1; continue; }`, :340), so it can be stopped and restarted without re-uploading. The second ceiling is not the binding one at this pace: `ROLE_LIMIT_PER_HOUR = 600` (policy.mjs:31) against ~85 uploads/hour. Do not raise either constant — that is a code change and a deploy for no benefit. **[CORRECTED 2026-09-08 · this line was NOT truncated, so it was outside this pass's original scope; corrected because it is a `mitigate:` an operator paces a live run from · sources: scripts/linear-media-rescue.mjs:324-340, 331-334, 384; supabase/functions/description-image-upload/index.ts:183-206; policy.mjs:24,31; this brief's own line 167 and its adversarial review at lines 263-265, whose `truth:` line is itself one of the ~284 clipped lines · found independently by Codex round eleven (P2)]**
