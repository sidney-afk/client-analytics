...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  ...
  - [storage] CONDITIONAL (E4 only): `update storage.buckets set file_size_limit = 26214400 where id = 'syncview-description-images';`
    undo: `update storage.buckets set file_size_limit = 4194304 where id = 'syncview-description-images';` — objects already stored above the old limit stay readable; the limit only gates new writes. That property is not mine to assert and is not platform folklore here: it is stated in `docs/ops/OPEN_REPAIRS.md` item 173 point 2, in the same parenthetical that supplies this undo. This is a BUCKET setting, not the project-wide Storage ceiling: the two are separate controls, this statement touches only the one row in `storage.buckets`, and it neither raises nor lowers the project-level ceiling (a Supabase project setting, and what a separate approval covers elsewhere in this programme). **REVERSING E4 IS THREE CONSTANTS, NOT ONE, AND NOT TWO.** Item 173 lists the whole change as "one statement plus two constants": the bucket row above, `MAX_BYTES` (`supabase/functions/description-image-upload/policy.mjs:17`) and **`MAX_DIMENSION = 8000`** (`policy.mjs:20`), which "would have to move with it or large captures still refuse" — a 25 MiB limit does not admit a file the dimension gate rejects at `policy.mjs:381/476/521`. `test/description-image-upload.js` pins two of the three (`policy.MAX_BYTES === 4*1024*1024` at :169 and the migration's `file_size_limit` 4194304 at :170), so CI catches a half-reversal of those but NOT a stranded `MAX_DIMENSION`. Reverse all three in one window, and redeploy the function (see the step below) or the bucket and the code disagree. **[RESTORED 2026-09-08 · sources: docs/ops/OPEN_REPAIRS.md item 173 point 2 (the undo sentence and the MAX_DIMENSION condition); supabase/functions/description-image-upload/policy.mjs:17,20,381,476,521; test/description-image-upload.js:169-171; migrations/2026-09-05-description-images.sql:19-30. Citation and the third constant added in a self-audit of scope claims prompted by the #1352 review.]**
