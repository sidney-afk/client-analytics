...
refuses. One line contains an honest gap: `count_unproven` and `inconclusive`
appear on no branch reachable from this clone, so what the clipped text claimed
about them is unestablished and is not guessed.
...
RISKS
  ...
  - The client-continuity lane goes live with a census credential that RLS can filter, producing counts that look authoritative and are not — a watcher that says 'healthy' about a partially-hidden board is worse than no watcher.
    mitigate: The package already treats this as an owner precondition; make it a runbook checkbox with a named verification (compare the credential's count against a service-role count for one scope, once, privately). **`count_unproven` and `inconclusive` COULD NOT BE VERIFIED FROM THIS SESSION and the rest of the original sentence is not continued: neither identifier appears anywhere in the reachable tree** — searched main, the coordinator branch and `claude/lx-f-cutoff-part2` — so whatever the clipped text claimed was "already" true of them is unestablished. The client-continuity package they belong to is behind the CLIENT_CONTINUITY_* repo vars and secrets and is not in this repository at a readable commit. What to do instead of trusting that clause: before switching the lane on, confirm in the package's own source that a filtered or partial read produces an explicitly unproven/inconclusive result rather than a healthy-looking count, and that the observer treats it as failure. Until somebody has read that code, the honest state is that the census's RLS-filtering behaviour is UNVERIFIED, and a watcher that says 'healthy' about a partially-hidden board is worse than no watcher. **[RESTORED 2026-09-08 · gap stated rather than filled: `count_unproven` / `inconclusive` return no matches on any branch reachable from this clone; the surrounding precondition is restored from the risk line it mitigates.]**
