...
CRITICAL COUPLING: linear-deliverables-reconcile.yml lines 120-126 carry BOTH the `reconciler_pager` heartbeat AND the second, independent host of `monitoring-watchdog.js --check`. monitoring-deadman.yml's header states outright that the two hosts watch each other and 'a checker cannot report its own death'. Disabling the re
  ...
  [F5] (medium risk, new) WATCHER 1 — outbox debt census: the queue must be empty and STAY empty, and an unreadable census must scream
     WHAT IT CHECKS: counts by status in `public.mirror_outbox` for real clients (`test_only=false and legacy_parity=false`), plus `max(now()-created_at)` among ('pending','failed','shadow_ok'). **KEEP THAT RAW STATUS CENSUS IN EVERY STATE — THE DEBT VIEW IS AN ADDITION, NEVER A REPLACEMENT, AND "THE FIVE DISPOSITIONS" IS WRONG.** `linear_outbound_cutoff_debt_rows_v1()` has SIX branches and the second one is `when not c.cutoff_enabled then 'cutoff_inactive'`, evaluated before every non-terminal branch (migrations/2026-09-06-linear-outbound-cutoff.sql:189-206 @fix/cloud-outbound-cutoff-20260906). So while F1 is installed but the fence is still INACTIVE — which is the whole period this watcher exists to guard — every non-terminal row returns `cutoff_inactive`, none of the four pre/post-cutoff dispositions can appear, and a watcher that swapped the raw census for the view reports nothing about the queue at exactly the time the queue matters. Two further facts about the view: it returns only `(id, status, outbound_generation, disposition)` and applies NO client filter, so it spans test and legacy-parity rows and must be joined back to `mirror_outbox` on `id` to be scoped to real clients; and it is service-role only. Report all SIX dispositions explicitly, `cutoff_inactive` included, and read the view as an extra signal only after activation. **[CORRECTED 2026-09-08 · this line was NOT truncated and is outside this pass's original scope (a work-item spec line, not a LIVE ACTION or a risk `mitigate:`); corrected because it is an instruction a watcher gets built from · sources: migrations/2026-09-06-linear-outbound-cutoff.sql:189-208 @fix/cloud-outbound-cutoff-20260906; this brief's own adversarial review at lines 396-398, whose `truth:` line is itself one of the ~284 clipped lines · found independently by Codex round eleven (P1)]**
...
LIVE ACTIONS THE OWNER MUST TAKE (prepare them exactly; never execute them)
  - [other] **AND THE STEP THIS LIST IS MISSING, WHICH THIS READ EXISTS TO SIZE: DRAIN THE QUEUE TO ZERO BEFORE ANY FLAG MOVES.** An earlier version of this restoration recorded the drain as an open question because the alarm-fatigue `mitigate:` refers to a "STEP 0" this list does not print. It is not ambiguous, it is missing: dispatch https://github.com/sidney-afk/client-analytics/actions/workflows/linear-outbound-drain.yml repeatedly until this census returns zero non-terminal real-client rows, and verify that zero by re-running the read above — not by the workflow's own summary. Only then set `linear_outbound_enabled` to `off`. Taking the flag first PAUSES the drain rather than finishing it: those rows sit `pending`, the `oldest_pending_age` alert pins ON for any undrained real-team row (threshold 30 min, both teams syncview), and if the cutoff is later activated they are classified as debt by a fence that cannot be lifted. Draining is the one part of this sequence that gets harder every minute you wait and impossible after activation. Service-role read of public.mirror_outbox before anything else: `select status, legacy_parity, test_only, count(*) from public.mirror_outbox group by 1,2,3` plus the oldest created_at among ('pending','failed','shadow_ok') for real clients. OPEN_REPAIRS 75 names this as the single highest-value unresolved read from the 2026-08-30 backend audit — the browser's publishable key cannot read `mirror_outbox` at all, so nobody has ever looked. What it answers: whether any of the constantly-reported `backlog: 14` rows carry `legacy_parity = true`. Post-flip the drain's parity gate can never be true (`linear-outbound/index.ts:1377-1379` requires `authority === "linear"`), so such a row is counted `paused`, unlocked after 30 minutes and retried forever — it never reaches `failed`, `alerts.failed_write` stays false, and nothing pages. Item 75's own SQL is the fuller form and worth running in the same session — **BUT DO NOT PASTE IT AS WRITTEN: `public.mirror_outbox` HAS NO `kind` COLUMN.** As item 75 quotes it (`select id, kind, legacy_parity, test_only, attempts, created_at …`) the query fails with `42703 column "kind" does not exist` and returns nothing, which on the one read that has never been taken is the worst possible failure: it looks like a permissions problem and it is a typo. The table carries `op` from B1 (`migrations/2026-07-06-b1-linear-data-model.sql:111-119`, one of `create|update_state|update_fields|comment|archive`) and `entity` / `entity_id` / `operation` from B4 (`migrations/2026-07-11-b4-linear-outbound.sql:11-32`); `kind` belongs to deliverables, not here. Run it with the columns that exist: `select id, entity, operation, op, status, legacy_parity, test_only, attempts, created_at from public.mirror_outbox where status <> 'written' order by created_at asc;` The defect is in the ledger item rather than in this transcription of it, and OPEN_REPAIRS is append-only, so the correction is recorded in item 179 instead of edited into item 75. The answer decides STEP 0: the queue must drain to zero before the flags move, and a parity row that cannot drain has to be quarantined rather than waited on. **[RESTORED 2026-09-08 · sources: docs/ops/OPEN_REPAIRS.md item 75 (lines 5581-5610); migrations/2026-07-06-b1-linear-data-model.sql:111-119; migrations/2026-07-11-b4-linear-outbound.sql:11-32. The quoted census query was corrected after Codex round eleven (P1) found that the column item 75 names does not exist on this table — a quoted source is not a verified one.]**
  ...
  - [runtime-flag] Set `linear_outbound_enabled` = {"mode":"off"}.
    undo: **BEFORE CUTOFF ACTIVATION:** restore `{"mode":"live"}`; the queue resumes from where it paused because nothing was terminalized, and it is fully reversible. **AFTER ACTIVATION THIS IS NOT AN UNDO AND THE FLAG READBACK WILL LIE TO YOU.** `linear_outbound_claim_v1` opens with `if v_control.cutoff_enabled then return null; end if;` (migration :94), so the cutoff-aware worker claims nothing, on every row, forever; and the pre-cutoff worker is refused by `linear_outbound_stale_worker_guard_v1` with `linear_cutoff_stale_worker_refused`. Setting the flag live post-activation therefore reads back as success and leaves outbound entirely unavailable — the worst combination, because the operator has a green readback and a dead lane. Post-activation the path is the reviewed recovery delta named in the activation step's own undo, which may advance from an exact generation only after every `authorized_before_cutoff` row is reconciled against provider truth and every `accepted_after_cutoff` receipt is classified. Not this flag. **[RESTORED 2026-09-08 · sources: migrations/2026-09-06-linear-outbound-cutoff.sql lines 80-107 (`claim_v1`, the `cutoff_enabled` early return at :94) and 139-162 (the stale-worker guard) @fix/cloud-outbound-cutoff-20260906; docs/ops/LINEAR_OUTBOUND_CUTOFF.md §"Installation, rehearsal, and recovery dependencies". This line was NOT truncated and so was outside this pass's original scope; corrected after Codex round five (P1) because "fully reversible" is false on the far side of the activation step two lines below it.]**
  - [runtime-flag] Set `linear_inbound_enabled` = {"enabled":false}.
    undo: **BEFORE THE TWO LINEAR WEBHOOKS ARE DELETED (the step below):** restore `{"enabled":true}`. Deliveries during the off window are acknowledged but not applied and are NOT replayed, so the reversal restores the lane, not the missed events. **AFTER THE WEBHOOKS ARE DELETED THIS FLAG RESTORES NOTHING** — there is no delivery to accept, so the flag flips green over a lane with no traffic, and re-creating the webhooks is the actual undo (see below). After 2026-09-15 not even that: the account is gone and no registration can be made. Order the two steps so this flag is the reversible probe and the webhook deletion is the commitment, not the reverse. **[RESTORED 2026-09-08 · this line was NOT truncated and was outside this pass's original scope; corrected in the round-five boundary sweep because it promises a reversal that the deletion step immediately below it removes.]**
  - [other] Delete the two EF-bound Linear webhooks in Linear's own settings (the hard stop for inbound delivery).
    undo: Re-create them — **only if the URL and LINEAR_INBOUND_SIGNING_SECRET are captured first, and only while the Linear account still exists.** Capture both before deleting or this is one-way immediately; after 2026-09-15 it is one-way regardless of what was captured, because there is no account left to register a webhook against. Treat the capture as necessary and not sufficient. Note what this step really costs, so it is not read as a mere flag flip: it is the hard stop for inbound delivery, and the flag above becomes inert the moment it is taken. If the intent is a reversible pause, use the flag and leave the registrations alone. **[RESTORED 2026-09-08 · this line was NOT truncated and was outside this pass's original scope; the post-cancellation clause added in the round-five boundary sweep.]**
  - [migration] Install migrations/2026-09-06-linear-outbound-cutoff.sql via scripts/f27-apply-migration.js, ONLY as one release window with the matching linear-outbound source.
    undo: **THIS UNDO IS VALID ONLY BEFORE THE CUTOFF IS ACTIVATED. AFTER ACTIVATION IT DESTROYS THE EVIDENCE THE RECOVERY NEEDS — DO NOT TAKE IT.** The activation step below is one-way, and its recovery requires reconciling every `authorized_before_cutoff` row against provider truth and classifying every `accepted_after_cutoff` receipt. Those dispositions are DERIVED, not stored: they come out of `linear_outbound_cutoff_debt_rows_v1()` / `linear_outbound_cutoff_debt_v1` reading `linear_outbound_cutoff_control` and `mirror_outbox.outbound_generation`. Drop that table, view and RPC set after activation and you have removed both the fence state and the only way to classify the debt it fenced — the reviewed recovery delta then has nothing to work from. Post-activation, the path is the reviewed delta, not this list. **STEP 0, BEFORE ANY DROP (pre-activation only): take the callers off these RPCs.** Set `linear_outbound_enabled` to `{"mode":"off"}` (fully reversible, nothing terminalized) and/or restore the pre-cutoff `linear-outbound` closure. The cutoff-aware worker calls `linear_outbound_claim_v1` and `linear_outbound_authorize_dispatch_v1` on every drain — the cutoff doc's own "deploying only the Edge Function fails closed at claim" — so dropping those RPCs underneath a deployed cutoff-aware closure stops the outbound drain, which is exactly what this lane's flags exist to do gracefully. Structure comes off after the callers, never before. Then, additive and in dependency order: drop view `linear_outbound_cutoff_debt_v1`, then `linear_outbound_cutoff_debt_rows_v1()` (the view selects from it); drop triggers `zzzz_linear_outbound_stamp_generation_v1`, `linear_outbound_control_lock_v1` and `linear_outbound_stale_worker_guard_v1` on `public.mirror_outbox`, then their three same-named functions; drop the three remaining RPCs `linear_outbound_claim_v1(bigint,text,integer)`, `linear_outbound_authorize_dispatch_v1(bigint,uuid,bigint)` and `linear_outbound_cutoff_activate_v1(bigint,text)`; drop `mirror_outbox_cutoff_disposition_check` only if the four columns are dropped rather than left dormant; drop table `linear_outbound_cutoff_control` LAST, because every RPC and trigger function above reads it. **COUNT CORRECTION: the file creates SEVEN functions, not the four the original said** — four service-only RPCs plus three trigger functions that share their triggers' names and are left behind by a naive four-object drop. The four `mirror_outbox` columns can stay dormant. The F27 recovery contract must own all of them BEFORE install (F1), or the documented one-step database rollback no longer covers the schema — the cutoff doc: "the new control owner is outside selected37 recovery and must be included in an explicit compatible recovery extension before installation". If that extension cannot be reviewed in time, do not install: the runtime flag alone already stops the mirror and is fully reversible. **[RESTORED 2026-09-08 · sources: migrations/2026-09-06-linear-outbound-cutoff.sql lines 13-41, 43-75, 80-160, 165-223 @fix/cloud-outbound-cutoff-20260906; docs/ops/LINEAR_OUTBOUND_CUTOFF.md §"Installation, rehearsal, and recovery dependencies". Stop-first precondition added in the #1352 sequencing sweep; the pre-activation restriction added after Codex round four (P1) showed this undo and the activation undo below contradicted each other across the activation boundary.]**
  - [edge-function-deploy] Deploy the cutoff-aware linear-outbound through https://github.com/sidney-afk/client-analytics/actions/workflows/deploy-f27-section4-closures.yml. Requires the sealed four-function rollback bundle: run `& "$env:USERPROFILE\.syncview\f27-capture.ps1"` from ANY directory, drag the printed .sourcebundle into the `SyncView Backups/` Shared Drive root, THEN dispatch with `commit_sha` = main's tip at that moment, `operation` = `deploy-reviewed-release`, `confirm` = `DEPLOY_REVIEWED_F27_SECTION4_CLOSURES`, and `rollback_bundle_sha256` / `rollback_bundle_byte_length` copied from the capture's own JSON (`sealed_bundle_sha256`, `sealed_bundle_byte_length`) with no re-derivation. The script prints the bundle's full path on its LAST line — read that rather than assuming a folder. Order is load-bearing and enforced: the lane FETCHES the bundle out of Drive by content-addressed name during the run, so dispatching before the upload fails in about 20 seconds with `{"status":"FAIL","code":"OBJECT_MISSING"}`, deploys nothing, and is recovered by uploading and pressing `Re-run jobs` on the SAME run (which keeps all five inputs) — never by re-capturing. Capture minutes before dispatching, not hours: a bundle that sealed an older live set restores the wrong code. And nothing may be merged to main between handing over the SHA and the run going green, because the lane requires `commit_sha` to equal main's tip at dispatch time. This deploy must land in the SAME release window as the cutoff migration — see the risk below. **[RESTORED 2026-09-08 · sources: CLAUDE.md (capture order, OBJECT_MISSING recovery, merge freeze); AGENTS.md:100-127; .github/workflows/deploy-f27-section4-closures.yml lines 9-33 and 318-345]**
    undo: The sealed bundle IS the rollback: restore the captured prior four-function source. Both lanes refuse on a fingerprint mismatch, so a wrong digest cannot deploy the wrong code — it can only decline. **BUT SCOPE THIS TO BEFORE ACTIVATION: after the cutoff is activated, do not go back to the pre-cutoff worker, and keep a cutoff-aware closure for any recovery.** The prior worker claims `mirror_outbox` rows by direct table update and never consults the control RPCs, so post-activation it is the wrong tool for every purpose — including recovery, which has to be able to authorize deliberately rather than by accident. **One correction to the mechanism, because the danger is real but not the one it looks like.** While the cutoff SQL is INSTALLED, restoring the old worker does NOT bypass the fence: `linear_outbound_stale_worker_guard_v1` fires `before update on public.mirror_outbox` and, once `cutoff_enabled` is true, raises `linear_cutoff_stale_worker_refused` on any update touching `lock_token`, `locked_at`, `dispatch_authorization`, `dispatch_authorized_at` or `status` — the guard's own comment says it exists to "cover an old worker acquiring its first lease after cutoff". So that combination fails closed; it just fails closed noisily and confusingly, and will read as an outage. The genuine bypass is the COMBINATION with the install undo above: drop the control table and guard, restore the old worker, set `linear_outbound_enabled` back to `{"mode":"live"}`, and the fence is gone entirely with nothing left to say what was fenced. That sequence is now forbidden at both ends. **[RESTORED 2026-09-08 · sources: migrations/2026-09-06-linear-outbound-cutoff.sql lines 139-162 (the stale-worker guard) and 165-189 @fix/cloud-outbound-cutoff-20260906; docs/ops/LINEAR_OUTBOUND_CUTOFF.md §"Correction evidence and monitoring obligation". Scoped to pre-activation after Codex round four (P1); its stated mechanism is corrected here rather than repeated.]**
  ...
  - [n8n-edit] Disable the n8n pager nodes that dispatch the two card reconcilers (every 15 min) and gate B1 refresh (every 30 min) on workflow qllIDZPkdNAPRj0b; later deactivate the 'SyncView Workload — Reconcile' workflow (schedule: every 10 min).
    undo: **BEFORE CREDENTIAL REVOCATION:** re-enable the nodes / re-activate the workflow. Capture the exact active version id first. CLAUDE.md forbids editing an n8n workflow without the owner's explicit go-ahead in the same request; the brief says n8n edits are authorized where safe, so re-enabling is an n8n edit under exactly the same rule and needs its own go-ahead in the request that asks for it — a prior authorization to DISABLE is not an authorization to re-enable later. **AFTER REVOCATION IT IS NOT AN UNDO:** these dispatch the card reconcilers and the B1 refresh, all of which reach Linear, so re-enabling them restores jobs that fail on every run and heartbeats that go stale — the same latched alarm-fatigue outcome as the scheduled-workflow step below. Post-2026-09-15 they are retired, not paused. Two mechanical preconditions for the pre-revocation undo to actually work: capture each workflow's `activeVersionId` in the same session as the edit and write it into an `n8n-backups/` status stub (never take a version id out of git — the brief's own risk list records those as already stale), and unpublish rather than delete, which is the documented reversible step. Reverse in the opposite order to the disable, and re-enable the pager nodes before re-activating `SyncView Workload — Reconcile` so the reconcilers are not dispatched into a lane nothing is watching. **[RESTORED 2026-09-08 · sources: CLAUDE.md (n8n standing constraint); docs/truth/N8N.md; ROLLBACK.md rule 2. Post-revocation boundary added in the round-five sweep.]**
  - [runtime-flag] Disable the scheduled GitHub workflows: sample-linear-reconcile.yml, linear-deliverables-reconcile.yml, b1-linear-incremental-refresh.yml, production-shadow-audit.yml.
    undo: **BEFORE CREDENTIAL REVOCATION:** re-enable in the Actions UI or revert the PR. **AFTER REVOCATION IT IS NOT AN UNDO.** These four lanes hard-require the Linear keys, so re-enabling them once `LINEAR_MIRROR_API_KEY` / `LINEAR_API_KEY` / `LINEAR_READ_API_KEY` are gone does not restore anything — it restores four jobs that fail on every run, and their dead-man heartbeats go stale, which latches exactly the alarm-fatigue incident the first risk in this section is about. After 2026-09-15 the account is gone and no re-issue is possible, so there is no recovery at all: the honest post-revocation position is that these lanes are retired, and the `monitoring-watchdog` LANES entries must be retired with them rather than re-enabled. Unchanged and still binding for the forward direction: this MUST land together with the monitoring-watchdog LANES edit (F6) or the dead-man's switch pages forever on lanes that were deliberately retired. That coupling is enforced by the test suite, not by this note — `scripts/monitoring-watchdog.js` states it: "you cannot disable a Linear workflow without retiring its lane, and you cannot retire a lane while its workflow still runs". **[RESTORED 2026-09-08 · sources: scripts/monitoring-watchdog.js lines 85-130 @claude/lx-f-cutoff-part2; this brief's own credential-revocation step and its alarm-fatigue risk. This line was NOT truncated and so was outside this pass's original scope; corrected after Codex round five (P2) because it sits on the reversible side of a one-way step three lines below it.]**
  ...
  - [other] Revoke LINEAR_MIRROR_API_KEY, LINEAR_API_KEY, LINEAR_READ_API_KEY, LINEAR_INBOUND_SIGNING_SECRET. **HARD PRECONDITION, STATED HERE AND NOT ONLY IN THE UNDO BELOW, BECAUSE IT DECIDES WHETHER THIS STEP IS SAFE: production-write's 503-on-missing-key call sites must be removed and deployed FIRST.** `linearLabelsRequest` reads `LINEAR_MIRROR_API_KEY` and throws 503 `label_catalog_unavailable` when it is empty (production-write index.ts ~:837-839 on the candidate), and the paired site around :2291 does the same for the create path — so revoking before that closure ships turns "pick a label" and "create a deliverable" into hard 503s for staff, on a day when nobody will be able to give the keys back. **That removal-and-deploy is NOT a step in this list**, which is the gap worth noticing: it belongs to the production-write lane and this list's last action silently depends on it. Do not take this step until that closure is deployed and verified, and treat "the keys are revoked" as the LAST irreversible act of the whole programme — after 2026-09-15 the account is gone and no re-issue exists. **[RESTORED 2026-09-08 · sources: supabase/functions/production-write/index.ts ~:837-839 and ~:2291 @origin/integration/linear-exit-candidate-20260906; this line's own undo, which already carried the precondition. Promoted onto the action in the LIVE-ACTIONS ordering sweep Codex round nine prompted — a precondition that decides an action's safety belongs on the action, not in its rollback.]**
