...
Complete relevant asset access/preservation without inventing a file-replacement requirement. Preserve expected existing notifications; roster mappings are reference inputs to verify against existing behavior, not authorization for new messages. Do not remove normal urgent editor/review alerts. No live sends are authorized during preparation.
...
- Native activation: moving work ownership from Linear to SyncView, enabling background workers, scheduled writes, gateway wakeups or new automation requires its own scope. Preserve existing expected internal notifications; additional communications are not implicitly approved.
...
- Linear retirement/shutdown is OUT OF SCOPE, not a pending rollout approval. Leave Linear, its billing, credentials, data and external integrations untouched; preserve native receipts.
...
## Owner-facing review before proposing installation

Present a short before/after description of the website and necessary backend changes. Verify that approvals, comments and urgent actions save correctly, retain existing expected notification behavior, avoid unexpected/duplicate messages and use website links rather than Linear links where applicable. Review communication paths in the selected release without treating normal Slack workflows as forbidden. No live/test sends are authorized during preparation.

Show how the website stops depending on Linear while leaving the existing Linear account and external automation untouched. Identify any conflict caused by existing workers continuing to write website data; do not solve it by silently disabling those workers or modifying Linear. Linear-side shutdown is outside scope. Website-side disconnection and the backend changes needed to retain normal functionality remain part of the intended decoupling; review their exact release scope.

Files: the current 14 inaccessible Drive references comprise nine deliverable file links and five calendar thumbnail links. Present exact private affected items and proposed action per type. Metadata absence does not prove deletion; neither replacements, link changes nor deletion are approved. Database backup files are a separate already-approved recovery-copy scope.
