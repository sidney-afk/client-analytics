...
Present a short before/after description of the website, the exact backend components needed for it, and every automatic side effect. Explicitly show how status changes, comments and urgent actions avoid Slack/client communications; review any email/invitation/reminder/webhook path in the selected release rather than assuming the Slack check covers it. Native notification source currently includes status, staff/client comment and urgent-editor intents: this is functionality in the prepared package, not proof of active live sends. Prove no queued-message replay or automatic sender activation. No new internal communications are approved either.

Show how the website stops depending on Linear while leaving the existing Linear account and external automation untouched. Identify any conflict caused by existing workers continuing to write website data; do not solve it by silently disabling those workers or modifying Linear. The broader previous retirement plan cannot be used as the installation recipe for this narrower request.
