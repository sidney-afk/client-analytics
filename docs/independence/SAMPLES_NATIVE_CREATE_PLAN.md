...
> Status: PLANNED, deliberately not built unattended (2026-08-19, overnight session).
...
1. **Gateway (`production-write`)** — the sxr lane today allows only
...
2. **Schema (owner SQL #1)** — `batches.purpose text` (default `'calendar'`,
   ...
   unexecuted).
3. **RPC (owner SQL #2)** — the append RPC pins `origin = 'calendar'`
...
4. **Browser materialization** — the samples twin of
...
5. **UI** — the option-E dialog mounted from the Samples tab create buttons,
