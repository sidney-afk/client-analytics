...
- **Part 2 gateway backend + #812 Production caller (live, authority-locked):** `production-write` is the single
  ...
  removed the legacy `supabase` alias that several backend consumers used to accept; every
  consumer now accepts only canonical `linear`/`syncview`. D-28 requires cohort soak before a Graphics-first flip; a D-29 data incident is contained
