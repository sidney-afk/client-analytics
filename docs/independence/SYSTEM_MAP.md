...
- **Edge Functions.** 36 are represented under `supabase/functions/`; **the app calls 29**
  (**"29 literal + 4 composed" Edge Functions**, see §7; `description-image-upload` is the
  ...
  (`hiring-automation`). The admin-only `hiring-applications` sidecar and the private
  `hiring-automation` bridge are deployed. Application capture and reviewer alerts are live;
  `hiring_invites_enabled=false` and the inactive dispatcher keep candidate email default-off.
  `production-write` is app-called
...
- **Google Sheets** (`gviz` CSV, one workbook). Serves **all analytics numbers** (six tabs), the
  client roster (`Clients Info`), and the SMM directory. Current Pages uses the staff-gated
...
- **Config note.** The onboarding/list Edge Functions are composed onto a hardcoded edge-base
  constant declared *before* the main Supabase URL constant (TDZ avoidance). They account for the
...
- **Reads.** Five `gviz` CSV tabs from one workbook: `Metrics` + `Clients Info` (essentials) and
  ...
  workbook as a frozen archive. `client-token-verify` EF only on client links.
  Phase 2 (2026-09-25): a verified client link reads its own rows from the `analytics-read` EF
  instead, when `analytics_mirror_read_enabled` is on for everyone or lists its slug
  (`{"enabled": false, "clients": [slug]}`); it falls back to the five tabs on a failed read, an
  8 s timeout, or no complete copy. The staff overview always reads the Sheets. The flag is in the
  head boot flag batch. Chart.js CDN asset.
...
**Rename propagation (2026-09-23, OPEN_REPAIRS 242; not applied, all directions dormant).** A card rename is recorded by a database trigger and applied to the linked sub-issues by `rename_propagation_drain` (migration `2026-09-23-rename-propagation.sql`). The browser helpers in `125-title-name-rule.js.part` read the `rename_propagation_status_v1` view for the "name syncing" marker and call `rpc/rename_propagation_poke` / `rpc/rename_propagation_retry`. Every failure is swallowed, the save never waits on them, and until the migration is applied the view answers 404 and the marker shows nothing.
...
- **Retirement warning (F104).** The former Phase-4 checklist is quarantined: it falsely treated the
  ...
  `LINEAR_STATUSES_URL` itself was retired on 2026-09-22 under exactly that bar and not by that
  checklist — measurement in OPEN_REPAIRS 233, the three-part consumer proof in 235. The quarantine
  stands for everything else the checklist named.
...
- **Reads.** Submit no longer reads the n8n Linear projects webhook (retired 2026-09-24: it read
  Linear with the revoked key, failed 401 and raised an n8n alert on every open), so the legacy
  dropdown list holds only what the page already had. Only clients in `write_ui_reroute_clients` overlay that list from the
...
- **Writes.** Submit uses the authenticated native path in the #850 overlay below. Routing that
  cannot prove the native lane saves a visible hold; a pre-existing legacy receipt keeps its exact
  identity in that hold rather than being replayed through another transport. B1-2 removed the
  orphan browser sender for n8n `video-form` and `graphic-form`; `log-linear-submission` remains
  best-effort telemetry on the native path, not its durable acceptance receipt. Calendar cards are
  materialized from the native create response through the calendar's shared upsert fetch.
- **State.** `syncview_linear_form` (autosaved draft, cleared on submit), `syncview_last_link`,
  ...
  `syncview_workload_v2_off`. Kill switches: `calendar_upsert_ef_clients`,
  `write_ui_reroute_clients`, `?wl2=0`.
...
- **Failure/fallback.** With no legacy projects read, the non-enrolled dropdown stays empty rather
  ...
  produced no parent, and had no replayable input. Background card writer: durable job saved *before* the async
...
- **Client-comment front door (2026-08-14).** `client_comment_gateway_enabled` gates the SECOND
  ...
  enrollment, and they now take the gateway only when this flag is the exact operator value
  `{"enabled": true}` AND the tab can build a verified gateway context
  (`_prodClientCommentGatewayContext`: verified client-entry capability, matching mounted slug,
  card-named deliverable id, and a crosswalk that proves the widened server contract —
  `clientCommentFrontDoorTargetAllowed` — will accept the row). Missing/malformed/unreadable is
  OFF and fails dark to legacy; primed by the same fetch and realtime subscription as
...
- **Reads.** Core rows use Supabase REST via the dynamic `'/rest/v1/'+table` pager (`_prodRestRows`): boot
  ...
  An image pasted or dropped into a description (parent or sub-issue) posts
  its bytes to the staff-only `description-image-upload` Edge Function (2026-09-05,
  admin/SMM, one exact roster actor, declared type checked against the magic
  bytes, 4 MiB ceiling, hourly per-actor limit off the `description_images`
  ledger) and receives a public `syncview-description-images` URL, inserted as
  a markdown image the outbound mirror carries to Linear verbatim. The browser
...
**Corrected 2026-09-24:** Phase 2 removed the legacy Samples page's browser code (renderer, state,
`samples-*` webhooks, `content_samples` read, its styles). Only the route resolver remains, so old
`#samples[/...]` links still land on Sample reviews. Statements below about the legacy module being
present are history; its n8n workflows, Sheet tabs and `content_samples` rows still exist.
...
- **Entry.** `?Kasper=1` sets sessionStorage `syncview_kasper_unlocked` and reveals the nav button
  ...
  enabled), replies/messages, editors, filming, sales-intake, hiring-process, onboarding, quiz-leads,
  client-credentials, ad-performance, and time-off (only when `pto_v1` is on) — hash
  `#kasper/<subtab>`, persisted in `syncview_kasper_subtab_v1`. Review, Samples, Messages, and
  Filming Plans stay in the priority row. More groups Editors/Time Off under **Team**, Sales
  Intake/Hiring Process/Onboarding/Quiz Leads/Client Credentials under **Pipeline & Admin**, and
  Ad Performance under **Analytics**; a selected overflow destination replaces the generic More
...
- **Reads.** Review queue is a **3-tier fallback**: `calendar_posts` REST (paginated, v2 default) →
  ...
  SMM-directory CSV. Admin-only `kasper-ad-performance-read` EF (ad-performance): daily
  `kasper_ad_performance_daily` rows plus a server-computed summary (CPC, landing-page-view rate,
  conversion rate, cost-per-booking with/without cancelled bookings) for Kasper's own Meta prospecting
  campaign; the table has no browser PostgREST access, only this EF reads it, and it is
  deliberate-manual (no CI deploy path, matching `workload-plan`'s first release) and not yet live.
  Admin-only `quiz-leads-list` EF (quiz-leads): every synchrosocial.com Growth Bottleneck Quiz
  ...
  sidecar and the server-to-server `hiring-automation` bridge are deployed. The active capture
  workflow accepts only complete/fresh application snapshots, versions every accepted refresh, and
  sends the reviewer alert. The dedicated interview callback binds by stable iClosed contact ID
  rather than email and is isolated from sales by a strict early branch. The invitation flag remains
  false and the dispatcher inactive, so candidate email is not continuously enabled. Realtime
...
- **Writes.** Approvals/tweaks/comments/finish-close stamps via the shared calendar & sample upsert
  ...
  future approved leave while preserving the original decision record. Hiring Process can queue one
  durable interview-invite job only after the separate default-off flag is explicitly enabled; the
  current flag is false. The deployed but inactive dispatcher obtains a one-shot, claim-scoped
  ...
  may retry a confirmed pre-send failure. There is no continuously enabled outbound email path.
- **State.** sessionStorage `syncview_kasper_unlocked`; localStorage `syncview_kasper_subtab_v1`,
  ...
  reads/realtime only and is unsafe for writes (F125). Hiring Process data remains memory-only and
  is purged when the verified staff identity is revoked; it adds no localStorage state or badge poll.
  Its source snapshot and invite actions carry a server-side state version, so stale staff views
  cannot overwrite a later source refresh, delivery result, or booking.
- **Roles.** Hidden staff role, **no password for the queue itself** — only the URL param / session
  ...
  Sensitive subtabs add a **real** role gate: admin can open onboarding, credentials, ad-performance,
  and hiring-process; SMM can open credentials; creative/editor/designer can open neither. Ad Performance
  is admin-only because it exposes real spend; Hiring Process is also admin-only, including its sole
  explicit retry path for a confirmed pre-send failure. Both return 403 for a valid-but-disallowed SMM/creative
  key, 401 for no/wrong key. Time Off administrative controls require
...
- **Reads.** `workload_issues` REST (`active=eq.true`, paged 1000, **default ON** since 2026-06-17)
  ...
  upstream scheduled mirror reconcile, so a newly created issue can still wait for that producer. The
  Tweak-Needed popover reads `production-comments` only; its n8n `linear-tweak-comments` read was
  retired 2026-09-23 (B2), and a row with no native binding shows a "not shown here" notice. The `editors-week` webhook (**one** browser
...
**Corrected 2026-09-24:** the TikTok Pilot was abandoned after TikTok rejected the app. Its browser
code, `#tiktok-pilot` route, nav button, `?ttpilot=1` unlock and `TTP_*` constants were removed; the
Pilot statements below are kept as history. Its backend (the `ttp-*` n8n workflows, the status cron,
the token refresh and the `tiktok_accounts` / `tiktok_oauth_state` / `tiktok_pilot_posts` tables) still
exists; what to do with it is the owner's call. TikTok Upload is unchanged.
...
### 4.15b Clients (admin; new row 2026-09-25, editing added the same day)

- **Entry.** Kasper › More › Pipeline & Admin › Clients (`#kasper/clients`), admin role only
  (`clients-admin` capability); no header button.
- **Reads.** `analytics-read` EF, `action=list_client_profiles`: every `client_profiles` row (active
  and archived) plus the `client_profiles_authority` flag. The EF checks the ADMIN role key; smm,
  creative and client link tokens are refused. `client_profiles` grants the browser nothing.
- **Writes.** `client-profile-write` EF, `action=update_client_profile` (admin role key AND an active
  admin `team_members` id). While `client_profiles_authority` is "sheet" it writes ONLY the changed
  cells to the Clients Info Sheet first, refusing if that Sheet row no longer matches what the editor
...
- **Part 2 gateway backend + #812 Production caller (live, authority-locked):** `production-write` is the single
  ...
  lane remains unsafe for real enrollment until every open gate closes. F55 (closed 2026-07-28,
  source and live) removed the legacy `supabase` alias that several backend consumers used to
  accept; every consumer now accepts only canonical `linear`/`syncview`, including the deployed
...
- **Runtime kill-switch flags (9):** `analytics_mirror_read_enabled` · `kasper_urgent_ping_enabled` / `calendar_upsert_ef_clients` · `client_comment_gateway_enabled` · `prod_authority` · `pto_v1` · `sample_review_ef_clients` · `settings_ef_clients` · `write_ui_reroute_clients`
