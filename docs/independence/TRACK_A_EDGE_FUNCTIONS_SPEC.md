...
| `samples-upsert`, `samples-reorder` (Samples Old) | — | Phase-1 staff-route retirement shipped, but F117 proves the old tokened client redirect loses exact-client scope. Fail it closed or restore a bound portal before Phase 2; retained backends are not evidence of compatible routing. Do not delete or silently reactivate them. |
