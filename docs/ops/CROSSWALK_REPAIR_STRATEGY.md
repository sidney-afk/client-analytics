...
linear_identity_unproven 1) with a person's ruling for it — "repair by hand"
or "leave it, and here is why"; either closes the slot. The public ledger keeps
only the counts of those rulings (OPEN_REPAIRS 156, "The seven, ruled"), because
a per-row description would select production rows through the anonymously
readable tables; the per-row record is the owner's Claude session of
2026-09-05, linked as `Claude-Session` in the trailers of every commit on
PR #1312 and private to the owner. (b) therefore holds **for the slot set of
2026-09-05 only**: a slot the runner reports later that was not in that set —
the plan summary's reason counts do not prove this, since a replacement can
keep the counts, so compare the slot list itself against the private record —
is unruled until a person rules on it and the count here is updated — and (c) the browser-side readback through
`_prodCrosswalkMismatchFields`, run over the same population, names the same
slots and no others. (a) holds as of 2026-09-05 21:00Z; (b) holds for the slot set of the same evening (OPEN_REPAIRS 156,
"The seven, ruled": six "leave", one one-row move by the owner; per-row record
in the private session linked from PR #1312's commit trailers); (c) does not
