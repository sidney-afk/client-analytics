...
| `linear-outbound` | NONE | **NO CI DEPLOY PATH - DELIBERATE-MANUAL.** RETIRING (B2 Slice 8, 2026-09-24): removed from the Section 4 release set and the onboarding lane, so nothing deploys it. It is deleted after the three-function Section 4 deploy is green. The source stays only as the frozen reference its contract tests read, until B2 Slice 10. Do not redeploy it except as the Section 4 captured-prior restore. | `_shared/linear-create-id.mjs` | `linear-outbound/f27-replay.mjs`<br>`linear-outbound/mapping.mjs`<br>`linear-outbound/monitoring.mjs` |
