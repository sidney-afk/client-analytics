...
| `notify` | [deploy-onboarding](../../.github/workflows/deploy-onboarding-edge-functions.yml) | workflow_dispatch only (pinned SHA guard) | `_shared/staff-role-auth.ts` | `notify/slack-api.ts`<br>`notify/urgent-link.ts` |
...
| `write-diagnostics` | NONE | **NO CI DEPLOY PATH.** | `_shared/staff-role-auth.ts`<br>`_shared/write-refusal-codes.mjs`<br>`_shared/write-refusal-diagnostics.mjs` | - |
