...
| `production-comments` | [deploy-onboarding](../../.github/workflows/deploy-onboarding-edge-functions.yml) | workflow_dispatch only (pinned SHA guard) | `_shared/native-brief-media.mjs`<br>`_shared/native-comment-media.mjs`<br>`_shared/staff-role-auth.ts` | `production-comments/feedback.mjs`<br>`production-comments/policy.mjs` |
