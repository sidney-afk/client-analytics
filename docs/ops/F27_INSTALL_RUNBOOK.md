...
gh workflow run deploy-f27-section4-closures.yml --ref main \
  -f commit_sha=<RELEASE_SHA> \
  -f operation=restore-captured-prior-three \
...
- [ ] Before DDL, use the separate Node-only Section 1 operations to capture/seal/private-round-trip the prior exact source/JWT closure for `production-write`, `deliverable-write`, and `batch-write` (three functions since B2 Slice 8; `linear-outbound` is no longer in the release or prior set), and separately the reconciler's exact raw-Git workflow/runtime closure. Before the first provider read require the clean release's exact project target and CLI 2.109.0; after sealing require all-three private provider project/CLI/readback-adapter/restore-adapter compatibility and public `provider_contract=PASS`. Record all three `PRIOR_*_VERSION` values, both bundle SHA-256/byte-length pairs, `PRIOR_RECONCILER_SHA`, and `PRIOR_RECONCILER_CLOSURE_SHA256`. The Section 4 lane consumes only the three-function bundle; a four-function bundle sealed before B2 Slice 8 is not restorable by the lane.
...
- [ ] Dispatch only `deploy-f27-section4-closures.yml` from current `main` with the exact `RELEASE_SHA`, `deploy-reviewed-release`, `DEPLOY_REVIEWED_F27_SECTION4_CLOSURES`, and the sealed prior-three bundle hash/length (a fresh three-function capture of `production-write`, `deliverable-write`, `batch-write`). Require CLI 2.109.0, Docker, all-three private restore-target/CLI/adapter compatibility, exact import/candidate gates, all three captured forward JWT arguments equal to `--no-verify-jwt`, three literal serial deploys in runbook order (`production-write`, `deliverable-write`, `batch-write`), per-function source/entrypoint/JWT/version/provider readback before the next deploy, and the final version-stable three-function capture/fingerprint bound to every immediate receipt. `linear-outbound` is not deployed or restored by this lane. Never use the onboarding or inbound lane; do not deploy inbound or either frozen writer. A failed/ambiguous response is never retried forward: use the same lane's separately confirmed `restore-captured-prior-three` operation.
