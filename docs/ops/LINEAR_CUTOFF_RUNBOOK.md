...
|---|---|---|---|
| P1 | **Lane B's native naming mint is live** | `linear-outbound:857` and `:868` mint `deliverables.linear_identifier` for SyncView-native cards. Flip outbound off before the mint lands and every card created afterwards renders as `b1_d_188ba4ad…` in the Production list, the command palette, the Workload parent header and all three deep links merged this week. Nothing errors; the estate just stops producing names. (OPEN_REPAIRS 162, 163) | coordinator |
