...
| **Brief media** | `native_brief_media` (mode `off`) | **code installed but not wired.** Dormant since `migrations/2026-09-07-native-brief-media.sql`; `production-write`'s `description_read` computes a native `media` projection but `index.html` never reads it, so a brief's raw `uploads.linear.app` image URLs render straight from Linear's CDN. No live media has been copied/verified. Browser reads the native projection since PR #1442; remaining work is the one-time copy of Linear-hosted brief images into native storage plus flipping `native_brief_media`, to be done before 2026-10-15. | Checkpoint row *"Historical Linear import/link UI and retained URL fields"*. |
...
**29a is pending today**, after the last code PR of this slice (#1445). The owner's live functional click-through of Submit, Workload, Calendar and Samples has not yet been recorded against this closure wave.
