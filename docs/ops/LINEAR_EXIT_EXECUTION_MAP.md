...
| **Card materialization** | `native_card_materialization` | **dropped, 2026-09-20 — nothing to materialize.** `migrations/2026-09-06-native-card-materialization-boundary.sql` is still the only file in the repository that references the flag, and nothing in `index.html`, any Edge Function or any script reads it — but the reason to wire it is gone: the same 2026-09-20 native-snapshot measurement behind the Workload row above found **0 legacy-source video or graphics rows**, so there is no remaining legacy row for this boundary to materialize against. | **unknown, verify** — no checkpoint row or OPEN_REPAIRS entry identified. |
| **Step 6 — outbound cutoff** (`LINEAR_CUTOFF_RUNBOOK.md` STEP 6, "Stop the schedulers") | `linear_outbound_enabled`, `linear_legacy_parity_enabled` | **off, 2026-09-20.** `linear_outbound_enabled` moved `{"mode":"live"}` → `{"mode":"off"}`, committed **2026-09-20T17:26:22.363379Z**; `linear_legacy_parity_enabled` moved `{"enabled":true}` → `{"enabled":false}`, committed **2026-09-20T17:35:54.465602Z**. Both writes recorded, verified before commit and read back after, in `EXECUTION_LOG.md`, `ROLLBACK.md` and `docs/truth/BRIEFING.md`. The **nine-minute interval** between the two flips is accepted by the owner, since Linear is retired — the runbook's own ordering was not waited on. The **latched pending-age alarm** (`linear-outbound-drain.yml` run 35526389014: `alerts.oldest_pending_age: true`, both teams 3005 minutes oldest-pending against a 30-minute threshold, backlog 45) is a known, accepted state, not a fault: with outbound off the backlog cannot drain and its age only grows. It **awaits an owner decision on marking the 45 frozen outbox rows terminal** so the alarm can clear; until then it stays latched by design. | Closes the outbound half of the provider-branch dependency across every remaining capability; gates the intake plan's check 11 (final cutoff) and this map's identifier-mint row's deferred dependency. |
