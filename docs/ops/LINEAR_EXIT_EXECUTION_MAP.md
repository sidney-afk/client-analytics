...
| **Intake form path** | `native_intake_epochs` | **native for new submissions from the updated browser.** PR **#1436** merged 2026-09-20 (browser holds): its offline tests cover plan check 2 (all four browser exits hold visibly with zero legacy requests) and plan check 6 (native pending submissions keep their identity). Post-merge live observation 2026-09-20, not one of the plan's numbered checks: one normal submission on the test client produced VID-15027 and GRA-8023 with no Linear issue, exactly one `skipped` `mirror_outbox` row each (the native marker) and zero queued or sent rows. Plan checks 1, 3, 4, 5 and 7 remain open, and check 4 (endpoint enforcement) needs an owner decision. Plan and checks: `docs/ops/LINEAR_EXIT_STEP26_NATIVE_INTAKE.md`. The most wired of the remaining set: read from `index.html` (2), two Edge Functions and nine scripts. | Checkpoint row *"Intake: VIDEO_FORM_WEBHOOK and legacy dispatch selection"*, plus the **intake** half of the provider-branch row. |
...
| **Brief media** | `native_brief_media` (mode `off`) | **code installed but not wired.** Dormant since `migrations/2026-09-07-native-brief-media.sql`; `production-write`'s `description_read` computes a native `media` projection but `index.html` never reads it, so a brief's raw `uploads.linear.app` image URLs render straight from Linear's CDN. No live media has been copied/verified. Browser reads the native projection since PR #TBD-PR-NUMBER; remaining work is the one-time copy of Linear-hosted brief images into native storage plus flipping `native_brief_media`, to be done before 2026-10-15. | Checkpoint row *"Historical Linear import/link UI and retained URL fields"*. |
