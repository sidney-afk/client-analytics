...
| **Intake form path** | `native_intake_epochs` | **native for new submissions from the updated browser; five of seven plan checks now closed.** PR **#1436** merged 2026-09-20 (browser holds) closed checks 2 and 6: offline tests cover all four browser exits holding visibly with zero legacy requests (check 2) and native pending submissions keeping their identity (check 6). Check 5 (unenrolled clients held, not enrolled or submitted automatically) closed on PR **#1441**, which holds for non-cohort clients. Check 7 (both team drills ran on native intake) closed 2026-09-20 on the owner's acceptance of the live test-client submission as drill evidence — the same one recorded below. Check 1 (every unfinished server-visible legacy batch/receipt disposed of) closed by **owner decision 2026-09-20**: two July receipts were abandoned, with no recovery attempted. Post-merge live observation 2026-09-20, not one of the plan's numbered checks: one normal submission on the test client produced VID-15027 and GRA-8023 with no Linear issue, exactly one `skipped` `mirror_outbox` row each (the native marker) and zero queued or sent rows. **Check 3 is still empty/open — no reported receipt exists to dispose of.** **Check 4 (endpoint enforcement at both legacy webhooks) remains an open owner decision, but is explicitly NOT a blocker on this row's overall closure** — the outbound cutoff below proceeded without it, by the same owner decision. Plan and checks: `docs/ops/LINEAR_EXIT_STEP26_NATIVE_INTAKE.md`. | Checkpoint row *"Intake: VIDEO_FORM_WEBHOOK and legacy dispatch selection"*, plus the **intake** half of the provider-branch row. |
| **Workload page** | no single flag key — closes on the source-reading measurement below, not a flag flip | **Native, closed 2026-09-20.** PRs **#1438**, **#1441**, **#1444** and **#1445** merged 2026-09-20 completed the two-slice native-mirror move: #1438 bound legacy feedback routes to `production-comments`; #1441 retired the legacy Calendar post-create discovery path and routed bound-row due-date/label reads off `workload-linear`; #1444 minted a synthetic parent for natively-created batches with no Linear parent (so their video/graphics children stop surfacing as unparented top-level issues); #1445 fixed four Workload/Calendar sites that still assumed a Linear URL/UUID was always present on a row (`wlFetchNativeMetadata`, `_calLatestNativeBatches`, `_calApplyFocusRequest`, `_calLinkDuplicatePeers`/`_calIsArchivedRef` — OPEN_REPAIRS **218**). The native snapshot (`workload_native_snapshot_v1`) was measured **0 legacy-source rows on both the video and graphics teams** (28 `source='legacy'` rows total, all CON/STR, none video or graphics — journal 2026-09-20 "Cohort/outbound/Gate 0 readback"). **The browser makes no Linear call from the board.** Plan and checks doc: `docs/ops/LINEAR_EXIT_STEP26_NATIVE_WORKLOAD.md`. | Checkpoint row *"Workload: index.html LINEAR_ISSUES_WEBHOOK, LINEAR_TWEAK_COMMENTS_WEBHOOK and WORKLOAD_LINEAR_URL calls; workload-linear function"*. **Closed.** |
| **Urgent editor assignee lookup** | `urgent_video_destination` | **native for bound cards, verified in PR #1440.** Investigation split the checkpoint's one row into two candidates: Candidate A (`_calUrgentSlackDispatch`/`_calSendUrgentSlack`, the urgent Slack ping's editor resolution) was found already fully built on `main` — a bound card reroutes to `native_urgent_dispatch` on `production-write`, an unbound legacy card falls through to the legacy `send-urgent-slack` webhook, and the transitional `400 unsupported_action` case falls back the same way once — and verified by **42 offline test cases** in `test/native-urgent-ui.js` covering every branch across all four staff surfaces. Candidate B (`_calLinearMissingForCard`'s "incomplete sub-issue" banner) had a real gap — it fetched `linear-issue-statuses` for a SyncView-authoritative team's idents even though the result was already discarded (OPEN_REPAIRS 62) — trimmed to exclude a sealed team's idents from the batch, with no UI or banner-copy change. **No `urgent_video_destination` flag exists live** — the 2026-09-20 Cohort/outbound readback reports it absent from `syncview_runtime_flags`, not guessed. **No unbound legacy video card remains on the board.** What remains open is narrower than the checkpoint row implied: a genuinely never-bound legacy card has no native deliverable id to send at all, which needs either a native-binding backfill feature or an n8n change accepting a pre-resolved editor identity — out of scope for this slice. | Checkpoint row *"Legacy urgent editor assignee lookup"*. |
| **Identifier mint** | `production_native_identifier_mint` (gate `production_native_identifier_capability`) | **native, both teams** — video 2026-09-19 (flipped between its 03:16:47Z seed and the 03:24:04Z observation), graphics **2026-09-19T03:27:59Z**. Seeds: `VID` from 15,000, `GRA` **hand-seeded** from 8,000 because its own seed function refuses the team. The backfill ran at 03:29Z and named the **45** rows that had no identifier, so **zero remain**. Corroborated here read-only: 47 grants, 47 cards carrying exactly their own grant, cursors at 15,025 and 8,022. Installed 2026-09-17 with **bodies verified** against the committed migration; nothing calls the gate from `index.html` or an Edge Function **because nothing needs to** — it is a `before insert or update` trigger on `deliverables`, so "not wired" was always the wrong test for it. ⏳ **Step 27 is 12 of 13 clauses; check 7 (both teams on real work) is OPEN** pending the first organic graphics card after 03:27:59Z, so **no step 28 closure is recorded.** **Still open as of 2026-09-20:** no real submission has landed on graphics since the flip, so check 7 continues to wait on that first real card, not a further measurement. See [the step 26/27 procedure](LINEAR_EXIT_STEP26_NATIVE_IDENTIFIER_MINT.md) and the checkpoint's filled form. | The **naming** half of the `linear-outbound` dependency — the reason lane F's outbound-off step is gated on this capability. Candidate only, to be confirmed against the checkpoint's dependency table at step 28; a pre-written closure with unfilled placeholders sits in the checkpoint and closes nothing. **Partial by construction:** unlike every row above, a green step 27 here does **not** make the legacy route unreachable — with outbound `live` the provider mint still runs and still mints at Linear, and the trigger only refuses to let its name land. That row is deferred to lane F's outbound-off step, so this dependency stays **open** with the capability recorded against it as *replacement built and proven, legacy route still live*. |
| **Brief media** | `native_brief_media` (mode `off`) | **code installed but not wired.** Dormant since `migrations/2026-09-07-native-brief-media.sql`; `production-write`'s `description_read` computes a native `media` projection but `index.html` never reads it, so a brief's raw `uploads.linear.app` image URLs render straight from Linear's CDN. No live media has been copied/verified. Browser reads the native projection since PR #1442. The one-time copy tool is built and merged in PR **#1451** (`scripts/native-brief-media-copy.mjs`, `plan` then `apply --apply` with an explicit confirm env; it re-resolves a fresh signed URL per file through Linear's API, writes one verified occurrence row per deliverable/team/offset, and prints the activating flag flip only once a zero-gap run supplies real coverage and recovery receipts). The **first live run of the tool happened the same evening** on the owner's machine: `plan` found 487 briefs carrying 1,339 Linear image references to 1,168 distinct files, every row carrying the Linear issue id the re-fetch needs (0 refusable). Two findings from that run: the script's CLI guard never matched on Windows (backslash path vs `file:///C:/` URL) so it exited 0 silently — fixed in **#1452** for this script and `linear-media-rescue.mjs`, which had the same guard; and `apply` stopped before writing because the Storage machine holds no `SUPABASE_SERVICE_ROLE_KEY`, which the owner is supplying by hand into that shell's environment. `apply` and the flag flip are the next step once both land. Deadline unchanged: before 2026-10-15, and before STEP 7. | Checkpoint row *"Historical Linear import/link UI and retained URL fields"*. |
| **Card materialization** | `native_card_materialization` | **dropped, 2026-09-20 — no remaining candidate, for a narrower reason than "0 legacy rows".** `migrations/2026-09-06-native-card-materialization-boundary.sql` is still the only file that references the flag, still seeded `{"mode":"hold","epoch":null}` and never flipped to `native`; nothing in `index.html`, any Edge Function or any script calls its RPC, `production_card_materialize`. **This is not the same concern as the Workload row's legacy-row count** — that measurement is `workload_issues`/`workload_native_snapshot_v1`, an unrelated table. `production_card_materialize`'s only real caller, repo-wide, is `legacy_intake_native_finish` (`migrations/2026-09-07-legacy-intake-native-triage.sql`), invoked with source `submission-native` to turn a **recovered legacy intake receipt** into a native Calendar card — the mechanism check 1 on the intake row would have used to recover the two abandoned July receipts. The owner's 2026-09-20 decision to abandon those two receipts with no recovery attempted (see the Intake form path row's check 1) is what removes the last candidate this boundary exists to materialize, not the Workload measurement. A future unresolved legacy receipt, or a decision to wire `calendar-native`/`samples-native` sources for ordinary native submissions (accepted by the function's source allowlist but never called for that purpose), would reopen this row. | **unknown, verify** — no checkpoint row or OPEN_REPAIRS entry identified. |
| **`LINEAR_CUTOFF_RUNBOOK.md` STEP 1 (legacy parity off) and STEP 3 (outbound off)** — **not** STEP 6, corrected below | `linear_legacy_parity_enabled`, `linear_outbound_enabled` | **off, 2026-09-20.** `linear_legacy_parity_enabled` moved `{"enabled":true}` → `{"enabled":false}` (the runbook's STEP 1), committed **2026-09-20T17:35:54.465602Z**; `linear_outbound_enabled` moved `{"mode":"live"}` → `{"mode":"off"}` (STEP 3), committed **2026-09-20T17:26:22.363379Z**. Both writes recorded, verified before commit and read back after, in `EXECUTION_LOG.md`, `ROLLBACK.md` and `docs/truth/BRIEFING.md`. The **nine-minute interval** between the two flips, and their reversed order against the runbook's own STEP 1-before-STEP 3 sequencing, is accepted by the owner, since Linear is retired. The **latched pending-age alarm** (`linear-outbound-drain.yml` run 35526389014: `alerts.oldest_pending_age: true`, both teams 3005 minutes oldest-pending against a 30-minute threshold, backlog 45) is a known, accepted state, not a fault: with outbound off the backlog cannot drain and its age only grows. **Cleared later the same day:** the owner authorised marking the 45 frozen `mirror_outbox` rows terminal, and the Storage session set them to the native `skipped` marker with a freeze note (no Linear write, nothing deployed); the next `linear-outbound-drain.yml` summary read `alerts.oldest_pending_age: false`, backlog 0. **These two flag flips are not STEP 6.** The runbook's own STEP 6 ("Stop the schedulers", `LINEAR_CUTOFF_RUNBOOK.md:414`) is disabling the n8n pager nodes and the scheduled GitHub Actions workflows/lanes — executed the same evening, see the STEP 6 row below. | Closes the outbound half of the provider-branch dependency across every remaining capability; gates the intake plan's check 11 (final cutoff) and this map's identifier-mint row's deferred dependency. STEP 6 (scheduler shutdown) closed separately the same evening, next row. |
| **`LINEAR_CUTOFF_RUNBOOK.md` STEP 6 (stop the schedulers)** | none — n8n pager + workflow `schedule:` blocks | **done, 2026-09-20 evening, in two halves.** (1) The n8n `SyncView Monitoring Pager + Reconciler V2 Trigger` (`qllIDZPkdNAPRj0b`) was **deactivated by the owner at about 20:12Z** — it was entirely Linear-shaped: it dispatched the two card reconcilers and the B1 refresh and DMed the owner about Linear mirror staleness. `SyncView Workload — Reconcile` (`lGwC9WWPVJtxphtf`) **stays active by owner decision**. The gate for stopping it is the runbook's own (`LINEAR_CUTOFF_RUNBOOK.md:457-458`): the Workload board no longer depends on `workload_issues` freshness — and it still does, because the native snapshot still carries **28 `source='legacy'` rows, all CON/STR** (the Workload row above), which only that reconcile keeps current. The freshness watcher (`workload-source-freshness.yml`, lane `workload_source_freshness`) already exists and is live; it only reports a frozen mirror, it does not remove the dependency, so its existence is **not** the condition. Stopping the reconcile before those 28 rows have a native source freezes them silently rather than emptying them. (2) PR **#1449** commented out the `schedule:` block (dispatch kept) on the five retire-classified workflows — `production-shadow-audit`, `linear-deliverables-reconcile`, `linear-outbound-drain`, `sample-linear-reconcile`, `b1-linear-incremental-refresh` — and retired the `reconciler_pager`, `b1_incremental_refresh` and `production_shadow_audit` watchdog lanes in the same commit, re-pinning both closure digests. Then, by **owner decision the same evening (option 2 of two)**, `production-write-drill.yml` was unscheduled and the `production_write_drill` lane retired in the same PR, so **all four `retires_with: 'linear'` lanes are retired and no scheduled workflow in the repo requires a `LINEAR_*` secret** — `test/monitoring-watchdog.js` asserts that property in both directions. The cost is explicit in the workflow header: the daily native write-gateway proof is lost until a credential-free rewrite re-enables the cron and un-retires the lane together. Nothing classified keep or rewrite was touched. | Closes the runbook's STEP 6. STEP 7 (revoke `LINEAR_API_KEY`) remains the one open runbook step and is deliberately last; the brief-media copy below must run before it, since the copy re-reads Linear once per file. |
...
**29b and 29c are filed.** `docs/ops/LINEAR_EXIT_STEP29B_INVENTORY.md` classifies every `.github/workflows/` file and every `monitoring-watchdog.js` lane keep/retire/rewrite, with nothing retired — journaled as "step 29b inventory filed: every workflow and lane classified keep/retire/rewrite, nothing retired". `docs/ops/LINEAR_EXIT_STEP29C_ALERT_CONSOLIDATION.md` is filed the same way, after two documented corrections (narrowed live pager scope, a bounded alert-retirement design) recorded in the journal on 2026-09-19. Neither has retired or changed what is sent — both remain proposals for the owner to act on.

**29a is pending today**, after the last code PRs of this slice (#1445, then #1447, #1450 and #1451 the same evening). The owner's live functional click-through of Submit, Workload, Calendar and Samples has not yet been recorded against this closure wave. Two things learned today shape it: the owner's own Submit test on the test client surfaced that a natively-created batch showed no parent or siblings (fixed in #1444, with the follow-on Linear-identity assumptions fixed in #1445 and #1447), so the pass must include **opening one freshly submitted video and confirming its batch and siblings are visible**; and the Submit tab must be reachable **without a staff sign-in** for videographers — that link is `https://syncview.synchrosocial.com/?intake=1`, to be tried in a private window. Every "Add sub-issue" affordance was removed from the Production tab in **#1450** (CLAUDE.md standing rule), so 29a should find none.

**Monday 2026-09-21 is covered by a scheduled read-only check**, not by anyone's memory: a routine fires at 15:00Z to count the first real native cards created after the cutoff (identifier-mint check 7 was accepted on test-client evidence, so this is the first look at real-client cards). If the session that fires cannot reach the database, it hands the owner a read-only query for the Storage session instead.
