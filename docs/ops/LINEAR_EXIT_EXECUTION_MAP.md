...
decisions are taken, and the storage session has seeded and flipped **video**
(03:16:47Z, `VID`, cursor 15,000). Two checks have passed — **3** (a card showed
its native name with no refresh) and **4a** (the cursor advanced by exactly
one). Graphics, the backfill and the remaining checks are outstanding. Reported
as:

```
Phase 7 of 7 · identifier mint: video seeded and flipped native 2026-09-19; checks 3 and 4a passed; graphics not yet seeded · step 28 form partly filled, NOT recorded
```
...
| **Identifier mint** | `production_native_identifier_mint` (gate `production_native_identifier_capability`) | **applied and inert**, corrected 2026-09-19 from *code installed but not wired*. Measured live: the four functions and `zzz_production_native_identifier_mint` are installed and their **bodies match** the committed migration (`md5(prosrc)` per routine, plus `prosecdef`/`provolatile`/`proconfig` and `pg_get_triggerdef`); the flag row was written **2026-09-17T16:14:54Z** and reads `provider` for both teams; **0 seed rows, 0 grants**. Nothing mints, and nothing calls the gate from `index.html` or an Edge Function — **because nothing needs to.** The capability is a `before insert or update` trigger on `deliverables`, so every creation path is covered without a gateway operation; "not wired" was the wrong test for this one. **Video is seeded and flipped native as of 2026-09-19** (03:16:47Z; `VID`, gap 1,064, cursor 15,000 → 15,001 after the first allocation), under [the step 26/27 procedure](LINEAR_EXIT_STEP26_NATIVE_IDENTIFIER_MINT.md). Checks **3** and **4a** have passed; graphics is not seeded, the backfill has not run, and **no step 28 closure is recorded.** | The **naming** half of the `linear-outbound` dependency — the reason lane F's outbound-off step is gated on this capability. Candidate only, to be confirmed against the checkpoint's dependency table at step 28; a pre-written closure with unfilled placeholders sits in the checkpoint and closes nothing. **Partial by construction:** unlike every row above, a green step 27 here does **not** make the legacy route unreachable — with outbound `live` the provider mint still runs and still mints at Linear, and the trigger only refuses to let its name land. That row is deferred to lane F's outbound-off step, so this dependency stays **open** with the capability recorded against it as *replacement built and proven, legacy route still live*. |
