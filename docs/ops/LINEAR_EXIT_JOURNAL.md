...
- **Card materialization.** Dropped — the same 0-legacy-row measurement above means there is nothing left for this boundary to materialize.
- **Identifier mint.** Check 7 (both teams on real work) is still open: no real graphics submission has landed since the flip.
- **New Step 6 row** (this map had none before today): records the outbound cutoff — `linear_outbound_enabled` off since 17:26:22Z and `linear_legacy_parity_enabled` off since 17:35:54Z, both recorded in `EXECUTION_LOG.md`, `ROLLBACK.md` and `docs/truth/BRIEFING.md`. The nine-minute gap between the two flips is accepted by the owner since Linear is retired. The latched `oldest_pending_age` alarm awaits an owner decision on marking the 45 frozen outbox rows terminal.
