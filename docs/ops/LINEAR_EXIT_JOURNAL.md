...
### 2026-09-20 — Native batch with no Linear parent at all now mints its own synthetic parent node. `_prodResolveBatchParentNodes` (index.html) only minted synthetic parent nodes from `batch.linear_parent_ids`, so a batch created entirely natively — outbound skipped, `linear_parent_ids` is `null` — had nothing for that map to key off: its video/graphics children surfaced as bare top-level issues in the Production tab, no "Sub-issue of", no way back to the batch or its siblings. Repro was the Calendar → submit a batch on the test client → open Video 1. Fix mints one synthetic parent per TEAM present among the batch's own children (via `childCounts`) whenever a batch has children but no usable `linear_parent_ids` entry, keyed the same way the existing two-team Linear-backed case is (bare batch id for the first team, `batchId::team` suffix for the second) and linked through the same `links`/`nodes` shape the renderer already consumes for "Sub-issue of" and the parent's progress chip. Linear-backed batches, the cross-batch-uuid ambiguity guard, and the archived tie-break are untouched — the new branch only fires where the existing one produced nothing. New fixture in `test/production-parent-link-hierarchy.js`: a native batch with 3 video + 3 graphics deliverables and `linear_parent_ids: null` — asserts two nodes mint (one per team), each child links to its own team's node, and it is order-independent; the file's existing Linear-backed fixtures pass unchanged. Also ran `docs/syncview-design/tests/prod-write-gateway-browser.js` clean. No live read, deployment, or database write.
