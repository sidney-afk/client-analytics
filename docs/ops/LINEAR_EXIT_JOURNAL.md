...
### 2026-09-21 — Supervisor close-out (Fable): the index.html split is COMPLETE, three staff-reported fixes shipped, bulk-park pending the SMMs. **Modularization**: Step 0 (#1464), cut 1 (#1465), then seven batch PRs (#1468 cuts 2–6, #1470 cuts 7–11, #1472 cuts 12–16, #1473 cuts 17–21, #1474 cuts 22–26, #1475 cuts 27–30, #1476 cuts 31–34 plus footer) all merged the same day; `src/index/999-remainder.html.part` is retired and `manifest.txt` lists 37 content fragments. Every cut PR proved an empty `index.html` diff against its own then-current main; the final served-page hash is `6b5f44936a5e5123cdeb343b778430be3d8680bb418b0a89a01046253bdf483d` (5,776,430 bytes), which differs from Step 0's only because three ordinary fixes landed between cuts. Supervisor decisions on the way: a trailing blank line at a fragment end is accepted (byte identity is the contract, no gate runs `git diff --check` on fragments); when a code PR and a cut PR are both open, the supervisor merges main into the trailing branch and regenerates `INDEX.md` with `npm run build:index` rather than spending an executor round on it (done four times today); the plan's line numbers go stale after any fix above a cut, so anchors were re-verified on main before every batch prompt. **Fixes shipped**: #1467 batch-view deliverables ordered like the parent view; #1469 archiving a post parks its sub-issues again (the callers stripped the row before the park step ever saw it; OPEN_REPAIRS 226); #1471 a batch deep link opens the batch's real parent card when it has exactly one, and batch rows draw with the sub-issue renderer. **Bulk-park of the cards the bug left open**: read-only phase 1 by Storage found 25 `todo` sub-issues on 17 posts archived since 2026-08-17 (plus 9 mid-review ones left alone); 4 of the 25 carry a Linear-era archived mark and are already hidden everywhere; the owner chose to ask the four SMMs before parking, so phase 2 waits on their answers. **Docs PR #1466** merged with four Codex corrections, including the hiring-mail truth (both hiring flags read enabled live; five docs said off). Nothing here wrote live data except the merges

### 2026-09-21 — Supervisor day log (Fable): the open list worked, one runbook order corrected, modularization Step 0 and cut 1 landed. **Item 1 (28 legacy rows)**: Storage's first retire at ~14:33Z was undone by the still-running n8n Workload reconcile at 14:50:16Z (all 28 rows re-synced `active=true`); the reconcile `lGwC9WWPVJtxphtf` was deactivated on the owner's word at ~14:57Z, Storage redid the retire at 15:32Z, and two verifications (immediately and at 15:47Z) plus a supervisor readback at 15:59Z all read 0 legacy rows and `legacy_teams` length 0. Runbook STEP 6 item 3 now records the corrected order (reconcile off first). #1463 merged. **Item 9 (modularization)**: Step 0 merged as #1464 (two fragments, build/check tooling, unit-lane wiring, page byte-identical at `5990e9fc9495c7420dad4bf55f4699f61a964f802be2927a513103efe28de745`, 5,768,894 bytes). Three decisions were needed on the way: the 24 open feature PRs touching `index.html` are not cut PRs and do not block; the identity gate's "added line" rule now ignores a line whose exact bytes already exist in the base's `index.html`, and only when the destination is a `src/index/*.part` fragment (the Codex bot's P1 tightened the first version); two hash pins of `package.json` (`scripts/f27-reconciler-closure.js`, PTO manifest) were refreshed from tooling. `production-polish` went red once on a byte-identical page with `page_error_pending_read` during a Supabase 502 window and passed on the next push. Cut 1 opened as #1465. Execution notes appended to the plan. **Also today**: Monday 15:00Z check found no real cards created yet (rescheduled); a report from an SMM led to the finding that archiving a calendar post no longer parks its sub-issues in Backlog natively (zero calendar-surface `status_change → backlog` events since 2026-08-17 against ~80 archives in 11 days; the refusal-receipt table has 0 rows ever) — handed to an execution session with the evidence, OPEN_REPAIRS 225 and the session's own entry to follow; a report from a video editor led to the batch-view ordering fix (batch detail lists deliverables in fetch order while the parent view orders video-then-graphics numerically) — handed to a second session. Everything here was read-only except the n8n unpublish and the merges

### 2026-09-21 — Legacy Workload parent rows retired AGAIN (redo of the entry below): the n8n reconcile re-activated the same rows at 14:50Z and is now off. Redo at 15:32Z: the read-only selection returned 28 active rows outside VID/GRA and their ids matched the saved rollback list exactly; one transaction set `active=false`, row count 28 verified before commit. Verification 1 (15:32Z, right after): the selection returns 0 rows, `legacy_teams` length 0, 0 of the saved ids active. Verification 2 (15:47Z, 15 minutes later): the same three checks return 0, 0, 0, so nothing re-activated them. Rollback list folder unchanged (private, outside the repo). The entry below is kept as written
...
- **Intake form path.** Checks 2 and 6 closed on PR #1436; check 5 closed on PR #1441 (holds for non-cohort clients); check 7 closed on the owner's acceptance of today's live test-client submission as drill evidence; check 1 closed by owner decision (two July receipts abandoned, no recovery). Check 3 stays empty/open — no reported receipt. Check 4 (legacy webhook enforcement) stays an open owner decision, explicitly not a blocker on this row's closure.
- **Workload page.** Moved to native and closed, on PRs #1438, #1441, #1444 and #1445 merged today. The native snapshot measured 0 legacy-source rows on both video and graphics teams (28 `source='legacy'` rows exist, all CON/STR, none on either production team).
- **Urgent editor assignee lookup.** Native for bound cards, verified in PR #1440 (42 offline cases in `test/native-urgent-ui.js`). No `urgent_video_destination` flag exists live. No unbound legacy video card remains on the board.
- **Card materialization.** Dropped — no remaining candidate. **Correction, same day, Codex review on PR #1446:** the row's first draft cited the Workload row's 0-legacy-row measurement as the reason, which is a different table (`workload_issues`) and not this boundary's concern. Read `production_card_materialize`'s only real caller (`legacy_intake_native_finish`) and confirmed the actual reason: it recovers a **legacy intake receipt** into a native Calendar card, and the owner's 2026-09-20 decision to abandon the two July receipts with no recovery (Intake form path row, check 1) is what removes the last candidate. Fixed before merge.
- **Identifier mint.** Check 7 (both teams on real work) is still open: no real graphics submission has landed since the flip.
- **New capability row for the two flag flips** (this map had none before today): records `LINEAR_CUTOFF_RUNBOOK.md` STEP 1 (legacy parity off, 17:35:54Z) and STEP 3 (outbound off, 17:26:22Z), both recorded in `EXECUTION_LOG.md`, `ROLLBACK.md` and `docs/truth/BRIEFING.md`. The nine-minute gap between the two flips, and their reversed order against the runbook's sequencing, is accepted by the owner since Linear is retired. The latched `oldest_pending_age` alarm awaits an owner decision on marking the 45 frozen outbox rows terminal. **Correction, same day, Codex review on PR #1446:** the row's first draft mislabeled these two flips as the runbook's STEP 6 ("Stop the schedulers"). They are STEP 1 and STEP 3. STEP 6 is a separate, later step — disabling n8n pager nodes and scheduled GitHub Actions workflows — and per the step-29b inventory it has not run: most of those lanes are still on their normal schedule. Fixed before merge; the row now says explicitly that STEP 6 stays open.
- **Step 29.** 29a (the owner's live functional click-through) is pending, after the last code PR (#1445). 29b and 29c remain filed, as recorded on 2026-09-19 — nothing retired.

Also appended one item to `docs/ops/LINEAR_EXIT_STEP29B_INVENTORY.md`: `production-polish-gate.yml` has been red on every push to `main` in the run history checked, unrelated to the Linear exit, flagged as a candidate for a separate repair-or-retire decision.

### 2026-09-20 — Native batch with no Linear parent at all now mints its own synthetic parent node. `_prodResolveBatchParentNodes` (index.html) only minted synthetic parent nodes from `batch.linear_parent_ids`, so a batch created entirely natively — outbound skipped, `linear_parent_ids` is `null` — had nothing for that map to key off: its video/graphics children surfaced as bare top-level issues in the Production tab, no "Sub-issue of", no way back to the batch or its siblings. Repro was the Calendar → submit a batch on the test client → open Video 1. Fix mints exactly ONE synthetic parent per batch (owned by the primary team — video when present) whenever a batch has children but no usable `linear_parent_ids` entry, and links every child of every team to that same node — the native mirror of the "ONE PARENT PER CARD" rule production-write's own batch-create intake already follows (owner ruling 2026-08-18). A first pass minted one node per team instead; Codex review on the PR caught that this left a mixed-team native batch with no route from a video to its thumbnail siblings, just through a different missing link, so it was corrected before merge. Review also caught that the native fallback was silently reparenting a row with an unresolvable *explicit* `raw_issue_parent_id` (missing/duplicated/self-referential/cyclic target) to its batch, reversing the function's existing fail-closed behavior; the fallback now only fires when `raw_issue_parent_id` is empty to begin with. Linear-backed batches, the cross-batch-uuid ambiguity guard, and the archived tie-break are untouched. A THIRD bug surfaced by `production-polish-gate`'s own live-backend `prod-structure-subset.js` run on this PR (first read as a pre-existing gate failure — it was not: it passed on the two prior same-day merges and only broke here): the fallback's "no `raw_issue_parent_id`" gate also matched a row that IS Linear-born (`linear_issue_uuid` set) but simply has no recorded parent — that fixture's own `same-batch-root` row, deliberately meant to stay a root — and the fallback swept it into the native synthetic parent instead. Both the batch-minting gate and the per-row fallback now additionally require the row to be genuinely native-born (`linear_issue_uuid` empty); a Linear-born unparented row is never swept into a synthetic native parent. `test/production-parent-link-hierarchy.js` gained three fixtures: a native batch with 3 video + 3 graphics deliverables and `linear_parent_ids: null` — asserts exactly one node mints and every child of both teams links to it, order-independent; a native batch row with an unresolvable explicit parent edge, asserting it stays a root; and a batch mixing native rows with a Linear-born unparented row, asserting the native rows link to the synthetic parent while the Linear-born row stays a root. The file's existing Linear-backed fixtures pass unchanged. Also ran `docs/syncview-design/tests/prod-write-gateway-browser.js` clean; `prod-structure-subset.js` could not be run locally in this sandbox (fails identically on unmodified `main` on an app-boot limitation, not this code), relying on CI. No live read, deployment, or database write.

### 2026-09-20 — Step 6: Linear outbound mirror switched OFF (authorized live write, `syncview_runtime_flags` only). Precondition: `prod_authority` = `{"video":"syncview","graphics":"syncview"}` for both teams — continued. `linear_outbound_enabled` was `{"mode":"live"}` (last set 2026-08-16 19:36:49Z), written to `{"mode":"off"}`, verified inside the transaction (`value->>'mode'`='off', `jsonb_typeof(value)`='object') before commit, and confirmed on a post-commit readback. The write ran twice — once as intended, once by accident while checking the script's own exit code; both wrote the identical `{"mode":"off"}` value, so the current row's `updated_at`, **2026-09-20 17:26:22.363379+00**, is the second call's timestamp and the one that is authoritative. 3-minute post-flip check: 7 `mirror_outbox` rows created after that timestamp, all `skipped`, **0 written**, 0 of any other status — as expected. Follow-up (Codex findings): `linear_legacy_parity_enabled` also switched `{"enabled":true}`->`{"enabled":false}` at 2026-09-20 17:35:54.465602+00, same verify-before-commit pattern, run once. STEP 0 census re-read: real-client `pending`/`failed`/`shadow_ok` unchanged at 17 `failed` rows (2026-09-18 15:30:34..15:35:26Z), 0 `pending`, 0 `shadow_ok`. `linear-outbound-drain.yml` dispatched once (run 35526389014): `mode` `off`, `backlog` 45, `alerts.oldest_pending_age` **true** (not the expected false) with both teams at 3005 minutes oldest-pending against a 30-minute threshold — latched because outbound is off and cannot drain it
