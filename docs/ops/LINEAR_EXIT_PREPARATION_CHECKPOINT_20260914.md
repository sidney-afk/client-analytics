...
| **The exact gate** | `production_native_identifier_capability(p_team)`. It returns `native` only when the runtime flag says native for that team **and** a seed row exists in `production_native_identifier_mint` — the self-guard that makes a premature flip inert rather than half-armed, deliberately unlike `production_label_catalog_capability()`. Native since: **video — flipped 2026-09-19, between the seed at 03:16:47Z and the observation at 03:24:04Z** (the flip's own timestamp was not reported and is not inferred here). Graphics «TIMESTAMP — not yet flipped». |
| **The accepted replacement** | The `zzz_production_native_identifier_mint` trigger on `public.deliverables`, allocating through `production_native_identifier_allocate` against a per-team cursor. Installed **2026-09-17**, bodies verified against the committed migration **2026-09-19** by `md5(prosrc)` per routine plus `prosecdef`/`provolatile`/`proconfig` and `pg_get_triggerdef`. Seeded: **video — 2026-09-19T03:16:47Z, prefix `VID`, `observed_provider_max` 13,935, `seed_gap` 1,064, `next_ordinal` 15,000.** The three agree: 13,935 + 1,064 + 1 = 15,000, so the cursor was derived from the observed provider maximum and the gap was chosen to land the first native name on a round number. Graphics «prefix, observed_provider_max, seed_gap, next_ordinal» — **hand-seeded by owner decision**, see the scope note. |
| **What was measured — video** | Grants in `production_native_identifier_grants`: «N, not reported». Per check, per clause: **1** «», **2a** ⚠ *see the note below — not closed*, **2b** «grant row for the minted name — not reported», **3** ✅ **owner-observed 2026-09-19T03:24:04Z**: card `del_670c2cb9` showed **`VID-15000`** in the Production list **without a refresh**, so the create response carried the post-trigger name and the gateway readback question raised in the procedure's section (a) is answered — no `production-write` change is needed for it. **4a** ✅ cursor **15,000 → 15,001**, advanced by exactly one, consistent with a single allocation. **4b** «offline, qa/native-identifier-mint/sql-proof.js run id», **5** «path: native-intake or draining», **5a–5c** «or "not applicable, native intake"», **5d** «», **6a** «», **6b** «», **6c** «», **7** «graphics, after its own flip». **Report per check and per clause** — the labels closure was withdrawn for closing a check whose third clause had never been run. |
| **⚠ Why check 2a is NOT closed by the check 3 observation** | The card visibly carried `VID-15000`, which is the *substance* of 2a, and it is tempting to tick both from one sighting. **They are different measurements.** Check 3 is satisfied by what the browser rendered; 2a asks for a readback of `deliverables.linear_identifier` for that row id, and 2b for the matching grant row. Neither readback was reported. The ordinal and the cursor make both near-certain — which is exactly the reasoning that closed a labels check on an adjacent measurement and had to be withdrawn. **Near-certain is not measured.** Both stay open until the storage session reports the readbacks. |
