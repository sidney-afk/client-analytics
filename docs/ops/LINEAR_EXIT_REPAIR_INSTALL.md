...
9. `migrations/2026-09-09-native-attribution-browser-projection.sql` must follow native client provisioning **and** the complete Workload chain below, because the replaced view calls `production_workload_label_projection`. Preflight v5 also requires the `native_intake_legacy_project` source marker in the installed view definition; columns alone cannot distinguish the older projection that hides the new stamp. It precedes the browser source that reads its bounded `raw_attribution_project_id` and `raw_attribution_native_epoch` fields.
