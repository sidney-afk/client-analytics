...
After step 11, install `migrations/2026-09-11-native-signoff-verifier.sql` before enabling the updated sign-off reconciler. The 64-entry inventory pins this dependency after the final ordinary-envelope repair. The updated deployment preflight checks its exact body, STABLE volatility, return columns, security-definer search path and service-only execution. See `../independence/LINEAR_EXIT_COMPOSITION_CHECKPOINT_20260912.md` for isolated evidence; hosted REST and configuration remain unproven.
