...
| 3 | **Every reported browser-held legacy receipt is disposed of on its original identity.** A visible hold (check 2) is necessary but does not pass this check. | Offline tests show that the saved-receipt hold leads to terminal recovery on the original request identity, with no new identity and no duplicate card, and that an unrecoverable receipt stays held until an approved disposition is recorded against that identity. Live: every browser-held receipt **reported** by a staff browser is recorded with its original identity and either a terminal recovery or an approved disposition. This check covers **reported receipts only**; it makes no claim about browsers that were not reported, and it is not evidence for check 4. **Recovery stays available after closure:** a receipt discovered later, in any browser, is still recovered or disposed of on its original identity, never re-issued. |
| 4 | **Old browsers cannot create new legacy submissions.** This is required before the legacy route may be declared unreachable. | Either **(a) verified enforcement at the receiving endpoint**: the legacy form webhooks (`video-form`, `graphic-form`) refuse a new submission, with the refusal read back from the endpoint itself (its configuration and a refused TEST request), not inferred from the browser, or **(b) equivalent bounded stale-browser evidence**: a measured bound on how long an old page can still submit (for example, a server-enforced expiry every old page is subject to), plus evidence that the bound has elapsed with no new legacy submission arriving. A browser hold, a reported-receipt count (check 3) or a quiet outbox is not evidence for this check. Any change to the webhooks themselves is separate approved work, outside this plan. |
