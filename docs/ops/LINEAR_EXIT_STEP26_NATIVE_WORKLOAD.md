...
| Early mirror prefetch (browser head script) | On page load, unless the cache is fresh or the kill switch is set, the head script fetches the first page of `workload_issues` into `window.__wlEarlyIssues`. Its only consumer is `_wlV2FetchIssues()`, which is reached only through `_wlLegacyLoadLinearIssues()` (always called with `force` set, which skips the mirror) and `_wlLegacyRefetchSilent()` (no callers). Read from source, 2026-09-19. | None needed: normal loading does not use the prefetched rows. | **A leftover mirror read.** It still reads `workload_issues` on every qualifying page load, and fails silently once the mirror stops. Remove it, together with its unreachable consumers, before closure. |
| Mirror watermark reader `_wlV2FetchLatestWatermark()` | Reads the newest `workload_issues.synced_at`. Its two callers are dead: `_wlLegacyCheckWatermark()` has no callers, and `wlRebaseMirrorWatermarkAfterDirectRefresh()` is reached from `wlManualRefresh()` only behind `shouldRebaseMirror`, which is the constant `false`. Read from source, 2026-09-19. | None needed: native snapshots do not use the mirror watermark. | **A dead mirror read.** Remove it and both callers before closure. |
| `wlNativeDiff()` (diagnostic, `?wlnative=1` only) | Reads `workload_issues` and the native view side by side and reports what differs. It is the acceptance comparison that Gate 0 item 2 and check 10 rely on. | Not a user path; it exists to measure the replacement. | **Keep during acceptance.** Once acceptance is recorded, explicitly retire its `workload_issues` dependency by removing it or re-pointing it at a native-only comparison, before the mirror reconcile stops. |
...
Checks 1–13 are **acceptance**. Checks 14 and 15 are **closure** checks: they are taken after acceptance is recorded, and before the background mirror reconcile stops.
...
| 15 | **No browser code reads the mirror at closure, shown by two independent searches run on the closure commit.** | **What passes this check:** both searches below are re-run **on the actual closure commit**. Every helper call site and every realtime subscription is re-resolved, their results are reconciled, and together they find **no** browser read of `workload_issues`. If the two disagree, the check fails until the difference is explained. A served-browser network capture of boot, Refresh and `?wlnative=1` also shows no `workload_issues` request. The four sites below are resolved first: sites 1–3 removed with their dead callers, and site 4 removed or re-pointed at a native-only comparison once acceptance (checks 1–13) is recorded. The diff stays available until then.<br><br>**Preliminary source evidence, not closure proof** (recorded 2026-09-19 against this PR's source at the time; it must be repeated at closure). **Search A, literal:** browser-served files (tracked `.html`/`.js`/`.mjs` outside `scripts/`, `test/`, `supabase/`, `docs/`, `migrations/`, `qa/`, `.github/` and `n8n-backups/`) searched for the string `workload_issues`. **Search B, structural:** in the same files, database accesses enumerated by form and each target relation resolved: (i) PostgREST (`/rest/v1/…`) URLs, whether literal, a named constant or built from a variable; (ii) the call sites found for each table-taking helper, with the argument parsed across line breaks: `readPage` → `production_deliverables_browser_v1`, `deliverables`; `_prodRestRows` → `production_deliverables_browser_v1`, `batches`, `clients`, `deliverables`, `deliverable_events`, `syncview_runtime_flags`, `team_members`; `_kedRestIn` → `deliverables`, `clients`, `team_members`; `_kedRestPage` → `deliverable_events` and what `_kedRestIn` passes; `_writeUiLegacySourceRows` → `sample_reviews` (`SXR_TABLE`) or `calendar_posts`; (iii) supabase-js use: five `createClient` call sites were found, with no `.from(…)` table query (the `.from(` calls found are `Array.from`), and 16 realtime `postgres_changes` subscriptions: `syncview_runtime_flags` ×6, `calendar_posts` ×2, `client_credentials_rev` ×2, `sample_reviews` (`SXR_TABLE`) ×2, `templates`, `deliverables`, `batches`, `content_samples`; (iv) split or computed spellings of the table name and `synced_at` use. **Preliminary result:** both searches found the same four reads, all in `index.html`: (1) the early head-script prefetch (`window.__wlEarlyIssues`); (2) `_wlV2FetchIssues()`, reached only through a forced call that skips the mirror or a function with no callers; (3) `_wlV2FetchLatestWatermark()`, whose callers `_wlLegacyCheckWatermark()` and `wlRebaseMirrorWatermarkAfterDirectRefresh()` are dead; (4) `wlNativeDiff()`. *Corrections kept on record:* earlier versions of this inventory missed `deliverables` at three helper call sites, and left `client_credentials_rev` out of the subscription list. This is why the inventory is preliminary, and why the check requires both searches to be repeated at closure rather than relying on this record. |

## Step 28 — closure sense

Close `native_workload` only when acceptance checks 1–13 have passed, closure
checks 14 and 15 pass in the order below, normal loading remains
...
`workload-linear` route/function. Before the mirror reconcile stops, retire the browser's own mirror reads
(check 15): remove the early prefetch, `_wlV2FetchIssues()` and
`_wlV2FetchLatestWatermark()` with their dead callers, and, once
acceptance is recorded, remove `wlNativeDiff()` or re-point it at a native-only
comparison. Retire the background mirror reconcile **last**, and only when checks
14 and 15 pass: every route above reads the rows it supplies, so stopping it
first would freeze them. Preserve the checkpoint with aggregate
