...
| 15 | **No browser code reads the mirror at closure, measured over every reader by two independent searches.** | **Search A, literal (2026-09-19):** every browser-served file (tracked `.html`/`.js`/`.mjs` outside `scripts/`, `test/`, `supabase/`, `docs/`, `migrations/`, `qa/`, `.github/` and `n8n-backups/`) searched for the string `workload_issues`. **Search B, structural (2026-09-19):** in the same files, every PostgREST (`/rest/v1/…`) and supabase-js (`.from(…)`) access is enumerated, and its target relation resolved: literal, named constant, or a table passed into a helper. The five dynamic targets resolve as follows: `readPage` → `production_deliverables_browser_v1`; `_prodRestRows` → `batches`, `clients`, `deliverable_events`, `production_deliverables_browser_v1`, `syncview_runtime_flags`, `team_members`; `_kedRestPage` / `_kedRestIn` → `deliverable_events`, `clients`, `team_members`; the legacy-source helper → `sample_reviews` or `calendar_posts`. Search B also flags split or computed spellings and `synced_at` use that the literal search would miss. **Reconciled:** both searches find the same four reads, all in `index.html`: (1) the early head-script prefetch (`window.__wlEarlyIssues`); (2) `_wlV2FetchIssues()`, reached only through a forced call that skips the mirror or a function with no callers; (3) `_wlV2FetchLatestWatermark()`, whose callers `_wlLegacyCheckWatermark()` and `wlRebaseMirrorWatermarkAfterDirectRefresh()` are dead; (4) `wlNativeDiff()`. Neither search found a read the other missed. **Pass condition:** **both** searches are re-run on the closure commit, every dynamic target is re-resolved, and together they find **no** browser read of `workload_issues`. If the two ever disagree, the check fails until the difference is explained. Sites 1–3 are removed with their dead callers, and site 4 is removed or re-pointed at a native-only comparison once acceptance (checks 1–14) is recorded. A served-browser network capture of boot, Refresh and `?wlnative=1` shows no `workload_issues` request. The diff stays available until acceptance is recorded. |
