...
| 15 | **No browser code reads the mirror at closure, measured over every reader by two independent searches.** | **Search A, literal (2026-09-19):** every browser-served file (tracked `.html`/`.js`/`.mjs` outside `scripts/`, `test/`, `supabase/`, `docs/`, `migrations/`, `qa/`, `.github/` and `n8n-backups/`) searched for the string `workload_issues`. **Search B, structural (2026-09-19; corrected the same day):** in the same files, every database access is enumerated by **form**, not by name, and each target relation is resolved: (i) every PostgREST (`/rest/v1/…`) URL, whether literal, a named constant or built from a variable; (ii) **every call site** of each helper that takes a table argument, with the argument parsed across line breaks: `readPage` → `production_deliverables_browser_v1`, `deliverables`; `_prodRestRows` → `production_deliverables_browser_v1`, `batches`, `clients`, `deliverables`, `deliverable_events`, `syncview_runtime_flags`, `team_members`; `_kedRestIn` → `deliverables`, `clients`, `team_members`; `_kedRestPage` → `deliverable_events` and whatever `_kedRestIn` passes; `_writeUiLegacySourceRows` → `sample_reviews` (`SXR_TABLE`) or `calendar_posts`; (iii) every supabase-js client (five `createClient` sites) and what it is used for: no `.from(…)` table query exists (all 126 `.from(` calls are `Array.from`), and all 16 realtime `postgres_changes` subscriptions resolve to `templates`, `deliverables`, `batches`, `content_samples`, `syncview_runtime_flags`, `calendar_posts` or `sample_reviews`; (iv) split or computed spellings of the table name and any `synced_at` use. *Correction:* the first version of this search listed helper targets from single-line matches only. That missed `deliverables` at three call sites, and it did not cover realtime subscriptions; both are now enumerated. **Reconciled:** both searches find the same four reads, all in `index.html`: (1) the early head-script prefetch (`window.__wlEarlyIssues`); (2) `_wlV2FetchIssues()`, reached only through a forced call that skips the mirror or a function with no callers; (3) `_wlV2FetchLatestWatermark()`, whose callers `_wlLegacyCheckWatermark()` and `wlRebaseMirrorWatermarkAfterDirectRefresh()` are dead; (4) `wlNativeDiff()`. Neither search found a read the other missed. **Pass condition:** **both** searches are re-run on the closure commit, every helper call site and realtime subscription is re-resolved, and together they find **no** browser read of `workload_issues`. If the two ever disagree, the check fails until the difference is explained. Sites 1–3 are removed with their dead callers, and site 4 is removed or re-pointed at a native-only comparison once acceptance (checks 1–14) is recorded. A served-browser network capture of boot, Refresh and `?wlnative=1` shows no `workload_issues` request. The diff stays available until acceptance is recorded. |
