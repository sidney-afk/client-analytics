...
| 15 | **No browser code reads the mirror at closure, measured over every reader, not named examples.** | **Complete inventory (one search, 2026-09-19):** every browser-served file was searched for `workload_issues`, meaning all tracked `.html`/`.js`/`.mjs` outside `scripts/`, `test/`, `supabase/`, `docs/`, `migrations/`, `qa/` and `.github/`. Only `index.html` reads it, at four sites: (1) the early head-script prefetch (`window.__wlEarlyIssues`); (2) `_wlV2FetchIssues()`, reached only through a forced call that skips the mirror or a function with no callers; (3) `_wlV2FetchLatestWatermark()`, whose callers `_wlLegacyCheckWatermark()` and `wlRebaseMirrorWatermarkAfterDirectRefresh()` are dead; (4) `wlNativeDiff()`. The one other match, `n8n-backups/…jsCode.js`, is an n8n code backup, not browser code. **Pass condition:** the same search, re-run on the closure commit, finds **no** browser read of `workload_issues`. Sites 1–3 are removed with their dead callers, and site 4 is removed or re-pointed at a native-only comparison once acceptance (checks 1–14) is recorded. A served-browser network capture of boot, Refresh and `?wlnative=1` shows no `workload_issues` request. The diff stays available until acceptance is recorded. |
