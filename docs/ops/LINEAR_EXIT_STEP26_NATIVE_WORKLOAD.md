...
| 15 | No browser code still reads the mirror at closure. | Source test: the early `workload_issues` prefetch and its unreachable consumers are removed, and `wlNativeDiff()` either no longer exists or no longer reads `workload_issues`. A served-browser network capture of boot, refresh and `?wlnative=1` shows no `workload_issues` request. This check is taken after acceptance: the diff stays available until checks 1–14 are recorded. |
...
(check 15): remove the early prefetch and its unreachable consumers, and, once
