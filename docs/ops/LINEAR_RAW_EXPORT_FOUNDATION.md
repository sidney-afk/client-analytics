...
Every run creates a fresh private directory. Each received, in-budget response is written with exclusive creation, synced and made owner-read-only, using its request ordinal and exact byte SHA-256. No decoded/reserialized response substitutes for the original bytes. Malformed JSON, invalid UTF-8 and BOM-prefixed JSON are retained when in budget and then refused. Over-budget or unavailable bodies are not claimed retained. Raw HTTP/GraphQL errors and all descriptions, comment bodies, identities and URLs stay only in these private response files.
