...
**2026-09-07 serving composition preparation, SOURCE_ONLY:** a [pinned private builder](../audits/2026-09-07-frozen-native-composition.md) adds five native seams to exact captured anonymous Calendar v48/Samples v49, preserving every other serving byte.38 complete-handler/model-boundary checks pass; repository writers are untouched. One owned disposable HTTP/SQL run passed42 total groups/64 requests/zero external attempts; four groups directly exercise staged anonymous effects and current-row replay, while the broader native matrix still targets repository-auth handlers. Full staged negatives/lifecycle, current serving, n8n raw-body compatibility and release/recovery approval remain held. This is neither a re-gate nor a deployment.

**2026-09-06 HTTP source follow-up (held):** the [narrow shared adapter](../audits/2026-09-06-native-card-http-adapter.md) adds exact-marker terminal branches to both repository upserts, preserving their authorization bytes. Its 70 offline controls, including the independently identified empty-child-slot correction, do not establish serving anonymous compatibility. It is uninstalled, does not amend either n8n bypass and does not enable automatic creation. The SQL-only history below retains its original scope. Selected37 authenticated schema reconstruction now has its separate [local proof](../audits/2026-09-06-schema-v7-recovery.md); installed/live reconstruction remains unproven.
