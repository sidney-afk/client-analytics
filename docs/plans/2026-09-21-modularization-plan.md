...
- **2026-09-21: the split is complete.** All 34 numbered cuts landed; `999-remainder.html.part` is retired (deleted, dropped from `manifest.txt`) and `350-footer.html.part` carries the original tail through `</html>`. `src/index/manifest.txt` lists 37 content fragments. Each cut PR proved empty-diff against its own then-current main, never that one page hash held across the whole split — ordinary fixes (see the note above) changed the served bytes between cuts, which is why the Step 0 hash (`5990e9fc...`) differs from this final one. The final hash is `6b5f44936a5e5123cdeb343b778430be3d8680bb418b0a89a01046253bdf483d`, 5,776,430 bytes, matching `npm run check:index`'s three-way assembled/working-tree/committed comparison on the final commit.
