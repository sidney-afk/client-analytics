...
|  | **`110` (Time Off and weekly reports) changes bytes (+548 served).** It wrote 7 variables owned by `100`: the Time Off flag value, promise, channel and generation, and the staff overview, admin overview and quote generations. `100` now offers `_ptoStoreFlagValue`, `_ptoSetFlagPromise`, `_ptoSetFlagChannel`, `_ptoNextFlagGeneration`, `_ptoNextOverviewGeneration`, `_ptoNextAdminOverviewGeneration` and `_ptoNextQuoteGeneration`; `110`'s 10 writes call them at the same points with the same values. `110` imports 62 names and exports 40. It converts ahead of step 7, like `070` ahead of `080`: the 14 flag-state names `120` reassigns stay in `110` and the checker lists them as pending setters, which `120`'s PR (or step 7, moving them to `core-flags`) supplies; the move then only shortens `110`'s export list. |
|  | **`050` + `060` (market briefs and analytics profile; templates and filming) change bytes (+1,218 served), together as planned because each writes the other's state.** `050` wrote 8 of `040`'s analytics names (the two charts, three platform and mode picks, the client history, the sort column and direction), `060`'s nav pair and `260`'s client-entry capability. `060` wrote `040`'s two Submit project lists and 5 of `050`'s template names. New setters: `040` gains 10 (`_setGrowthChart`, `_setViewsChart`, `_setFollowersPlat`, `_setFollowersMode`, `_setViewsPlat`, `_setCurrentClientHistory`, `_setLinearProjects`, `_setLinearClientRows`, `_setSortCol`, `_setSortDir`); `050` gains 5 (`_templatesSetSelected`, `_templatesSetActiveTab`, `_templatesSetEditMode`, `_tplSetPinsEditMode`, `_tplSetPinSelectorOpen`); `260` gains `_syncviewSetClientEntryCapability`; `050` reuses `060`'s existing `_syncviewSetCurrentNav` and `_syncviewNextNavEpoch`. 32 writes became calls (16 in each fragment), same order and values. `050` imports 112 names and exports 53; `060` imports 74 and exports 48. Pending for others: `_templatesSelected` and `_templatesActiveTab` (owned by `050`), and `060`'s 7 onboarding-view names. |
|  | **`160` (Calendar organize UI) next, byte-identical (plan step 11).** It imports 105 names (from `040`, `050`, `070`, `100`, `120`, `125`, `130`, `131`, `133`, `134`, `140`, `150`, `170`, `180`, `190`, `210`, `260`, `270`) and exports the 63 that others use. It has no foreign writes; it reads the approval statuses only to label them. |
|  | **`100` (onboarding, staff sign-in and staff controls) changes bytes (+492 served).** It wrote 8 variables it does not own: 7 of `060`'s onboarding-view state, all in the sign-out purge (`_syncviewStaffPurgeSensitiveState`), and `260`'s `_syncviewAppBooted` boot flag, in `_syncviewLiftStaffGate`. `060` now offers `_obvSetSubs`, `_obvSetSelected`, `_obvSetError`, `_obvSetWarn`, `_obvSetLoadedAt`, `_obvSetLoading` and `_obvSetMode`; `260` offers `_syncviewSetAppBooted`; the 8 writes call them at the same points with the same values. `100` imports 61 names and exports 85. Pending for others: `OB_VARIANT`, `OB_DRAFT_KEY`, `OB_SUBID_KEY` (owned by `100`) and `060`'s `_obvMode`. |
|  | **`150` (Calendar hydration and spreadsheet import) changes bytes (+1,028 served).** It wrote 13 variables it does not own: 11 of `134`'s (the 9 import-wizard names, the last network load time and the pending background render flag), `130`'s `_calFocusRequestLoadFailed` and `140`'s `_calLinearMetaHydrated`. The owners now offer `_calSet<Name>` for each (`134` 11, `130` 1, `140` 1; `140` itself is not converted, it only gains the one-line setter); `150`'s 18 writes call them at the same points with the same values. `150` imports 113 names and exports 29. Pending for others: `_calLastLocalWriteAt`, `_calLoadSeq`, `_calBgLoadInFlight` (owned by `150`). |
|  | **`180` (Calendar native posts, Create Post dialog, captions and media) changes bytes (+894 served).** It wrote 11 variables it does not own: 8 of `120`'s caption-prompt and caption-job state, 2 of `170`'s Create Post dialog state and `150`'s `_calLastLocalWriteAt`. The owners now offer `_calSet<Name>` for each (`120` 8, `170` 2, `150` 1; `120` and `170` are approve-path fragments and are not converted, they only gain the one-line setters); `180`'s 18 writes call them at the same points with the same values. `180` imports 138 names and exports 41. Pending for others: `150`'s `_calLastLocalWriteAt` (still written by a classic fragment). |
|  | **`260` split, part 1 of 9: `040`'s names (+828 served).** At the owner's request `260` converts in small PRs, one per owning file, and stays a classic script until the last one marks it a module. This part: all 15 of `260`'s writes to `040`'s names, every one inside `_syncviewPurgeClientEntrySurface` (client-link entry: the purge that clears analytics state for an invalid link or on pagehide). `040` gains 12 setters (`_setAllData`, `_setClientMap`, `_setTopVideos`, `_setBriefs`, `_setMrBriefs`, `_setClientViewTab`, `_setContentSummaryState`, `_setTabSummaryCache`, `_setFetchExtrasPromise`, `_nextFetchExtrasAttempt`, `_setFetchExtrasState`, `_setClientEssentialsLoad`) and reuses 3 from the `050` PR (`_setGrowthChart`, `_setViewsChart`, `_setCurrentClientHistory`); the 15 writes call them in the same order with the same values. |
|  | **`260` split, part 2 of 9: `050`'s names (+12 served).** `260`'s 5 writes to `050`'s Templates selection and tab, all in `init()`'s boot routing (restoring `#templates/<client>` from the hash and history state, then canonicalising or dropping it once data has loaded), now call `050`'s existing `_templatesSetSelected` and `_templatesSetActiveTab` (from the `050` PR). No new setter; `050` has no pending setters left. |
|  | **`260` split, part 3 of 9: `100`'s names (+184 served).** `260`'s 3 writes to `100`'s onboarding funnel variant and its two storage keys, all in `init()`'s boot routing (the `?onboarding` branch that picks the AI or standard funnel before the form mounts), now call 3 new one-line setters in `100` (`_obSetVariant`, `_obSetDraftKey`, `_obSetSubIdKey`). `OB_DRAFT_KEY` and `OB_SUBID_KEY` are no longer exported. | |
|  | **`260` split, part 4 of 9: `134`'s names (+92 served).** `260`'s 1 write to `134`'s `_linearOutboxFlushTimer`, in `_writeUiCancelClientLegacyRetryTimers` (client-link entry: it runs first in the purge and cancels the legacy Linear retry timer on a client link), now calls a new one-line setter in `134` (`_calSetLinearOutboxFlushTimer`). `134` stays a classic script; its approve-path writer `140` is untouched. | |
|  | **`260` split, part 5 of 9: `150`'s names (+61 served).** `260`'s 1 write to `150`'s `_calLoadSeq`, in `_syncviewPurgeClientEntrySurface` (client-link entry: bumping the sequence retires any Calendar load still in flight before the Calendar state is cleared), now calls a new one-line helper in `150` (`_calNextLoadSeq`, returning `++_calLoadSeq`). `150` no longer exports `_calLoadSeq`. | |
|  | **`260` split, part 6 of 9: `200`'s names (+76 served).** `260`'s 1 write to `200`'s `_kasperUnlocked`, at the top of `init()` (boot routing: when the session carries the Kasper unlock flag it reveals the Kasper tab, and the `#kasper` hash and saved-nav restores then route there), now calls a new one-line setter in `200` (`_setKasperUnlocked`). | |
|  | **`260` split, part 7 of 9: `270`'s names (+61 served).** `260`'s 1 write to `270`'s `_sxrLoadSeq`, in `_syncviewPurgeClientEntrySurface` (client-link entry: bumping the sequence retires any Samples load still in flight before the Samples state is cleared), now calls a new one-line helper in `270` (`_sxrNextLoadSeq`, returning `++_sxrLoadSeq`). `270` is an approve-path fragment: it gains only this helper and stays a classic script, unconverted. | |
|  | **`260` split, part 8 of 9: `280`'s names (+82 served).** `260`'s 1 write to `280`'s `_sxrLinearOutboxTimer`, in `_writeUiCancelClientLegacyRetryTimers` (client-link entry: it runs first in the purge and cancels the legacy Samples Linear retry timer on a client link), now calls a new one-line setter in `280` (`_sxrSetLinearOutboxTimer`). `280` is an approve-path fragment: it gains only this setter and stays a classic script, unconverted. | |
|  | **`260` split, part 9 of 9: `260` becomes a module (+0 served).** After parts 1 to 8 moved every one of `260`'s writes to another file's names behind that file's setter (15 to `040`, 5 to `050`, 3 to `100`, 1 each to `134`, `150`, `200`, `270`, `280`), `260` is annotated: 212 imports from 24 fragments and 27 exports, which the other modules now import instead of reading as globals. The served page is byte-identical, because the header and footer are stripped when served. | |
|  | **`131` (core HTML escaping) becomes a module, byte-identical (+0 served).** It imports nothing and exports its 3 helpers (`_calEsc`, `_calEscAttr`, `_jsAttrArg`). No file writes to its names, so it needs no setter. | |
|  | **`132` (Calendar dates and ids) becomes a module, byte-identical (+0 served).** It imports nothing and exports its 7 date and id helpers (`_calIsoOf`, `_calIsoFirstOfMonth`, `_calParseIso`, `_calIsoMondayOf`, `_calAddDaysISO`, `_calAddMonthsISO`, `_calMintId`). No file writes to its names, so it needs no setter. | |
|  | **`133` (core loading skeletons) becomes a module, byte-identical (+0 served).** It imports 4 names (`_calEsc` and `_calEscAttr` from `131`, one from `130`, one from `070`) and exports its 3 skeleton builders (`_svSkel`, `_svLoadingSkeletonHtml`, `_calLoaderHtml`). No file writes to its names, so it needs no setter. | |
|  | **`040` (shared UI, briefs and analytics state) becomes a module (+4 served).** Its first line, the main script's opening `<script>` tag, moves to the end of `030`, so the assembled page keeps the same bytes there and `040` parses as a module. `040`'s one write to another file's name, `_syncviewClientEntryCapability` in `setClientViewTab` (client-link entry: recording the tab a verified client link switched to), now calls `260`'s existing `_syncviewSetClientEntryCapability` (from the `050`+`060` PR). It imports 33 names and exports 126. | |
|  | **`310` (Kasper Sales Intake and Hiring Process) becomes a module, byte-identical (+0 served).** It writes no other file's names, so it needs no setter. It imports 7 names (from `100`, `120`, `131`, `320`) and exports 6 (`_hpLoadList`, `_hpPaint`, `_hpPurgeSensitiveState`, `mountSalesIntakeView`, `renderHiringProcessView`, `renderSalesIntakeView`). | |
|  | **`220` (production attribution and views) becomes a module (+68 served).** It wrote 1 variable it does not own, `250`'s `_prodSubPop` (the open submenu), in 2 places: the submenu opener and `_prodClearLayer`. `250` now offers `_prodSetSubPop(value)` and no longer exports `_prodSubPop`; the 2 writes call it at the same points with the same values. Neither touches boot routing or client-link entry. `220` imports 76 names from 7 fragments and exports 72. | |
|  | **`130` (Calendar model and cache) becomes a module (+76 served).** It imports 35 names and exports 69. Its one write to another file's name, `_calOpenStatusMenu` in `_calCapPlatMenu` (the caption-platform menu recording itself as the open status menu), now calls a new one-line setter in `170` (`_calSetOpenStatusMenu`). `170` is an approve-path fragment: it gains only this setter and stays a classic script, unconverted. Neither touches client-link entry or boot routing. | |
|  | **`134` (Calendar prefs and mount) becomes a module (+76 served).** It imports 44 names and exports 69. Its one write to another file's name, `_calBgLoadInFlight` in `_calClearSuspendedLoadOwnership` (the staff Calendar pagehide reset), now calls a new one-line setter in `150` (`_calSetBgLoadInFlight`). Four of `134`'s own names are still written by the approve-path fragments `140` (the Linear outbox flush timer, promise and owner key) and `290` (`_calPendingBackgroundRender`); those move behind setters when `140` and `290` convert. Neither write touches client-link entry or boot routing. | |
|  | **`120` (Calendar runtime flags, write routing and repair) becomes a module (+1,345 served). Approve-path.** It imports 69 names and exports 118. Its 29 writes to 14 names owned by `110` (the Calendar upsert, write reroute, client-comment gateway and Settings runtime flags: their allowlists, read promises, realtime client and channels, the reroute read generation and its failed / roster-unusable marks) now call 14 new one-line setters in `110`, next to each declaration. The two generation increments become `_writeUiSetRerouteFlagGeneration(_writeUiRerouteFlagGeneration + 1)`. These flags are read at boot and on client links to pick each write's lane; which screen a link opens is unchanged. `_writeUiLastRenderedAuthoritySig` (owned here) is still written by `290` and gets its setter when `290` converts. Four test harnesses that rebuild these functions in a sandbox now load the setters too. | | |
|  | **`270` (Samples model) becomes a module (+178 served). Approve-path.** It imports 102 names and exports 58. Its 4 writes to 2 names owned by `280` (`_sxrPendingBackgroundRender`, set when a Samples load, a link fill or a link adoption lands while the user is busy, and `_sxrLastLocalWriteAt`, the self-echo stamp a link fill sets) now call 2 new one-line setters in `280` (`_sxrSetPendingBackgroundRender`, `_sxrSetLastLocalWriteAt`). `280` is approve-path and gains only these two lines; it stays a classic script until its own PR. Neither write touches client-link entry or boot routing. | | |
|  | **`280` (Samples cards and notes) becomes a module (+0 served). Approve-path.** It imports 138 names and exports 60. It writes no other file's names, so it needs no setter. Two of its own names (`_sxrPendingBackgroundRender`, `_sxrLastLocalWriteAt`) are still written by `290`, which will call the setters `280` already offers (added in the `270` PR) when `290` converts. Nothing here touches client-link entry or boot routing. | | |
