...
|  | | | **`260` split, part 3 of 9: `100`'s names (+178 served).** `260`'s 3 writes to `100`'s onboarding funnel variant and its two storage keys, all in `init()`'s boot routing (the `?onboarding` branch that picks the AI or standard funnel before the form mounts), now call 3 new one-line setters in `100` (`_obSetVariant`, `_obSetDraftKey`, `_obSetSubIdKey`). `OB_DRAFT_KEY` and `OB_SUBID_KEY` are no longer exported. | |
