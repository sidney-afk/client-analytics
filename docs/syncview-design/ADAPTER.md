...
- `due`: compact written display date ("Jul 15") for every year; hovers carry the full form with year for non-current years via `_prodFmtDateFull` (owner decision 2026-07-18: no two-line wrap in date columns).
...
- Link card: `prodDescLinkPop` (`.prod-linkpop`, artifact `#descLinkPop` / `.linkpop`) is mounted on `document.body` by `_prodDescLinkPopEnsure()` the first time a description link is hovered, at z-index `10000` beside the toast and under the tooltip. It carries no `title`/`data-tip`, so the host tooltip binder has nothing to claim on it.
