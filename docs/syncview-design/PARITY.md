...
Cycle 48 was local-only scope hardening. `pixel-wired.js` now opens My Issues and fails if broader live Linear tabs or manual-create chrome return while that owner decision remains unresolved; no issue/sub-issue data model or render relationships changed.

Cycle 49 was local-only scope hardening. `pixel-wired.js` now opens an issue detail and fails if broader live Linear toolbar controls return while that owner decision remains unresolved; no issue/sub-issue data model or render relationships changed.

Cycle 50 was local-only scope hardening. `pixel-wired.js` now opens the row-context Project submenu and fails if live Linear's `No project` detach row returns while that owner decision remains unresolved; no issue/sub-issue data model or render relationships changed.
...
Cycle 53 attempted a read-only live Linear probe through the saved probe profile. The browser reached Linear's desktop-app interstitial, then the login screen after "Open here instead"; no issue list or issue links were visible, no mutating path was touched, and no code changed.
