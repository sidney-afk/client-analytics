...
| GitHub polish workflow and issue intake | `.github/workflows/production-polish-gate.yml`, `.github/ISSUE_TEMPLATE/production-polish.yml`, `.github/pull_request_template.md`, `AGENTS.md`, `.github/copilot-instructions.md` | ported | Production UI PRs that touch the relevant files run the polish gate in GitHub Actions; the gate also runs on `main` at 09:17 UTC Monday through Friday after merge and cancels superseded runs for the same ref. Future rough owner feedback has an AI-ready issue template, PR checklist, and agent instructions. The workflow also has a guarded Argos upload hook that stays skipped until `ARGOS_TOKEN` is configured. |
