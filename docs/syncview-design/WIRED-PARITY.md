...
| Parent/sub-issue creation | ✅ F203 live | Admin/SMM may create one Production-only root or validated child through `production-write`. The request carries roster client, team, title, exact Markdown description, status, full-year due date, same-team assignee, and complete real labels; deterministic receipts recover response loss. No Calendar/Samples card is created, chosen, or linked. Migration `2026-07-23-f203-production-issue-create.sql` applied to production 2026-07-24 and `production-write` redeployed from `1738ad3` the same day (PR #931 Slice 4 reland); the create-form UI is in live `index.html`. TEST drills still owed. |
