...
| Production polish gate | owner automation request after PR #764 | ported | `npm run test:prod-polish` now runs boot, structure, interaction inventory, accessibility/focus, layout clipping, behavior, and pixel parity suites as one PR-ready gate. |
...
| GitHub polish workflow and issue intake | `.github/workflows/production-polish-gate.yml`, `.github/ISSUE_TEMPLATE/production-polish.yml`, `.github/pull_request_template.md`, `AGENTS.md`, `.github/copilot-instructions.md` | ported | Production UI PRs that touch the relevant files run the polish gate in GitHub Actions; the gate also runs on `main` at 09:17 UTC Monday through Friday after merge and cancels superseded runs for the same ref. Future rough owner feedback has an AI-ready issue template, PR checklist, and agent instructions. The workflow also has a guarded Argos upload that validates the review packet, exports desktop Production PNGs with metadata to `.codex-tmp/prod-argos-snapshots`, and stays skipped until `ARGOS_TOKEN` is configured. |
...
6. Empty project-board columns are static lanes: they keep title/count, collapse, and empty copy, but do not show fake add/options controls. Populated columns retain the guarded read-only controls, and the review packet records empty-column action-control evidence.
7. Issue-list and project-detail group headers no longer show no-op add buttons in the read-only preview. Detail pages still keep the guarded add-sub-issue affordance where it explains hierarchy, and populated board columns still keep guarded add/options controls where card context exists. The review packet records zero group-header add controls for desktop list and project-detail screenshots.
