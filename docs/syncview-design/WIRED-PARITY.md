...
| Reviewer visual packet | `prod-review-packet.js` | ported | GitHub Actions now uploads a compact `production-review-packet` artifact with a manifest plus named desktop, dark, and mobile screenshots for human review. |
| GitHub polish workflow and issue intake | `.github/workflows/production-polish-gate.yml`, `.github/ISSUE_TEMPLATE/production-polish.yml`, `AGENTS.md`, `.github/copilot-instructions.md` | ported | Production UI PRs that touch the relevant files run the polish gate in GitHub Actions; future rough owner feedback has an AI-ready issue template and agent instructions. |
...
14. `npm run test:prod-review` generates a compact screenshot packet and manifest for reviewer inspection; the workflow uploads it as `production-review-packet`.
