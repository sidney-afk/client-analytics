...
| `prod-polish-gate.js` | Production polish lane runner. With no argument it runs the complete boot, structure, zero-write smoke, interaction, accessibility/focus, layout, behavior, and pixel set. CI uses `--lane=fast`, `--lane=interaction`, and `--lane=heavy` to preserve the same assertions while shortening PR wall-time. | `npm run test:prod-polish` |
