...
| `prod-comments-browser.js` | Fully intercepted comment-read contract: staff auth, newest/older paging, refresh races, merge/escaping/visibility/error states, one-request 1,000-row canonical completion, deployed-reader fallback, bounded transport retry, malformed/overflow/wrong-audience sabotage, and a fail-closed writer. Its POST is a read operation to the mocked `production-comments` function. | `node docs/syncview-design/tests/prod-comments-browser.js` |
...
| `prod-structure-subset.js` | Read-only structural subset adapted from `behav.js`/`sweep.js`. Runs against the wired `index.html?prod=1` tab and verifies the SyncView artifact sidebar tree, Production boot skeleton wiring, grouped list rows, status glyphs, centered detail cards, dormant activity-renderer style structure (not runtime event loading; F138), child `Sub-issue of` context, title-first sub-issue rows, project toolbar order, project Display grouping/show-sub-issues behavior, removed workspace menu, searchable selected-issue Actions menu, team-scoped Projects board, disabled mutating controls, settled read/console health, and no write-like browser requests. | `node docs/syncview-design/tests/prod-structure-subset.js` |
| `behav-wired.js` | Authority-aware guard-mode interaction corpus. Its reset waits for loaded, non-error state and uses an All-tab Video/Graphics fallback only after an independent explicit-status count proves the default Video/Active fixture has zero owner-active rows. Eligible rows hidden by rendering fail immediately; missing/error/loading baselines fail with bounded diagnostics. Mutation-only behaviors remain covered by the fully mocked gateway lane. | `node docs/syncview-design/tests/behav-wired.js` (168/168 in the F105 candidate) |
| `prod-interaction-inventory.js` | Broad live-read/zero-mutation surface inventory. Locked row status/due/assignee checks select one exact non-TEST row, assert the current zero-picker authority guard, and never confuse the bounded active-TEST write override with a locked fixture. `--selection-only` runs that focused contract. | `node docs/syncview-design/tests/prod-interaction-inventory.js` |
| `prod-a11y-focus.js` | Accessibility/focus polish gate. Runs scoped axe-core checks, verifies visible controls have names/types, focused icon controls activate with Enter/Space, Escape closes transient chrome, keyboard row navigation still works, settled read/console health stays clean, and no write occurs. | `node docs/syncview-design/tests/prod-a11y-focus.js` |
| `prod-layout-polish.js` | Layout clipping gate across desktop, compact desktop, and mobile widths. Checks row/card metadata, filter pills, selected project cards, the current inline project-parent breadcrumb, project details, floating chrome, settled read/console health, and zero writes. | `node docs/syncview-design/tests/prod-layout-polish.js` |
...
| `prod-test-utils.js` | Shared module for the wired suites: static server, Production init, write-like request detection, and F105's fail-closed read/console audit. It accepts only exact method+URL network/429/5xx recovery to 2xx/304; each generic resource-console error additionally needs a one-to-one exact URL/time match. It proves old-document navigation aborts separately, drains outstanding reads, sanitizes diagnostics, and keeps non-retryable/persistent/pending/unmatched failures red. | Covered offline by `node test/prod-read-console-audit.js`; exercised by structure, accessibility, layout, and behavior |
...
`pixel-wired.js`. F105 made the contracts explicit about locked non-TEST authority, current inline
project-parent layout, legitimately empty active-team data, and exact recovered-read correlation.
Preserve those behaviors, the pending/unmatched-read failures, and the post-settle zero-live-mutation
audit whenever these suites change. The prototype originals remain historical references; they are not authority to
