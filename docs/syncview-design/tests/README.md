...
| `prod-polish-gate.js` | Aggregate runner for boot, structure, locked live-read/zero-mutation smoke, comment reads, fully mocked write gateway, interaction, accessibility/focus, layout, behavior, and pixels. CI splits `fast`/`interaction`/`heavy`; only `fast` runs automatically on PRs. F105 repaired the post-#813 interaction/heavy test epoch. Locked lanes may read live data; no lane may send a live mutation. | `npm run test:prod-polish` |
...
| `behav-wired.js` | Authority-aware guard-mode interaction corpus. Its reset waits for a loaded, non-error list and, when the default active-team fixture is legitimately empty, selects an existing Video/Graphics row on the All tab before testing controls. It fails closed with bounded diagnostics when no usable baseline exists. Mutation-only behaviors remain covered by the fully mocked gateway lane. | `node docs/syncview-design/tests/behav-wired.js` (168/168 in the F105 candidate) |
...
`pixel-wired.js`. F105 made the interaction/behavior contracts explicit about locked non-TEST
authority and about legitimately empty active-team data. Preserve that exact-fixture behavior and
the zero-live-mutation audit whenever these suites change. The prototype originals remain historical references; they are not authority to
