...
| `behav-wired.js` | Primary wired-tab interaction coverage in guard mode. Drives list, board, project detail, issue detail, menus, pickers, palette, keyboard, selection, hover/right-click, history, scroll, and zero-write checks. Mutation-only Linear behaviors stay listed as `deferred-B3`. | `node docs/syncview-design/tests/behav-wired.js` |
| `prod-interaction-inventory.js` | Finished-read-only inventory sweep. Samples unique visible controls across list, selected list, filtered empty, detail, board, selected board, and project states; checks right-click menus, hover tips, row open/checkbox/status/due/assignee/client-chip pointer controls, sub-issue scroll reset, breadcrumb context, project-card cursor, no browser errors, and no write-like requests. | `node docs/syncview-design/tests/prod-interaction-inventory.js` |
