...
async function runTheme(port, browser, theme) {
  ...
  try {
    ...
    await artifact.waitForSelector('#layer .pop [data-subtoggle]');
    await wired.waitForSelector('#prodLayer .prod-pop [data-prod-show-subissues]');
    await requirePair(gaps, 'display menu inventory', artifact, wired, '#layer .pop [data-subtoggle]', '#prodLayer .prod-pop [data-prod-show-subissues]');
    await requirePair(gaps, 'display menu inventory', artifact, wired, '#layer .pop [data-order="due"]', '#prodLayer .prod-pop [data-prod-order="due"]');
    await requirePair(gaps, 'display menu inventory', artifact, wired, '#layer .pop [data-order="updated"]', '#prodLayer .prod-pop [data-prod-order="updated"]');
    await requirePair(gaps, 'display menu inventory', artifact, wired, '#layer .pop [data-order="created"]', '#prodLayer .prod-pop [data-prod-order="created"]');
    compareMenuInventory(gaps, 'display menu inventory', await pickerInventory(artifact, '#layer .pop .mi'), await pickerInventory(wired, '#prodLayer .prod-pop .prod-mi'));
    await compareStyles(gaps, 'display menu Show sub-issues', artifact, wired, '#layer .pop [data-subtoggle]', '#prodLayer .prod-pop [data-prod-show-subissues]', ['height', 'paddingLeft', 'paddingRight', 'display', 'alignItems', 'cursor', 'color']);
    await shotElement(artifact, '#layer .pop', 'artifact-crop-display-menu');
    await shotElement(wired, '#prodLayer .prod-pop', 'wired-crop-display-menu');
