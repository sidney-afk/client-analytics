...
function writeManifest(shots) {
  const lines = [
    ...
    '',
    'These screenshots are PR review evidence for the read-only `?prod=1` Production surface. They are regenerated on demand and are not committed. Open `index.html` in the artifact for a browsable gallery, or `review-manifest.json` for machine-readable metadata.',
    '',
...
(async () => {
  ...
  try {
    ...
    writeManifest(shots);
    writeGallery(shots);
    console.log(`prod-review-packet: wrote ${shots.length} screenshots, JSON manifest, Markdown manifest, and gallery to ${outDir}`);
  } finally {
