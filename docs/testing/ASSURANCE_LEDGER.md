...
| Samples/SXR + Kasper approval flow | 2026-07-16 (nightly, partial) / 2026-07-11 (last fully green master) | Samples E2E nightly 07-16: scenarios 11/12, tree 23/24, probes lane skipped on failure; production-preview-smoke job green. `realtime_parity.js` 11/11 re-green 07-17 (this run) | **fail-open-defects** | **F73 (P1, confirmed on `main` 07-17 at source level): fresh-browser deep link/reload to `#sample-reviews` breaks — fix + reworked guard shipped this run as draft PR `claude/f73-sxr-boot-gate-ga`.** **F140 (new, this run): Kasper Finish stays disabled after a graphic tweak request — deterministic nightly failure since 07-13, classification (intended vs regression) owner-gated.** **Samples nightly red 4 consecutive nights, untriaged (F140/F141).** F75 doc-rewrite residue open | FRESH data + open P1s → worked this run (cycle 2) | 8+P1 |
...
- **O-6 (F140):** Classify the Kasper Finish behavior — is Finish MEANT to stay disabled after a graphic tweak request? (If yes, the nightly tree lane gets updated; if no, it's a regression to fix.)
- **O-7 (F141):** Approve a TEST-client drag-reorder drill next run to classify the persist failure (silent-loss risk).
- **O-8 (process):** The samples nightly was red 4 nights with no consumer — approve wiring nightly failures to an owner-visible alert (or a standing triage step).
...
- **Recommended next run:** 2026-07-24 (T0 window) or immediately after the owner answers O-5/O-6/O-7 — with TEST drills allowed, so the reorder repro, the `?c=` client-link probe (O-3), and a real browser share (O-2) can close.
