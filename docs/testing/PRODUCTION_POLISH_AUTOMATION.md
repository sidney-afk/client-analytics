...
`.github/workflows/production-polish-gate.yml` keeps pull requests on the fast subset by running boot, structure, the explicit locked-state live-read/zero-mutation smoke, mocked comment reads, the fully mocked write-gateway lane, accessibility/focus, and layout. The longer interaction inventory, `behav-wired`, pixel parity, review packet, validation, and local Argos-format export run only after relevant pushes to `main`, on weekday schedule, and on manual dispatch—not on pull requests. F105 repaired the post-#813 test epoch: locked row controls are asserted on one exact non-TEST row, the bounded TEST override keeps its writable contract, the layout check selects a real child and follows the owner-ratified inline parent trail, and the behavior reset uses a fallback only after loaded state plus an independent owner-active row count prove the default fixture is genuinely empty. The shared read/console audit permits only exact method+URL eligible network/429/5xx recovery to 2xx/304; each generic resource-console error additionally needs a one-to-one exact URL/time match. Non-retryable, persistent, pending, unrelated, and unproven-abort failures stay red. Mutation requests are checked after the bounded settle, so late traffic cannot bypass the zero-live-mutation invariant. A green fast job alone is not aggregate readiness; run the complete aggregate on the exact candidate and have the cloud reviewer verify the manual long-lane run before merge. A Playwright-version-keyed cache reuses Chromium and skips the apt install on cache hits. The workflow cancels superseded in-progress runs for the same ref so repeated pushes do not leave stale browser jobs queued.
