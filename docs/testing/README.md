...
| **Graphics F2 hard pre-flight** | `test/graphics-f2-preflight.js` offline plus its disposable PostgreSQL 17 lane; manual production execution is `.github/workflows/graphics-f2-preflight.yml` | Production execution may use only GitHub Actions reads plus the existing dedicated F2 PostgreSQL role in one repeatable-read/read-only snapshot. It never changes a flag or row. A red queue result is bounded to `id`/`team`/`status`; no payload, client, provider ID, credential, or artifact is emitted. | `docs/ops/FLIP_RUNBOOK.md` F2 machine gate |
...
| **Visible boot sequence** | `qa/boot/` | Fully intercepted fictional data only. Streams the real document so the static first frame, verifier transition, route loader, settled view, reload, and history traversal are observed rather than inferred from a settled screenshot. One attempt per navigation; every unmocked request or browser error is fatal. | `qa/boot/README.md` |
...
|---|---|---|
| `node qa/workload-consistency/run.js` | Offline cross-view comparator self-tests and extracted source helpers; `--contracts` separately asserts native-independent behavior and retains nonzero exits for product failures | Bounded Workload consistency investigation; [contract and evidence limits](../../qa/workload-consistency/README.md). No live lane or automatic scheduling. |
...
| `node test/graphics-f2-preflight.js` | Offline source/logic proof for exact pre-receipt/binder/current-main binding, current F1/F2 state, all-team/both-parity production residue selection, fixed bounded output, five-minute scheduled clear air, exhaustive latest-attempt/active drainer refusal, and manual-only workflow wiring. The Graphics F2 hosted proof reruns it against disposable PostgreSQL 17 and sabotages RLS visibility. | Any F2 pre-flight, evidence-lane, outbound-drainer, or flip-runbook change |
