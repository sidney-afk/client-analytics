...
`node test/assurance-ledger-staleness.js` uses a unique child of the operating
system's temporary directory for each CLI fixture and removes that child in
`finally`; it does not require a global `/tmp` directory.
