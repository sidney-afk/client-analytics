...
| `npm run test:prod-polish` | Aggregate Production gate: boot, structure, locked live-read/zero-mutation smoke, comment reads, fully mocked write gateway, interactions, a11y, layout, behavior, pixels. F105 repaired the post-#813 interaction/heavy test epoch without weakening zero-live-mutation coverage; the fast PR subset alone is not a go-live pass. | Any Production-tab change; must be green on the exact candidate before go-live |
