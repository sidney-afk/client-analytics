...
-- The compute-on-read views run with the caller's existing Supabase service-role
-- source-table and extensions privileges. This delta grants only the new pure
-- helper EXECUTEs and view reads; it does not widen an existing source ACL.
