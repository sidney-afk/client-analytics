-- B3 exact-scope Calendar graphics linkage repair.
--
-- SOURCE ONLY / NOT INSTALLED BY THIS FILE. Installing these RPCs is a
-- separate owner-approved operation. The forward RPC can update only
...
begin
  if jsonb_typeof(p_plan) <> 'object'
     or p_plan->>'contract' is distinct from 'b3-scoped-card-linkage/v1'
     or p_plan->>'scope_policy' is distinct from 'exact_calendar_graphic_url_to_native_v1'
  ...
  if exists (
    ...
    where not (c.card ?& array[
       ...
       or coalesce(c.card->'graphic_deliverable_id'->>'state', '')
            not in ('null', 'empty')
       or (
         c.card->'graphic_deliverable_id'->>'state' = 'null'
         and jsonb_typeof(c.card->'graphic_deliverable_id'->'value') <> 'null'
       )
       or (
         c.card->'graphic_deliverable_id'->>'state' = 'empty'
         and (
           jsonb_typeof(c.card->'graphic_deliverable_id'->'value') <> 'string'
           or c.card->'graphic_deliverable_id'->>'value' <> ''
         )
       )
  ...
  join (
    select distinct e.value->'card'->>'client' as client
    from jsonb_array_elements(v_entries) e(value)
  ) requested on requested.client = cl.slug
  order by cl.slug
  for share of cl;
...
begin
  ...
  where c.client = requested.client
    ...
    and (
      (requested.pointer_state = 'null' and c.graphic_deliverable_id is null)
      or (requested.pointer_state = 'empty' and c.graphic_deliverable_id = '')
    );
...
-- and restores each pointer to its original NULL-versus-empty state.
...
begin
  ...
  select encode(extensions.digest(convert_to(
    'b3-scoped-card-linkage-rollback/v1' || E'\n'
  ...
  update public.calendar_posts c
  set graphic_deliverable_id = case requested.pointer_state
    when 'null' then null
    when 'empty' then ''
  end
  from (
