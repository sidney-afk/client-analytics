-- Write-UI epoch: atomic append-to-existing-batch intake (v3, 2026-08-19).
--
-- SUPERSEDES migrations/2026-08-18-production-intake-append-v2.sql, which IS
-- applied and working -- v3 changes exactly ONE rule.
--
-- v2 required the batch-create dependency's team to equal the appended row's
-- team. The gateway does not work that way and deliberately so: it resolves
-- ONE parent route for the batch and shares it across every team on the card,
-- because the whole batch has to hang under a single parent issue
-- (production-write index.ts, parentRouteByTeam / ownsDistinctParent). The
-- native create flow makes ONE Linear issue serving both teams, so the
-- graphics half of a Video+Thumbnail append arrives carrying the VIDEO
-- dependency -- and v2 refused it as batch_parent_mapping_missing.
--
-- That refused a thumbnail append to EVERY batch the native flow created.
-- Reproduced against PostgreSQL 16 with the live function and the real batch
-- shape before this fix was written, and re-run after it to prove the repair.
--
-- A differing dependency team is accepted ONLY when both teams resolve to the
-- identical single parent issue. A batch with genuinely distinct per-team
-- parents still requires its own team's dependency, exactly as before.
--
-- Additive only: replaces the two functions, touches no table and no data.
  ...
  loop
    ...
    if v_parent_id is not null then
      ...
      if not found
         ...
         -- v3: the gateway resolves ONE parent route per batch and shares it
         -- across every team on the card, because a batch must hang under a
         -- single parent issue. So a graphics row legitimately arrives
         -- carrying the VIDEO batch-create dependency, and demanding the
         -- dependency's team equal the row's team refused every
         -- Video+Thumbnail append to such a batch. A differing team is
         -- accepted only when both teams resolve to the identical single
         -- parent issue -- i.e. one issue really does serve both.
         or (v_dependency.team is distinct from v_team
             and not (
               cardinality(public.production_batch_parent_ids_for_team(
                 v_batch.linear_parent_ids, v_team)) = 1
               and public.production_batch_parent_ids_for_team(
                     v_batch.linear_parent_ids, v_team)
                   = public.production_batch_parent_ids_for_team(
                     v_batch.linear_parent_ids, v_dependency.team)
             ))
