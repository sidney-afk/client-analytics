-- Write-UI epoch: atomic append-to-existing-batch intake (v4, 2026-08-19).
--
-- SUPERSEDES migrations/2026-08-19-production-intake-append-v3.sql (applied).
--
-- v3 fixed one comparison and left its two siblings. The gateway resolves ONE
-- parent route per batch and shares it across every team on the card, so the
-- graphics half of a Video+Thumbnail append arrives carrying the VIDEO
-- batch-create dependency. That dependency describes its own lane: team
-- video, legacy_parity true (video is Linear-authoritative), video project.
-- The appended graphics row runs parity FALSE post-flip -- read off the live
-- outbox rows -- so v3's parity equality refused the same appends v2's team
-- equality had, and the project equality was next in line for any client
-- whose per-team projects differ.
--
-- Reproduced against PostgreSQL 16 with the live v3 function and the real
-- parity values before this fix was written: both the same-project and
-- distinct-project shared-parent shapes refuse under v3 and complete under
-- v4, and a same-team mismatch still refuses.
--
-- All three per-lane comparisons waive together, and only under proof: both
-- teams resolve to the identical single parent issue. Additive only:
-- replaces the one function, touches no table and no data.
