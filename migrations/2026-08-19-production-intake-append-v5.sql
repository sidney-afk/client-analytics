-- Write-UI epoch: atomic append-to-existing-batch intake (v5, 2026-08-19).
--
-- SUPERSEDES migrations/2026-08-19-production-intake-append-v4.sql (applied).
--
-- WHAT CHANGES: exactly one condition. v4 pinned every appended row to
-- `origin = 'calendar'`, which is the same shape of hard pin that refused
-- Doug's appends for three rounds -- it encoded "the only thing that exists is
-- the calendar" into a validation rule. Samples native create makes that false.
--
-- The pin becomes an AGREEMENT check between the row's origin and the batch's
-- purpose. A samples row may append only to a purpose='samples' batch, and a
-- calendar row only to a calendar batch. That is strictly narrower than a
-- fixed value in one direction (it now also refuses a calendar row appended to
-- a samples batch, which v4 would have allowed) and wider in exactly one
-- intended direction (samples into samples). An origin that is neither word is
-- refused outright, so widening cannot become an open door.
--
-- REQUIRES migrations/2026-08-19-samples-batch-purpose.sql (the column) --
-- v_batch is a public.batches%rowtype, so v_batch.purpose does not resolve
-- without it and this function will not compile.
--
-- Nothing else in the function is touched: the shared-parent waiver from v4,
-- the parity and project comparisons, the ordinal and title rules and every
-- dedup lock are byte-identical to the applied v4 definition, which this file
-- was derived from mechanically.
--
-- Compiled against a disposable PostgreSQL 16 before handover, and the new
-- condition proven across the full four-way origin/purpose matrix (house rule
-- since the v2 CASE defect: no migration is handed over unexecuted).
