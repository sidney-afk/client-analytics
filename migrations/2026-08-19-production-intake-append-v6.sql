-- Write-UI epoch: atomic append-to-existing-batch intake (v6, 2026-08-19).
--
-- SUPERSEDES migrations/2026-08-19-production-intake-append-v5.sql (applied).
--
-- WHAT CHANGES: title flavour only. A samples batch titles its children
-- 'Sample Video N' / 'Sample Thumbnail N' (owner ruling 2026-08-19 -- the
-- parent and children must say they are samples), so:
--   1. the expected-title comparison prefixes 'Sample ' when the batch's
--      purpose is 'samples' -- derived from the SAME column the v5 origin
--      agreement checks, so a title can never disagree with its batch; and
--   2. the base-ordinal count accepts the optional prefix in BOTH kinds of
--      batch. That is deliberate: the first live samples batch predates the
--      ruling and its children read 'Video 1' / 'Thumbnail 1'; a strict
--      per-purpose count would restart its numbering at 1. A calendar batch
--      never contains 'Sample ' titles, so the wider count changes nothing
--      for it.
--
-- Everything else -- the origin/purpose agreement, the shared-parent waiver,
-- the parity and project comparisons, every dedup lock -- is byte-identical
-- to the applied v5, which this file was derived from mechanically.
--
-- Compiled against a disposable PostgreSQL 16 before handover, and the title
-- matrix proven on fixtures for BOTH batch purposes (house rule).
