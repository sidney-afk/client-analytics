-- Write-UI epoch: atomic append-to-existing-batch intake (v7, 2026-08-26).
--
-- SUPERSEDES migrations/2026-08-19-production-intake-append-v6.sql (applied).
--
-- WHAT CHANGES: exactly one condition is REMOVED --
--
--     or (v_batch.team is not null and v_team is distinct from v_batch.team)
--
-- and nothing else in either function is touched.
--
-- WHY. That clause asks the `team` COLUMN whether a batch may hold a row of a
-- given team. The column cannot answer it, because it is not about the batch's
-- capacity -- it is about the batch's EXISTING CHILDREN. The B1 import derives
-- it as "the one team all my children share, or null when they span both"
-- (scripts/b1-linear-backfill.js:760) while the parent map keys come from each
-- child's PARENT's team, and the importer says so in as many words at :848-865:
-- "Those legitimately disagree -- a graphics child can hang off a video batch
-- card."
--
-- So the clause refuses appends whose parents are perfectly resolvable, for the
-- single reason that the batch has not held that kind of work BEFORE. Measured
-- 2026-08-26 across 397 active batches: 143 carry a stamp, and every one of
-- them is refused a mixed post no matter what its parent map contains. Two SMMs
-- reported it on the same day as batches "not appearing in the list", and the
-- standing workaround -- clear the column by hand -- is undone by the next
-- import, because the import recomputes it from those same children.
--
-- WHAT STILL REFUSES, unchanged. Everything that is actually about parents:
-- the origin/purpose agreement, the shared-parent waiver, the parity and
-- project comparisons, the per-row team vocabulary (`v_team not in
-- ('video','graphics')`), the kind/team agreement, every ordinal and title
-- rule and every dedup lock. A row whose team has no resolvable parent is
-- refused exactly as before -- by the parent map, which is the thing that knows.
--
-- ORDER OF INSTALLATION. This migration is the FIRST of three steps and is
-- safe on its own: alone it only widens what the RPC accepts, and the gateway
-- in front of it still refuses the same appends with `batch_team_mismatch`
-- until it is redeployed. Run this, then deploy production-write, then ship the
-- browser picker. Any other order shows an SMM an error where she used to see
-- an absence.
--
-- Compiled against a disposable PostgreSQL 16 before handover, applied over the
-- live schema baseline, with v6 installed first and this file replacing it --
