...
-- A3, the release gate. `production_workload_label_projection` returns
-- complete:false for ANY `linear_raw` without a well-formed `issue.labels`
-- relation, and the native intake paths (`handleIntakeCreate`,
-- `handleComponentFill` in supabase/functions/production-write/index.ts) write
-- `linear_raw: { attribution: ... }` with no `issue` at all -- `linear-outbound`
-- is what adds `issue` on drain. So the moment `linear_outbound_enabled` is set
-- to off, every newly created deliverable is stamped incomplete forever, and a
-- reader that treats incomplete as fatal blanks the whole board for everyone.
--
-- This distinguishes "no provider label state was ever stamped" (provable, and
-- the permanent post-cutoff shape) from "a label relation exists but is
-- malformed or paginated" (genuinely unprovable, and still refused). Only the
-- first is answered as complete-with-no-labels. The accepted cost is that a row
-- with no provider label state weighs 1x: `2x Workload` / `3x Workload` are
-- provider labels and nothing native mints them yet.
create function public.workload_native_label_state_absent(p_raw jsonb)
