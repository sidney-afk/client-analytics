-- Write-UI epoch: atomic append-to-existing-batch intake (v8, 2026-09-07).
--
-- SUPERSEDES migrations/2026-08-26-production-intake-append-v7.sql (applied).
--
-- WHAT CHANGES: the two places this function reads or writes a child TITLE now
-- tolerate an optional ' — <name>' suffix, and nothing else in either function
-- is touched. No table, column, index, policy or grant moves.
--
--   * the ordinal scan (`v_base_ordinal`) counts a NAMED title as a used
--     number, and
--   * the per-group title check accepts the composed title either bare or with
--     that suffix.
--
-- WHY. Owner request 2026-09-07: "when they create a new batch, they should be
-- able to choose the name ... and if they add to a previous batch ... they
-- should be able to name that sub-issue also." The batch half needed nothing
-- here -- `batches.name` has always come from the caller and becomes the Linear
-- parent title. The sub-issue half ends at this function, which demands the
-- title be EXACTLY `[Sample ]Video N` / `[Sample ]Thumbnail N` and raises
-- `invalid_intake_append_order` on anything else.
--
-- WHY A SUFFIX AND NOT A FREE TITLE. Two reasons, both structural.
--
--   1. THE NUMBER IS THE ONLY RECORD OF THE ORDINAL. `deliverables` has no
--      column for it. This function derives the next number by reading it back
--      out of the titles already in the batch (`v_base_ordinal`, below), and so
--      does planAppendIntakeItems in the gateway. A free-form title erases it,
--      and the very next append reissues a number that is already in use.
...
--
-- WHAT THIS DELIBERATELY DOES NOT DO: it does not validate the NAME. The
-- gateway composes every title from one function (`intakeChildTitle` in
-- policy.mjs) and caps the name at 160 characters; this function still pins
-- everything that protects the batch -- the ordinal, the sort key, the card
-- pairing, the team vocabulary, the kind/team agreement, the origin/purpose
-- agreement, the parent map and every dedup lock. What it stops asserting is
-- that a title carries NOTHING after its number, which was never a safety
-- property, only a formatting one.
--
-- ORDER OF INSTALLATION. This migration is the FIRST of three steps and is safe
-- on its own: alone it only widens what the RPC accepts, and the deployed
-- gateway in front of it still composes bare numbered titles, so nothing
-- changes until production-write is redeployed. Run this, then deploy
-- production-write, then the browser (index.html ships on merge, and its Create
-- Post dialog checks the response and says so out loud when a name did not
-- land, so an early merge is visible rather than silent).
--
-- Compiled against a disposable PostgreSQL 16 before handover, applied over the
-- live schema baseline, with v7 installed first and this file replacing it --
