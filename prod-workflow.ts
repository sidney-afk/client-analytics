...
const fetchTopVideosJson = node({
  ...
  output: [{
    scraped_date: '2026-06-02',
    summary: { clients_processed: 22, total_video_rows: 66 },
    clients: [
      {
        ...
        status: 'active',
        top_videos: [
          { rank: 1, video_url: 'https://www.instagram.com/reel/abc/', creator_name: 'Sample Creator', creator_handle: '@samplecreator', views: 500000, outlier_score: 10.0, summary: 'sample summary', scraped_date: '2026-06-02' }
        ]
      }
    ]
  }]
...
const readClientsInfo = node({
  ...
  output: [{ client_name: 'Sample Client', slack_channel_id: 'C0123', content_description: 'sample niche' }]
});
...
const readTopVideos = node({
  ...
  output: [{ client_name: 'Sample Client', period: 'week', views: '50000', video_url: 'https://www.instagram.com/reel/xyz/', scraped_date: '2026-06-02' }]
});
...
const matchTopReelPerClient = node({
  ...
  output: [{
    ...
    content_description: 'sample niche',
    platform: 'instagram',
...
const claudeWhyItPerformed = node({
    ...
    parameters: {
      ...
      headerParameters: {
        parameters: [
          ...
          { name: 'Raw', value: 'application/json' }
        ]
      },
      sendBody: true,
      contentType: 'raw',
  ...
  output: [{
    content: [{ text: 'Your hook worked because it named a universal feeling.' }]
  }]
});
...
const formatSlackMessage = node({
  ...
  output: [{
    slack_channel_id: 'C0123',
    client_name: 'Sample Client',
    text: 'Top reel of the week for Sample Client: 50,000 views',
    blocks: '{"blocks":[]}'
  }]
});
...
const sendSlackMessage = node({
  ...
  config: {
    ...
    parameters: {
      ...
      operation: 'post',
      select: 'channel',
      channelId: { __rl: true, mode: 'id', value: expr('{{ $json.slack_channel_id }}') },
      messageType: 'block',
...
const webhookResponse = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody: '={ "success": true, "message": "Weekly Slack updates sent" }',
      options: {}
    },
    position: [2912, 112]
  },
...
const productionNote = sticky(
  '## PRODUCTION WORKFLOW\n\n**Sends to REAL client Slack channels** via `slack_channel_id` column in Clients Info sheet.\n\n**Triggers:**\n- Every Monday 9 AM (scheduled)\n- Manual webhook: POST /weekly-slack-top-reel\n\n**Pipeline:**\n1. Read clients + this week\'s top reels from SYNCVIEW\n2. Fetch top viral videos in each client\'s niche from `data/top_videos.json` on GitHub (populated by Sandcastles routine Monday 1 AM)\n3. Apify scrape → Whisper transcribe → Claude analyze the client\'s own reel\n4. Format combined Slack message (top reel + top videos in niche)\n5. Send to each client\'s channel\n\nDo not modify unless you mean it. Use the TEST workflow for changes.',
  [],
  { color: 4, position: [-40, -160], width: 700, height: 300 }
);

export default workflow('weekly-slack-top-reel', 'Weekly Slack – Top Reel of the Week')
  .add(productionNote)
  .add(scheduleTrigger)
