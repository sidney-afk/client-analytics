...
if ($RetirementProfile -and $Lane -notin @('control-schema','control-recovery')) { throw 'RetirementProfile restricted to control-schema/control-recovery.' }
if ($RetirementProfile -and $Lane -eq 'control-recovery' -and !$CurrentPublicOwners) { throw 'RetirementProfile recovery requires CurrentPublicOwners and explicit observed input.' }
...
if ($Lane -eq 'observed-full-install') {
 ...
 $ObservedInputDirectory=(Resolve-Path -LiteralPath $ObservedInputDirectory).Path
} elseif ($Lane -eq 'install-operator') {
 if (!$ObservedInputDirectory -or $CalibrateTarget -or $ExpectedTarget -or $ExpectedTargetSha256) { throw 'Install operator requires only ObservedInputDirectory.' }
 $ObservedInputDirectory=(Resolve-Path -LiteralPath $ObservedInputDirectory).Path
} elseif ($Lane -eq 'native-preinstall-backup') {
 if ($CalibrateTarget -or $ExpectedTarget -or $ExpectedTargetSha256) { throw 'Native preinstall backup accepts only optional ObservedInputDirectory.' }
 if ($ObservedInputDirectory) { $ObservedInputDirectory=(Resolve-Path -LiteralPath $ObservedInputDirectory).Path }
...
$expectedMajor=if ($Lane -in @('f27','priority-observed-baseline','priority-application-recovery','sequence-consistency','sequence-application','credential-schema','credential-recovery','retirement-freeze','calendar-freeze','native-signoff','complete-application-recovery','card-atomic-admission','card-atomic-handlers','application-dml-admission','followup-worker','followup-outcome-proof','admission-preflight','source-baseline','source-phases','install-finalize','install-operator','install-maintenance','observed-full-install','control-recovery','control-schema','retirement-switch','native-preinstall-backup','provider-issue-observation','provider-comment-observation','provider-terminal-history','provider-recorded-create','provider-create-observation','provider-create-recovery','provider-comment-recovery','provider-ack-recovery','provider-checkpoint-recovery','provider-send','write-diagnostics','install-bootstrap','install-journal','observed-routines','provider-debt-disposition','followup-retry','provider-closed-snapshot','followup-outcome-matrix') -or $RecoveryPostgres17) { '17' } else { '16' }
...
try {
 ...
 $localeArgs=if ($Lane -in @('observed-full-install','install-operator') -or ($Lane -eq 'native-preinstall-backup' -and $ObservedInputDirectory)) { @('--locale-provider=icu','--icu-locale=en-US') } else { @() }
 $initExit=Invoke-Hidden (Join-Path $pgPath 'initdb.exe') (@('-D',$data,'-U','postgres','--auth=scram-sha-256',('--pwfile='+$pwFile),'--encoding=UTF8','--locale=C')+$localeArgs) 'init'
 ...
 if ($Lane -eq 'native-preinstall-backup' -and $ObservedInputDirectory) { [IO.File]::AppendAllText((Join-Path $data 'postgresql.conf'),"`nwal_level = logical`n") }
 Assert-OwnedData
 ...
 if ($Lane -in @('observed-full-install','install-operator') -or $CurrentPublicOwners -or ($Lane -eq 'native-preinstall-backup' -and $ObservedInputDirectory)) { Set-ProofEnvironment 'OBSERVED_INPUT_DIRECTORY' $ObservedInputDirectory }
 ...
 if ($Lane -eq 'control-schema') { $entry=Join-Path $repoRoot 'test\linear-exit-control-schema-postgres.js' }
 if ($Lane -eq 'control-recovery') { if (!(Test-Path -LiteralPath (Join-Path $pgPath 'pg_dump.exe') -PathType Leaf)) { throw 'Control recovery requires pg_dump.' }; $entry=Join-Path $repoRoot 'test\linear-exit-control-recovery-postgres.js' }
 if ($Lane -eq 'observed-routines') { $entry=Join-Path $repoRoot 'test\linear-exit-observed-routines-postgres.js' }
 if ($Lane -eq 'install-journal') { $entry=Join-Path $repoRoot 'test\linear-exit-install-journal-postgres.js' }
 if ($Lane -eq 'install-finalize') { $entry=Join-Path $repoRoot 'test\linear-exit-install-finalize-postgres.js' }
 if ($Lane -eq 'install-maintenance') { $entry=Join-Path $repoRoot 'test\linear-exit-install-maintenance-postgres.js' }
 if ($Lane -eq 'provider-send') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-send-postgres.js' }
 if ($Lane -eq 'write-diagnostics') { $entry=Join-Path $repoRoot 'test\linear-exit-write-diagnostics-postgres.js' }
 if ($Lane -eq 'provider-checkpoint-recovery') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-checkpoint-recovery.js' }
 if ($Lane -eq 'provider-ack-recovery') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-ack-recovery.js' }
 if ($Lane -eq 'native-preinstall-backup') { $entry=Join-Path $repoRoot 'test\linear-exit-native-preinstall-backup.js' }
 if ($Lane -eq 'provider-issue-observation') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-issue-observation.js' }
 if ($Lane -eq 'provider-comment-observation') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-comment-observation.js' }
 if ($Lane -eq 'install-operator') { $entry=Join-Path $repoRoot 'test\linear-exit-install-operator-postgres.js' } elseif ($Lane -eq 'retirement-switch') { $entry=Join-Path $repoRoot 'test\linear-exit-retirement-switch-postgres.js' } elseif ($Lane -eq 'provider-terminal-history') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-terminal-history.js' }
 if ($Lane -eq 'provider-recorded-create') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-recorded-create-recovery.js' }
 if ($Lane -eq 'provider-create-observation') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-create-observation.js' }
 if ($Lane -eq 'provider-create-recovery') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-create-recovery.js' }
 if ($Lane -eq 'provider-comment-recovery') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-comment-recovery.js' }
 if ($Lane -eq 'install-bootstrap') { $entry=Join-Path $repoRoot 'test\linear-exit-install-bootstrap-postgres.js' }
 if ($Lane -eq 'source-phases') { $entry=Join-Path $repoRoot 'test\linear-exit-source-phases-postgres.js' }
 if ($Lane -eq 'source-baseline') { $entry=Join-Path $repoRoot 'test\linear-exit-source-baseline-catalog-postgres.js' }
 if ($Lane -eq 'admission-preflight') { $entry=Join-Path $repoRoot 'test\linear-exit-admission-preflight-postgres.js' }
 if ($Lane -eq 'followup-outcome-matrix') { $entry=Join-Path $repoRoot 'test\linear-exit-followup-outcome-matrix.js' }
 if ($Lane -eq 'provider-closed-snapshot') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-closed-snapshot.js' }
 if ($Lane -eq 'followup-retry') { $entry=Join-Path $repoRoot 'test\linear-exit-followup-retry.js' }
 ...
 if ($Lane -eq 'observed-full-install') { $entry=Join-Path $repoRoot 'test\linear-exit-observed-full-pipeline.js' }
 if ($Lane -notin @('unit','f27') -and $entry -eq (Join-Path $repoRoot 'test\run-all.js')) { throw 'Selected lane has no explicit test entry; refusing full-suite fallback.' }
 $arguments=@($entry)
 if ($RetirementProfile) { $arguments+='--retirement' }
 ...
 if ($result -eq 0 -and $RetirementProfile -and $Lane -eq 'control-recovery' -and !(Select-String -LiteralPath (Join-Path $runRoot 'unit.log') -Pattern 'LINEAR_EXIT_CONTROL_RETIREMENT_OK' -Quiet)) { throw 'Retirement control recovery marker missing.' }
 ...
 if ($result -eq 0 -and $Lane -in @('composition','f27','notifications','upstream-ledger','installation-order','installation-resume','installation-interruption','preflight','preflight-installed','view-provenance','priority-companion','priority-snapshot','priority-restore','priority-application-schema','priority-application-supplement','priority-observed-baseline','priority-application-recovery','sequence-consistency','sequence-application','credential-schema','credential-recovery','retirement-freeze','calendar-freeze','native-signoff','complete-application-recovery','card-atomic-admission','card-atomic-handlers','application-dml-admission','followup-worker','followup-outcome-proof','admission-preflight','source-baseline','source-phases','install-finalize','install-operator','install-maintenance','observed-full-install','control-recovery','control-schema','retirement-switch','native-preinstall-backup','provider-issue-observation','provider-comment-observation','provider-terminal-history','provider-recorded-create','provider-create-observation','provider-create-recovery','provider-comment-recovery','provider-ack-recovery','provider-checkpoint-recovery','provider-send','write-diagnostics','install-bootstrap','install-journal','observed-routines','provider-debt-disposition','followup-retry','provider-closed-snapshot','followup-outcome-matrix')) {
  $marker=if ($Lane -eq 'observed-full-install') { if ($CalibrateTarget) { 'LINEAR_EXIT_OBSERVED_FULL_CALIBRATION_OK' } else { 'LINEAR_EXIT_OBSERVED_FULL_PIPELINE_OK' } } elseif ($Lane -eq 'control-schema') { 'LINEAR_EXIT_CONTROL_SCHEMA_OK' } elseif ($Lane -eq 'control-recovery') { 'LINEAR_EXIT_CONTROL_RECOVERY_OK' } elseif ($Lane -eq 'install-operator') { 'LINEAR_EXIT_INSTALL_OPERATOR_OK' } elseif ($Lane -eq 'retirement-switch') { 'LINEAR_EXIT_RETIREMENT_SWITCH_OK' } elseif ($Lane -eq 'install-operator') {
 if (!$ObservedInputDirectory -or $CalibrateTarget -or $ExpectedTarget -or $ExpectedTargetSha256) { throw 'Install operator requires only ObservedInputDirectory.' }
 $ObservedInputDirectory=(Resolve-Path -LiteralPath $ObservedInputDirectory).Path
} elseif ($Lane -eq 'native-preinstall-backup') { 'LINEAR_EXIT_NATIVE_PREINSTALL_BACKUP_OK' } elseif ($Lane -eq 'provider-issue-observation') { 'LINEAR_EXIT_PROVIDER_ISSUE_OBSERVATION_OK' } elseif ($Lane -eq 'provider-comment-observation') { 'LINEAR_EXIT_PROVIDER_COMMENT_OBSERVATION_OK' } elseif ($Lane -eq 'provider-terminal-history') { 'LINEAR_EXIT_PROVIDER_TERMINAL_HISTORY_OK' } elseif ($Lane -eq 'provider-recorded-create') { 'LINEAR_EXIT_PROVIDER_RECORDED_CREATE_OK' } elseif ($Lane -eq 'provider-create-observation') { 'LINEAR_EXIT_PROVIDER_CREATE_OBSERVATION_OK' } elseif ($Lane -eq 'provider-create-recovery') { 'LINEAR_EXIT_PROVIDER_CREATE_RECOVERY_OK' } elseif ($Lane -eq 'provider-comment-recovery') { 'LINEAR_EXIT_PROVIDER_COMMENT_RECOVERY_OK' } elseif ($Lane -eq 'provider-ack-recovery') { 'LINEAR_EXIT_PROVIDER_ACK_RECOVERY_OK' } elseif ($Lane -eq 'write-diagnostics') { 'LINEAR_EXIT_WRITE_DIAGNOSTICS_OK' } elseif ($Lane -eq 'provider-checkpoint-recovery') { 'LINEAR_EXIT_PROVIDER_CHECKPOINT_RECOVERY_OK' } elseif ($Lane -eq 'install-finalize') { 'LINEAR_EXIT_INSTALL_FINALIZE_OK' } elseif ($Lane -eq 'install-maintenance') { 'LINEAR_EXIT_INSTALL_MAINTENANCE_OK' } elseif ($Lane -eq 'provider-send') { 'LINEAR_EXIT_PROVIDER_SEND_OK' } elseif ($Lane -eq 'install-bootstrap') { 'LINEAR_EXIT_INSTALL_BOOTSTRAP_OK' } elseif ($Lane -eq 'observed-routines') { 'LINEAR_EXIT_OBSERVED_ROUTINES_OK' } elseif ($Lane -eq 'install-journal') { 'LINEAR_EXIT_INSTALL_JOURNAL_OK' } elseif ($Lane -eq 'followup-outcome-matrix') { 'LINEAR_EXIT_FOLLOWUP_OUTCOME_MATRIX_OK' } elseif ($Lane -eq 'source-phases') { 'LINEAR_EXIT_SOURCE_PHASES_OK' } elseif ($Lane -eq 'source-baseline') { 'LINEAR_EXIT_SOURCE_BASELINE_CATALOG_OK' } elseif ($Lane -eq 'provider-closed-snapshot') { 'LINEAR_EXIT_PROVIDER_CLOSED_SNAPSHOT_OK' } elseif ($Lane -eq 'followup-retry') { 'LINEAR_EXIT_FOLLOWUP_RETRY_OK' } elseif ($Lane -eq 'provider-debt-disposition') { 'LINEAR_EXIT_PROVIDER_DEBT_DISPOSITION_OK' } elseif ($Lane -eq 'admission-preflight') { 'LINEAR_EXIT_ADMISSION_PREFLIGHT_POSTGRES_OK' } elseif ($Lane -eq 'followup-outcome-proof') { 'LINEAR_EXIT_FOLLOWUP_OUTCOME_PROOF_OK' } elseif ($Lane -eq 'followup-worker') { 'LINEAR_EXIT_FOLLOWUP_WORKER_POSTGRES_OK' } elseif ($Lane -eq 'application-dml-admission') { 'LINEAR_EXIT_APPLICATION_DML_ADMISSION_OK' } elseif ($Lane -eq 'card-atomic-handlers') { 'LINEAR_EXIT_CARD_ATOMIC_HANDLERS_OK' } elseif ($Lane -eq 'card-atomic-admission') { 'LINEAR_EXIT_CARD_ATOMIC_ADMISSION_OK' } elseif ($Lane -eq 'complete-application-recovery') { 'LINEAR_EXIT_COMPLETE_APPLICATION_RECOVERY_OK' } elseif ($Lane -eq 'native-signoff') { 'LINEAR_EXIT_NATIVE_SIGNOFF_VERIFIER_OK' } elseif ($Lane -eq 'calendar-freeze') { 'LINEAR_EXIT_CALENDAR_FREEZE_GAP_PROVEN' } elseif ($Lane -eq 'retirement-freeze') { 'LINEAR_EXIT_GLOBAL_FREEZE_UNPROVEN' } elseif ($Lane -eq 'credential-recovery') { 'LINEAR_EXIT_CREDENTIAL_RECOVERY_OK' } elseif ($Lane -eq 'credential-schema') { 'LINEAR_EXIT_CREDENTIAL_SCHEMA_OK' } elseif ($Lane -eq 'sequence-application') { 'LINEAR_EXIT_SEQUENCE_APPLICATION_OK' } elseif ($Lane -eq 'sequence-consistency') { 'LINEAR_EXIT_SEQUENCE_CONSISTENCY_OK' } elseif ($Lane -eq 'priority-application-recovery') { 'LINEAR_EXIT_PRIORITY_APPLICATION_RECOVERY_OK' } elseif ($Lane -eq 'priority-observed-baseline') { 'LINEAR_EXIT_PRIORITY_OBSERVED_BASELINE_OK' } elseif ($Lane -in @('priority-application-schema','priority-application-supplement')) { 'LINEAR_EXIT_PRIORITY_APPLICATION_SCHEMA_OK' } elseif ($Lane -eq 'priority-restore') { 'LINEAR_EXIT_PRIORITY_RESTORE_OK' } elseif ($Lane -eq 'priority-snapshot') { 'LINEAR_EXIT_PRIORITY_SNAPSHOT_OK' } elseif ($Lane -eq 'priority-companion') { 'LINEAR_EXIT_PRIORITY_COMPANION_POSTGRES_OK' } elseif ($Lane -eq 'view-provenance') { 'LINEAR_EXIT_VIEW_PROVENANCE_OK' } elseif ($Lane -eq 'preflight-installed') { 'LINEAR_EXIT_PREFLIGHT_ORDERED_OK' } elseif ($Lane -eq 'preflight') { 'LINEAR_EXIT_PREFLIGHT_POSTGRES_OK' } elseif ($Lane -eq 'installation-interruption') { 'LINEAR_EXIT_INSTALL_INTERRUPTION_OK' } elseif ($Lane -eq 'installation-resume') { 'LINEAR_EXIT_INSTALL_RESUME_OK' } elseif ($Lane -eq 'installation-order') { 'LINEAR_EXIT_INSTALL_ORDER_OK' } elseif ($Lane -eq 'upstream-ledger') { 'LINEAR_EXIT_UPSTREAM_LEDGER_OK' } elseif ($Lane -eq 'composition') { 'LINEAR_EXIT_OWNER_COMPOSITION_OK' } elseif ($Lane -eq 'notifications') { 'ok native notifications PostgreSQL proof' } else { 'F27_PROOF_OK' }
