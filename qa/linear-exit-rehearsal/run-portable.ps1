...
try {
 ...
 $localeArgs=if ($Lane -eq 'observed-full-install' -or ($Lane -eq 'native-preinstall-backup' -and $ObservedInputDirectory)) { @('--locale-provider=icu','--icu-locale=en-US') } else { @() }
 ...
 if ($Lane -eq 'observed-full-install' -or $CurrentPublicOwners -or ($Lane -eq 'native-preinstall-backup' -and $ObservedInputDirectory)) { Set-ProofEnvironment 'OBSERVED_INPUT_DIRECTORY' $ObservedInputDirectory }
 ...
 if ($Lane -eq 'retirement-switch') { $entry=Join-Path $repoRoot 'test\linear-exit-retirement-switch-postgres.js' } elseif ($Lane -eq 'provider-terminal-history') { $entry=Join-Path $repoRoot 'test\linear-exit-provider-terminal-history.js' }
