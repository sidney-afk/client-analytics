...
$expectedMajor=if ($Lane -in @('f27','priority-observed-baseline','priority-application-recovery','sequence-consistency','sequence-application','credential-schema','credential-recovery','retirement-freeze','calendar-freeze') -or $RecoveryPostgres17) { '17' } else { '16' }
...
try {
 ...
 $arguments=@($entry)
 if ($Lane -eq 'calendar-freeze') { $arguments=@('--experimental-strip-types',$entry) }
