...
class Store{
  constructor(){this.calls=[];this.flagError=false;this.tables={deliverables:[{id:'synthetic-target',client_slug:'synthetic-client',team:'video',status:'todo',updated_at:'2026-09-05T10:00:00Z',linear_issue_uuid:uuid(500),linear_raw:{issue:{id:uuid(500),labelIds:[],labels:{nodes:[],pageInfo:{hasNextPage:false,endCursor:null}}}}}],
...
try{
  ...
  let source=fs.readFileSync(path.join(ROOT,EDGE),'utf8');assert.equal(source,baseline,'foundation must not change the active writer');pass('candidate gateway byte-identical to pinned baseline');
