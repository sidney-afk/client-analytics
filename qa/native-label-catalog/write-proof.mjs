...
try{
 ...
 // B2 Slice 8: the Edge no longer reads prod_authority (it is the constant syncview), but the
 // transactional authority assert still does, so a new write under a stale "linear" value is
 // refused by the database with no mutation and no provider. (Native policy so the Edge reaches SQL.)
 policy('native',uuid(700));const newAfterFlip=await call(candidate,operation('labelsv',[]));policy('hold');assert.equal(newAfterFlip.status,409);assert.equal(newAfterFlip.body.error,'team_is_linear_authoritative');assert.deepEqual(state(),replayStart);assert.equal(providerCalls,start);pass('new request remains refused under changed authority');
 const replayState=replayStart;
 db.query(`insert into public.team_members(id,name,role,active) values(${lit(uuid(887799))},'Other Admin','admin',true);`);
 const wrongActor=await call(candidate,write,{...keyHeaders,'x-syncview-actor':'Other Admin'});assert.equal(wrongActor.status,409);assert.equal(wrongActor.body.error,'idempotency_conflict');assert.deepEqual(state(),replayState);assert.equal(providerCalls,start);pass('current authenticated different actor cannot adopt another native receipt');
 db.query("delete from public.syncview_runtime_flags where key='prod_authority';");r=await call(candidate,write);assert.equal(r.status,200);assert.deepEqual(state(),replayState);assert.equal(providerCalls,start);policy('native',uuid(700));r=await call(candidate,operation('labelsv',[uuid(2)]));policy('hold');assert.notEqual(r.status,200,JSON.stringify(r));assert.deepEqual(state(),replayState);assert.equal(providerCalls,start);pass('with the prod_authority row missing, exact replay stays read-only and new work is refused (by the transactional authority assert) with no provider and no mutation');
 ...
 r=await call(candidate,write);assert.equal(r.status,200);assert.deepEqual(state(),replayState);assert.equal(providerCalls,start);db.query('grant select on public.syncview_runtime_flags to service_role;');pass('actual denied flag read still permits exact read-only receipt adoption with no provider');
 ...
 // B2 Slice 8: there is no provider label lane left. A provider-mode catalog holds new work
 // instead of reading Linear, even with a healthy provider on offer.
 policy('provider');provider='healthy';db.query(`update public.deliverables set linear_issue_uuid=${lit(uuid(500))} where id='labelsp';`);
 const providerWrite=operation('labelsp',[uuid(1)]);const providerState=state();start=providerCalls;r=await call(candidate,providerWrite);assert.equal(r.status,503,JSON.stringify(r));assert.equal(r.body.error,'native_label_catalog_held');assert.equal(providerCalls,start);assert.deepEqual(state(),providerState);
 r=await call(candidate,readBody('labelsp'));assert.equal(r.status,503);assert.equal(r.body.error,'native_label_catalog_held');assert.equal(providerCalls,start);pass('provider-mode catalog holds new label reads and writes with zero provider egress and no mutation');
