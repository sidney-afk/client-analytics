...
(async () => {
  ...
  const linear = [];
  for (const wh of ['linear-add-comment', 'linear-set-status']) await sctx.route('**/webhook/' + wh, async (r) => { let b = {}; try { b = JSON.parse(r.request().postData() || '{}'); } catch (e) {} linear.push({ wh, b }); await fulfilLinearHook(r); });
