...
function compare(snapshot, options = {}) {
  const nativeScope = options.scope === 'native_workload';
  if (nativeScope && typeof options.eligibility !== 'function') throw new Error('NATIVE_ELIGIBILITY_REQUIRED');
  ...
  for (const s of nativeScope ? ['native','workload','members'] : [...surfaces, 'members', 'expected']) {
  ...
  for (const s of (nativeScope ? ['workload'] : surfaces.filter(s => s !== 'native'))) {
  ...
  for (const native of snapshot.native) {
    ...
    if (!nativeScope && !matches.production?.length) add(complete('production') && !ambiguousLinks.get(native).has('production') ? 'native_missing_from_production' : 'production_absence_unproven', native);
    const eligibility = nativeScope ? options.eligibility(native) : workloadEligibility(native, snapshot.workloadRoster).state;
