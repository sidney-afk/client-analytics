...
function boardContext() {
  ...
  const ctx = context([
    ...
    'wlIssueClientAllowed','wlIssueEditorAllowed','wlSortSubIssues',
...
async function nativeBoard(envelope, today) {
  ...
  Object.assign(ctx,{
    ...
    _syncviewStaffIdentityForHeaders:()=>({key:'synthetic',role:'admin',member:{id:'synthetic'}}),
    wlFetchForeignLinearMetadata:async()=>{throw Error('SCOPED_PROVIDER_READ_FORBIDDEN');},
    fetch:async(url,init)=>{if(url!==ctx.WORKLOAD_PLAN_URL||init.method!=='POST'||init.body!==JSON.stringify({action:'native_snapshot'}))throw Error('OFFLINE_NETWORK_FORBIDDEN');
      calls.push('captured_native_snapshot');return {ok:true,status:200,json:async()=>JSON.parse(JSON.stringify(envelope))};},
    _syncviewStaffIdentityClear:()=>{throw Error('UNEXPECTED_IDENTITY_MUTATION');},
    wlPurgePlanSensitiveState:()=>{throw Error('UNEXPECTED_SCOPE_MUTATION');},
  });
  Object.assign(ctx.wlState,{issueSnapshot:[],dueAuthorityByIssueId:new Map(),nativeDueTargetByIssueId:new Map()});
  for(const name of ['wlSnapshotIdentity','wlProductionAuthorityValue','wlProductionAuthorityFingerprint',
    'wlMetadataTeamBucket','wlNativeWorkloadLabel','wlNativeDueDate','wlValidRfc3339Timestamp','wlNativeMetadataRow',
    'wlFetchNativeSnapshot','loadLinearIssues','wlAdoptPlanRows','wlAdoptLinearMetadata','wlLoadSnapshot'])vm.runInContext(extract(name),ctx);
  await ctx.wlLoadSnapshot(false,null);
  if(calls.length!==1)throw Error('SCOPED_BROWSER_READ_INCOMPLETE');
  const buckets=Object.fromEntries(['planned','nowWorking','tweaksNeeded','overdue','undated','unassigned'].map(k=>[k,ctx.wlSortSubIssues(ctx.wlState[k]).map(r=>r.id)]));
  const visible=new Set(Object.values(buckets).flat());
  const dates=Object.fromEntries(ctx.wlState.issueSnapshot.filter(r=>r.isSubIssue).map(r=>[r.id,{plan:ctx.wlPlanDate(r),display:ctx.wlDisplayDate(r),weight:ctx.wlWorkloadWeight(r)}]));
  return {buckets,visible,dates,issues:ctx.wlState.issueSnapshot,
    excluded:Object.fromEntries(Object.entries(ctx.wlState.excluded).map(([k,rs])=>[k,rs.map(r=>r.id)])),
    calendar:Object.fromEntries([...ctx.wlState.calendarByDate].map(([day,rs])=>[day,ctx.wlSortSubIssues(rs).map(r=>r.id)])),
    ordering:ctx.wlSortSubIssues(ctx.wlState.allActiveSubs).map(r=>r.id),calls:calls.length};
}
module.exports = { root, source, extract, context, boardContext, cardStatus, workStatus, workloadEligibility, nativeBoard };
