...
function payloadFor(result,id,drill=false){formatAlert(result,id);return relayPayload({type:drill?'client_continuity_DRILL':'client_continuity',summaryParts:[...(drill?['DRILL']:[]),result.lane,result.code],team:'client_surfaces',count:result.count,runId:id});}
function textFor(result,id,drill=false){const p=payloadFor(result,id,drill);return `[SyncView] ${p.issue_identifier} count=${p.count} run_id=${id}`;}
...
async function botRoute(result,id,config,env,fetchImpl,{reconcileOnly=false,drill=false}={}) {
  ...
  const text=textFor(result,id,drill);
  let ts;
  if(!reconcileOnly) {
...
async function relayRoute(result,id,config,env,fetchImpl,{reconcileOnly=false,drill=false,sleepImpl=ms=>new Promise(r=>setTimeout(r,ms))}={}) {
  ...
  const payload=payloadFor(result,id,drill),deadline=Date.now()+25000;
...
async function deliver(result,id,config,env=process.env,fetchImpl=fetch,options={}) {
  ...
  formatAlert(result,id); // reject unknown/free-text fields before any I/O
