...
function resolveConfig(file,env,activation,kind='view') {
  ...
  requireValue(/^[a-f0-9]{64}$/.test(config.pageSha256)&&config.pageSha256===createHash('sha256').update(fs.readFileSync(path.join(ROOT,'index.html'))).digest('hex'),'release_mismatch');
...
async function main(args=process.argv.slice(2),env=process.env) {
  ...
  const {config,output,lanes}=resolveConfig(option('--config'),env,option('--activate'),kind);
  ...
  try {
    ...
    for(const lane of lanes) {
      ...
      const start={version:1,runId,lane:lane.lane,releaseSha:config.releaseSha,startedAt};
    ...
    return {version:1,fixture:false,releaseSha:config.releaseSha,results};
  } finally {
...
module.exports={main,privatePath,resolveConfig,persist};
