...
function sections(payload,manifest){
 ...
 const other=omitted.filter(n=>!tables.some(t=>t.name===n)).map(n=>`do $priority_omitted$ begin if exists(select from public.${qi(n)}) then raise exception 'PRIORITY_RESTORE_UNCOVERED_TABLE_NOT_EMPTY';end if;end $priority_omitted$;`).join('\n');
