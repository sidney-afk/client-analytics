...
function sections(payload,manifest,credentialCompanionVerified=false){
 const tables=companion.schema().tables;
 const omitted=manifest.omitted_data_tables;
 if(!Array.isArray(omitted)||new Set(omitted).size!==omitted.length||omitted.some(n=>typeof n!=='string'||!/^[_a-z][_a-z0-9]{0,62}$/.test(n)))throw Error('PRIORITY_RESTORE_OMITTED_CONTRACT');
 if(tables.some(t=>!omitted.includes(t.name)||Object.hasOwn(manifest.data.tables,t.name)))throw Error('PRIORITY_RESTORE_COVERAGE_MISMATCH');
 const guards=pk=>tables.map(t=>{
  ...
  return `do $priority_shape$ begin if (${actual}) is distinct from ${lit(JSON.stringify(expected))}::jsonb then raise exception 'PRIORITY_RESTORE_SCHEMA_MISMATCH';end if;end $priority_shape$;`;
 }).join('\n');
 ...
 const bags=tables.map(t=>{
  ...
  return `do $priority_rows$ begin if exists(select row from (${actual}) a except all select value from jsonb_array_elements(${expected}::jsonb)) or exists(select value from jsonb_array_elements(${expected}::jsonb) except all select row from (${actual}) a) then raise exception 'PRIORITY_RESTORE_MULTISET_MISMATCH';end if;end $priority_rows$;`;
 }).join('\n');
 const credentialNames=credentialCompanionVerified===true?require('./linear-exit-credential-companion').schema().tables.map(t=>t.name):[];
 const other=omitted.filter(n=>!tables.some(t=>t.name===n)&&!credentialNames.includes(n)).map(n=>`do $priority_omitted$ begin if exists(select from public.${qi(n)}) then raise exception 'PRIORITY_RESTORE_UNCOVERED_TABLE_NOT_EMPTY';end if;end $priority_omitted$;`).join('\n');
