...
function validate(proof,states){
 ...
 if(canonicalJson(proof.sequences.map(s=>s.name))!==canonicalJson(NAMES)||canonicalJson(states.map(s=>s.name).sort())!==canonicalJson(NAMES))fail('SEQUENCE_COVERAGE');
 const allowed=new Set([...resolveCorpus('history-v11').tables.map(t=>t.name),...companion.schema().tables.map(t=>t.name)]);
