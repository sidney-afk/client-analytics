...
const {limit,mapObjects}=require('./linear-exit-object-concurrency');
...
function createReader({projectRef,serviceKey,fetch:transport=globalThis.fetch,timeoutMs=300000,maxObjects=100000,maxPages=100000,objectConcurrency=1}){
 ...
 const concurrency=limit(objectConcurrency);
 ...
 async function census(){const bucketRows=await buckets(),objects=[],seen=new Set();let pages=0;
  for(const bucket of bucketRows){const folders=[''],visited=new Set();while(folders.length){const prefix=folders.shift();if(visited.has(prefix))fail('FOLDER_DUPLICATE');visited.add(prefix);let offset=0;for(;;){if(++pages>maxPages)fail('PAGE_LIMIT');const rows=await json('/object/list/'+segment(bucket.id),{prefix,limit:100,offset,sortBy:{column:'name',order:'asc'}});if(!Array.isArray(rows)||rows.length>100)fail('LIST');const pending=[];for(const row of rows){segment(row?.name);const name=prefix?prefix+'/'+row.name:row.name,id=identity({bucket:bucket.id,path:name});if(seen.has(id))fail('DUPLICATE');seen.add(id);if(row.id===null&&row.metadata===null){folders.push(name);continue;}if(typeof row.id!=='string'||!row.id)fail('LIST_OBJECT');if(objects.length+pending.length>=maxObjects)fail('OBJECT_LIMIT');pending.push({bucket:bucket.id,path:name});}objects.push(...await mapObjects(pending,concurrency,item=>info(item)));if(rows.length<100)break;offset+=rows.length;}}
...
function createStorageExportAdapter(options){
 ...
 return {
  ...
  async listObjectMetadata(){return mapObjects(inventory.objects,options.objectConcurrency,async item=>{const found=await reader.info(item,true);if(!same(metadataOnly(found),metadataOnly(item)))fail('INVENTORY_DRIFT');return {bucket:found.bucket,path:found.path,version:found.version,metadata:found.object_metadata};});},
...
async function captureStorageInventory(options){
 const capabilityBefore=await capability.observe(options.readOnlyCapabilityQuery);const reader=createReader(options),before=await reader.census();if(capabilityBefore.bucket_count!==before.buckets.length||capabilityBefore.object_count!==before.objects.length)fail('CAPABILITY_COUNT_MISMATCH');
 const captured=await mapObjects(before.objects,options.objectConcurrency,async item=>{const metadataBefore=await reader.info(item,true);const response=await reader.download(item),hash=crypto.createHash('sha256');let size=0;for await(const bytes of response.body){size+=bytes.length;hash.update(bytes);}if(size!==item.size||!same(await reader.info(item),item))fail('CAPTURE_CHANGED');if(!same(metadataBefore,await reader.info(item,true)))fail('METADATA_CHANGED');return {metadata:{bucket:item.bucket,path:item.path,version:item.version,metadata:metadataBefore.object_metadata},object:{...item,sha256:hash.digest('hex')}};});
 const objects=captured.map(x=>x.object),object_metadata=captured.map(x=>x.metadata);
 const after=await reader.census();if(!same(before,after))fail('CENSUS_CHANGED');await mapObjects(object_metadata,options.objectConcurrency,async expected=>{const actual=await reader.info(expected,true);if(!same(expected.metadata,actual.object_metadata)||expected.version!==actual.version)fail('METADATA_CHANGED');});
