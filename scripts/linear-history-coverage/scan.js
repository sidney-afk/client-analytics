...
async function scan(input, { key = crypto.randomBytes(32), inspectReferences } = {}) {
  ...
  // Private in-process inspection only; the CLI never supplies this callback.
  // Clone so an observer cannot change classifications or evidence bindings.
  if (inspectReferences !== undefined) {
    if (typeof inspectReferences !== 'function') throw new Error('invalid_reference_observer');
    inspectReferences(structuredClone(refs));
  }
