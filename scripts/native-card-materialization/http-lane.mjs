...
try{
  ...
  await check('actual retained browser adopts full current HTTP card before marking completed',async()=>{
    const c=cases[2],ctx={console,_isIntake:true,_linearIntakeCheckpointOrSuspend:()=>{},_calCacheRead:()=>null,_sxrCacheRead:()=>null,_calCacheWrite:()=>{},_calRenderBody:()=>{},calClientSlug:x=>x,
      calState:{client:c.body.client,posts:[]},_calUpsertFetch:async(_client,body,source)=>{const r=await send(routes.calendar.route,body,header(source));return new Response(r.raw,{status:r.status});}};
    ...
    const job={payload:c.intake_body,result:c.response.json,context:{},completed_card_ids:[]};await ctx._writeNativeSubmissionCardsToCalendar(job);
