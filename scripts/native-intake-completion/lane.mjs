 ...
 * functions and the REAL runner library (completion-lib.js, with a psql-backed
...
const { runReconcile } = require('./completion-lib.js');
...
try {
  ...
  console.log(JSON.stringify({ suite: 'native-intake-completion', passed: checks.filter(c => c.pass).length, failed: checks.filter(c => !c.pass).length,
