 ...
 *
 *   node scripts/native-intake-completion/run.js                      # public report of the backlog, write nothing
 *   node scripts/native-intake-completion/run.js --limit=10           # examine/plan at most 10 owed requests
 *   NATIVE_INTAKE_COMPLETION_CONFIRM=RECONCILE_NATIVE_INTAKE_APPLY \
 *   node scripts/native-intake-completion/run.js --apply --limit=10   # recover children and materialize cards
 *   node scripts/native-intake-completion/run.js --private-report=/absolute/path/outside/repo.json
 *
 ...
 * when NATIVE_INTAKE_COMPLETION_HASH_KEY is configured. Request ids, client
...
const { runReconcile, publicReport } = require('./completion-lib');
...
const ACTOR = String(args.get('actor') || process.env.NATIVE_INTAKE_COMPLETION_ACTOR || 'native-intake-completion').trim();
const PRIVATE_REPORT = String(args.get('private-report') || '');
const HASH_KEY = String(process.env.NATIVE_INTAKE_COMPLETION_HASH_KEY || '');
...
if (APPLY && process.env.NATIVE_INTAKE_COMPLETION_CONFIRM !== 'RECONCILE_NATIVE_INTAKE_APPLY') {
  fail('config_error: --apply requires NATIVE_INTAKE_COMPLETION_CONFIRM=RECONCILE_NATIVE_INTAKE_APPLY');
}
  ...
  try {
    report = await runReconcile({
      ...
      onEvent: (stage, entry) => {
        ...
        if (!HASH_KEY) return;
        const { correlation, boundOutcome } = require('./completion-lib');
