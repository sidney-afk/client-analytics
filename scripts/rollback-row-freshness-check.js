    ...
    while ((m = heading.exec(log))) {
        ...
        const objectless = leadsShipping ? named : named
            /* A NOUN FORM NAMES A THING: "Inspect deployment", "deployment
               ...
               on #1306). */
            .replace(/\b(?:deployment|deploy|rollout|release|cut ?over)\b(?!\s+(?:(?:was|is|are|were|has|have|had|been|got)\s+)*(?:(?:now|already|finally|just|then|[a-z]+ly)\s+)*(?:complete|completed|succeeded|successful|finished|done|failed|green|via|from|to|by|on|at|in|into|through|with|as|after|before|during|the|this|run)\b)(?:\s+[a-z]+\b)?/gi, ' ')
