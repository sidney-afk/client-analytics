-- v10: exact47 tables; adds provision, retirement-admission and native identifier durable owners.
...
-- confirmation=HISTORY_V10_BACKUP_GRANTS_ONLY|DISPOSABLE_SCRATCH_ONLY.
...
declare
  ...
  relations constant text[] := array[
    ...
    'production_card_materialization_ingress','production_label_catalog_versions','linear_outbound_cutoff_control','public_intake_log','legacy_intake_native_triage','native_brief_media_occurrences','production_native_client_provisions','syncview_retirement_admission','production_native_identifier_mint','production_native_identifier_grants','description_images'];
begin
  ...
  if current_setting('track_b.setup_confirmation') <> (case when mode='backup'
    then 'HISTORY_V10_BACKUP_GRANTS_ONLY' else 'DISPOSABLE_SCRATCH_ONLY' end) then
  ...
  foreach relation_name in array relations loop
    ...
    if relation_name=any(array['public_intake_log','legacy_intake_native_triage','native_brief_media_occurrences','production_native_client_provisions','syncview_retirement_admission','production_native_identifier_mint','production_native_identifier_grants','description_images']) then
    ...
    if relation_name = any(array['production_card_materialization_receipts','production_card_materialization_ingress','production_label_catalog_versions','linear_outbound_cutoff_control','legacy_intake_native_triage','production_native_client_provisions','syncview_retirement_admission','production_native_identifier_mint','production_native_identifier_grants']) then
    ...
    if relation_name = any(array['production_card_materialization_receipts','production_card_materialization_ingress','production_label_catalog_versions','native_brief_media_occurrences','production_native_client_provisions','syncview_retirement_admission','production_native_identifier_mint','production_native_identifier_grants','description_images'])
  ...
  )) then raise exception 'History v10 corpus foreign-key boundary is incomplete'; end if;
