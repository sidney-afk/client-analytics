-- v5: exact33 tables; includes private HR and provider recovery payloads.
...
-- confirmation=HISTORY_V5_BACKUP_GRANTS_ONLY|DISPOSABLE_SCRATCH_ONLY.
...
begin
  ...
  if current_setting('track_b.setup_confirmation') <> (case when mode='backup'
    then 'HISTORY_V5_BACKUP_GRANTS_ONLY' else 'DISPOSABLE_SCRATCH_ONLY' end) then
  ...
  if mode='scratch' and to_regprocedure('public.track_b_restore_set_history_v5_user_triggers(boolean)') is not null then
  ...
  )) then raise exception 'History v5 corpus foreign-key boundary is incomplete'; end if;
...
create or replace function public.track_b_restore_set_history_v5_user_triggers(enabled boolean)
...
revoke all on function public.track_b_restore_set_history_v5_user_triggers(boolean) from public;
revoke all on function public.track_b_restore_set_history_v5_user_triggers(boolean) from anon, authenticated, service_role;
grant execute on function public.track_b_restore_set_history_v5_user_triggers(boolean) to :"existing_role";
\endif
commit;
