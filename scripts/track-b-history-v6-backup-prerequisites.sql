-- v6: exact35 tables; includes private HR and provider recovery payloads.
-- RELEASE BLOCKER: this grants artifact does not supply a matching schema package.
-- Require the authenticated schema capture and empty-target restore proof in
-- docs/ops/TRACK_B_BACKUP.md before calling any data package recoverable.
...
-- confirmation=HISTORY_V6_BACKUP_GRANTS_ONLY|DISPOSABLE_SCRATCH_ONLY.
...
-- Prerequisites: reviewed journal + native manifest/epoch + card provenance +
-- Calendar feedback materialization schemas, all source-pinned in the paired artifact;
...
declare
  ...
  relations constant text[] := array[
    ...
    'workload_plan',
    'card_change_journal',
    'production_intake_manifests',
    'pto_members',
    'pto_requests',
    'pto_adjustments',
    'linear_project_ids_shape_migration_20260728',
    ...
    'linear_intake_receipts',
    'production_card_provenance',
    'calendar_feedback_materializations'];
begin
  ...
  if current_setting('track_b.setup_confirmation') <> (case when mode='backup'
    then 'HISTORY_V6_BACKUP_GRANTS_ONLY' else 'DISPOSABLE_SCRATCH_ONLY' end) then
  ...
  if mode='scratch' and to_regprocedure('public.track_b_restore_set_history_v6_user_triggers(boolean)') is not null then
  ...
  )) then raise exception 'History v6 corpus foreign-key boundary is incomplete'; end if;
  -- COPY re-evaluates receipt CHECK constraints as the scratch role. Their
  -- exact, pure validator closure needs EXECUTE; never grant writer RPCs or
  -- SECURITY DEFINER functions to solve a restore privilege failure.
...
create or replace function public.track_b_restore_set_history_v6_user_triggers(enabled boolean)
...
revoke all on function public.track_b_restore_set_history_v6_user_triggers(boolean) from public;
revoke all on function public.track_b_restore_set_history_v6_user_triggers(boolean) from anon, authenticated, service_role;
grant execute on function public.track_b_restore_set_history_v6_user_triggers(boolean) to :"existing_role";
\endif
commit;
