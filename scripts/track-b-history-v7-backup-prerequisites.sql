-- v7: exact37 tables; adds retained materialization receipts and ingress.
...
-- confirmation=HISTORY_V7_BACKUP_GRANTS_ONLY|DISPOSABLE_SCRATCH_ONLY.
...
begin
  ...
  if mode='scratch' and to_regprocedure('public.track_b_restore_set_history_v7_user_triggers(boolean)') is not null then
...
create or replace function public.track_b_restore_set_history_v7_user_triggers(enabled boolean)
...
revoke all on function public.track_b_restore_set_history_v7_user_triggers(boolean) from public;
revoke all on function public.track_b_restore_set_history_v7_user_triggers(boolean) from anon, authenticated, service_role;
grant execute on function public.track_b_restore_set_history_v7_user_triggers(boolean) to :"existing_role";
\endif
commit;
