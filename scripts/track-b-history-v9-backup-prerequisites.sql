-- v9: exact42 tables; adds public intake provenance, F44 triage and native media ledger owners.
...
-- confirmation=HISTORY_V9_BACKUP_GRANTS_ONLY|DISPOSABLE_SCRATCH_ONLY.
...
declare
  ...
  relations constant text[] := array[
    ...
    'production_card_materialization_ingress','production_label_catalog_versions','linear_outbound_cutoff_control','public_intake_log','legacy_intake_native_triage','native_brief_media_occurrences'];
begin
  ...
  if current_setting('track_b.setup_confirmation') <> (case when mode='backup'
    then 'HISTORY_V9_BACKUP_GRANTS_ONLY' else 'DISPOSABLE_SCRATCH_ONLY' end) then
  ...
  if mode='scratch' and to_regprocedure('public.track_b_restore_set_history_v9_user_triggers(boolean)') is not null then
  ...
  foreach relation_name in array relations loop
    ...
    if relation_name=any(array['public_intake_log','legacy_intake_native_triage','native_brief_media_occurrences']) then
    ...
    if relation_name = any(array['production_card_materialization_receipts','production_card_materialization_ingress','production_label_catalog_versions','linear_outbound_cutoff_control','legacy_intake_native_triage']) then
    ...
    if relation_name = any(array['client_access_events','syncview_auth_events','flag_flips','settings_events',
      'deliverable_events','mirror_outbox','calendar_post_events','sample_review_events','card_change_journal','production_card_provenance','public_intake_log'])
      and pg_get_serial_sequence('public.' || relation_name,'id') is null then
      raise exception 'History identity sequence missing';
    end if;
    if relation_name = any(array['production_card_materialization_receipts','production_card_materialization_ingress','production_label_catalog_versions','native_brief_media_occurrences'])
  ...
  )) then raise exception 'History v9 corpus foreign-key boundary is incomplete'; end if;
...
create or replace function public.track_b_restore_set_history_v9_user_triggers(enabled boolean)
...
relations constant text[] := array[
  ...
  'production_card_materialization_receipts','production_card_materialization_ingress','production_label_catalog_versions','linear_outbound_cutoff_control','public_intake_log','legacy_intake_native_triage','native_brief_media_occurrences'];
begin
...
revoke all on function public.track_b_restore_set_history_v9_user_triggers(boolean) from public;
revoke all on function public.track_b_restore_set_history_v9_user_triggers(boolean) from anon, authenticated, service_role;
grant execute on function public.track_b_restore_set_history_v9_user_triggers(boolean) to :"existing_role";
\endif
commit;
