...
// Refusal-log triage 2026-09-25: tell our own automation (CI, executor
// sessions, scripts) from people without joining request logs. Headless
// browsers and HTTP libraries say so in their user agent; a caller may also
// declare itself with x-syncview-traffic: automation. Never 'person' by
// declaration: a header cannot upgrade a headless agent to a human.
const AUTOMATION_AGENT=/HeadlessChrome|Playwright|Puppeteer|curl\/|node-fetch|undici|axios\/|python-requests|Deno\/|Go-http-client/i;
export function requestTraffic(req){try{const agent=req?.headers?.get?.('user-agent')||'';if(req?.headers?.get?.('x-syncview-traffic')==='automation'||!agent||AUTOMATION_AGENT.test(agent))return 'automation';return 'person';}catch{return 'automation';}}
export async function makeRefusalReceipt(context,code,status,origin='gateway',extra={}){
