// Supabase Edge Function: onboarding-list
//
...
// Account-login keys inside `answers` that must never reach the public dashboard.
...
Deno.serve(async (req: Request) => {
  ...
  // Credential fields are stripped below, so admin, SMM, and creative staff
  // can all read this list — only onboarding-full (unredacted, with
  // passwords) stays admin-only. The service-role client must never exist
  // on an unauthenticated request path.
