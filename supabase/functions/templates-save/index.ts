...
function patchObject(v: unknown): Record<string, string> {
  ...
  for (const [k, val] of Object.entries(v as Record<string, unknown>)) {
    // Target identity is owned by the authenticated top-level clientName.
    if (!k || k === "client_slug" || k === "client_name" || k === "updated_at") continue;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const clientSlug = normalizeBrowserWriteClient(clientName);
    ...
    const actor = await authorizeBrowserWrite(db, req, clientSlug, "templates-save");
