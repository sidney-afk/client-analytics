...
(async () => {
  ...
  const EXEMPT = {
    ...
    _wlFetchArchiveMarkerRows: [2, 'the archive-marker read added 2026-09-21 for OPEN_REPAIRS 229, plus the comment above it naming the view. It reads only the archive/delete marker columns for an explicit id list and counts, sums and ranks nothing: its one caller, _wlArchivedNativeIds, filters to is_sub_issue rows before it is asked, so a batch parent never reaches it, and the answer is a set of ids to HIDE rather than any total. The same reasoning as wlFetchNativeMetadata directly above'],
