...
(async () => {
  ...
  const EXEMPT = {
    ...
    assetSnapshot: [1, 'the CHILD-BATCH BORROW, added 2026-09-01 after the owner reported a post parent showing four Missing slots while its own sub-issue showed the filming plan. It selects `batch_id` from the rows whose raw_issue_parent_id is THIS row, pinned to the same client, purely to find which batch the post actually keeps its links on -- measured that day, 5,729 of 6,230 live deliverables sit on a b1_ mirror batch while their children sit on the native one. Nothing is counted, summed or ranked: the ids are de-duplicated and the first batch CARRYING links wins. A batch parent among the children is impossible by construction (a synthetic parent has no deliverable row, so nothing links to it), and a real one would at worst contribute a batch id its siblings already contribute'],
