...
(async () => {
  ...
  const EXEMPT = {
    ...
    _wlFetchArchiveMarkerRows: [2, 'the archive-marker read added 2026-09-21 for OPEN_REPAIRS 229, plus the comment above it naming the view. It reads only rows carrying archive/delete markers and counts, sums and ranks nothing: its one caller, _wlArchivedNativeIds, intersects the answer with the snapshot\'s is_sub_issue id set, so a batch parent never reaches the returned set, and the answer is a set of ids to HIDE rather than any total. The same reasoning as wlFetchNativeMetadata directly above'],
    ...
    assetSnapshot: [2, 'the POST ROSTER and the BUCKET OCCUPANTS, both added 2026-09-05 when the 2026-09-01 child-batch borrow was replaced. The first selects `batch_id` for every row of the post -- found by the parent uuid from either direction, pinned to the same client -- purely to learn which batch rows the post keeps its links on; measured that day, of 1,138 posts 44 span more than one. The second selects `batch_id,linear_issue_uuid,raw_issue_parent_id` for the rows sitting in those candidate buckets, to learn which buckets belong to this post ALONE, and is asked only when the post spans more than one. Neither counts, sums or ranks anything: the first de-duplicates ids and resolves per slot in a deterministic order, the second builds a set of bucket ids to EXCLUDE as write targets. A parent among the rows is not a hazard in either -- it contributes a batch id its siblings already contribute, or a post key that is this post\'s own'],
