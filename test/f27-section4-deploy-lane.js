...
const CANDIDATES = new Map([
  ...
  ['production-write', {
    ...
    // Re-pinned 2026-09-07 (thirty-second release): post
    ...
    // FIRST -- the deployed RPC refuses a named title until it is. Deployed as
    // v69 2026-09-07 (EXECUTION_LOG.md).
    //
    // Re-pinned 2026-09-08 (thirty-third release; NOT yet deployed): a
    // Submit-tab note written for the video editor can no longer suppress or
    // stand in for AI thumbnail-title generation. Gate 3 in
    // submissionThumbnailText used to treat ANY non-empty item.brief (which
    // index.html's _linearThumbnailBrief filled with the per-video note) as a
    // human override and skip generation outright, so an ordinary editing
    // note -- never authored as a graphics brief -- silently produced exactly
    // the "AI never wrote a title" symptom the owner reported on a real
    // client's batch. The generated line is prefixed with
    // THUMBNAIL_TEXT_AI_LABEL ("Thumbnail title (generated by AI): ") so it
    // can never be mistaken for a human-authored brief once it lands in a
    // description.
    //
    // Amended before merge, same day: the owner does not want the note
    // dropped to make room for a generated title either -- he wants the note
    // kept as a baseline the designer always sees, with a generated line
    // appended below it. Gate 3 now excludes an item only when a PRIOR
    // ATTEMPT already wrote its brief (idempotent-retry protection, not
    // human-override protection); a caller-supplied note no longer excludes
    // an item from generation. The brief expression composes
    // `existingBrief || [sourceBrief, generatedLine].filter(Boolean).join("\n")`
    // -- a prior attempt always wins outright, short of that the note and the
    // labelled generated line are additive, joined by one newline, neither
    // blocking nor replacing the other. index.html's _linearThumbnailBrief
    // reverted to forwarding the note (unchanged from before 2026-08-21).
    ...
    source: 'a27c0e0ede7f3e271ae5a61fc4b479390b76ace7b63a153b0a0c3619ada53c73',
    entrypoint: '3e23bd2f7b3a58291a21714796b4e94d421376a79fb5e47a2d8121f1b60a0c22',
    files: 6,
  }],
]);
