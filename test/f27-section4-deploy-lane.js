...
const CANDIDATES = new Map([
  ...
  ['production-write', {
    ...
    // stand in for AI thumbnail-title generation. index.html's
    // _linearThumbnailBrief stopped forwarding the per-video note as the
    // graphics child's caller-supplied brief -- that note was never authored
    // as a graphics brief, but Gate 3 below treats ANY non-empty item.brief as
    // a human override and skips generation outright, so an ordinary editing
    // note silently produced exactly the "AI never wrote a title" symptom the
    // owner reported on a real client's batch. Gate 3 itself, and the
    // existing/caller-supplied/generated precedence, are UNCHANGED -- a brief
    // someone genuinely writes on the Linear sub-issue still wins over a
    // generated one. The generated line is now also prefixed with
    // THUMBNAIL_TEXT_AI_LABEL ("Thumbnail title (generated by AI): ") so it
    // can never again be mistaken for a human-authored brief once it lands in
    // a description; the empty case stays unlabelled and honestly empty.
    ...
    source: 'c3d88287db059c0e6a5d0fb26c87e6f327fc74bc3b30d95fc29dd1ae0d3be760',
