...
function plan(){const bytes=fs.readFileSync(path.join(ROOT,'docs/independence/LINEAR_EXIT_INSTALL_SOURCE_INVENTORY_20260918_3.json'));const manifest=JSON.parse(bytes.toString('utf8'));inventory.verify(manifest);const skip=new Set([...COVERED,'live-schema-baseline-2026-07-03.sql']);return{manifest,digest:sha(bytes),remaining:manifest.dependency_order.filter(id=>!skip.has(id))};}
