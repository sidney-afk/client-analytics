...
const result=moduleUnderTest.verify();assert.equal(result.sql_owners.length,7);assert.deepEqual(result.sql_owners.map(x=>x.order),[1,2,3,4,5,6,7]);assert.equal(result.application_inventories.length,2);assert.equal(result.hosted_baseline_observed_proven,false);assert.equal(result.complete_hosted_closure_proven,false);assert.equal(result.runtime_worker_enabled,false);assert.equal(result.final_activation,'REFUSING');
...
console.log('ADMISSION_RELEASE_EXTENSION_PASS exact source pins, seven-owner order, immutable baseline inventory, refused drift, incomplete hosted flags');
