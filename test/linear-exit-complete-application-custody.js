...
try{const sourceFile=path.join(temp,'source.bin');fs.writeFileSync(sourceFile,bytes);const encryptionKey=crypto.randomBytes(32),keyId=crypto.randomBytes(16).toString('hex'),target=path.join(temp,'package'),opts={sourceFile,target,hmacInput:key,encryptionKey,keyId};
 custody.pack(opts);const restored=path.join(temp,'restored.bin');custody.restore({...opts,packageDirectory:target,target:restored});assert.deepEqual(fs.readFileSync(restored),bytes);assert.equal(Object.keys(complete.read(fs.readFileSync(restored),key).payload.tables).length,complete.expectedNames().length);
 ...
 console.log('COMPLETE_APPLICATION_CUSTODY_PASS synthetic v1 exactbytes, invalid source, invalid restored content, wronglayout, nooverwrite, cleanup; live/offdevice UNPROVEN');
