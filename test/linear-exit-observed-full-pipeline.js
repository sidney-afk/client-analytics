...
// rebuilds observed67; applySettledWorld() adds the storage supplement, the opt-out prerequisite
// and the hiring migration, in that order, to reach settled68 (catalog ddfa4c4f...).
// CORRECTED 2026-09-17: this lane briefly had its OWN copy of that construction which applied the
// hiring migration and skipped the opt-out prerequisite, so it built 5864a28f... and the storage
// session's calibrate run refused. Both callers now share one construction, which is the fix.
...
require(ROOT+'/scripts/linear-exit-b9-catalog-derive').applySettledWorld(c);
...
if(mode==='--verify'){
 ...
 let blocked=false;try{c.exec('update public.calendar_posts set id=id where false');}catch(e){blocked=/maintenance/i.test(e.message);}assert(blocked);assert.equal(c.scalarJson("select count(*)::int from pg_trigger where tgname='linear_exit_maintenance_dml_v1' and tgenabled='A'"),require(ROOT+'/scripts/linear-exit-observed-public-catalog').postInstallPublicTables(plan.initial_catalog_sha256).expected);
