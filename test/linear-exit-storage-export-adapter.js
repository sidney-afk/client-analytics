...
(async()=>{try{
 ...
 const capability={bucket_count:1,disabled_buckets:1,object_count:102,archived_objects:0,delete_markers:0,versioned_objects:0,null_flags:0,version_listing_migrations:0};
 const opts={readOnlyCapabilityQuery:async sql=>{assert(sql.startsWith('begin read only;'));return {...capability};},projectRef:'a'.repeat(20),serviceKey:crypto.randomBytes(32).toString('hex'),hmacInput:crypto.randomBytes(32).toString('base64'),encryptionKey:crypto.randomBytes(32),keyId:crypto.randomBytes(16).toString('hex'),fetch:async(url,options)=>{assert.equal(new URL(url).origin,'https://'+'a'.repeat(20)+'.supabase.co');assert.equal(options.redirect,'error');return fetch('http://127.0.0.1:'+port+new URL(url).pathname,options);}};
 ...
 for(const objectConcurrency of [4,8]){
  let active=0,peak=0;const counted={...opts,objectConcurrency,readOnlyCapabilityQuery:async sql=>{assert.equal(active,0,'capability barrier drains object work');return opts.readOnlyCapabilityQuery(sql);},fetch:async(...args)=>{active++;peak=Math.max(peak,active);try{const r=await opts.fetch(...args);await new Promise(resolve=>setTimeout(resolve,2));return r;}finally{active--;}}};
  const concurrent=await captureStorageInventory(counted);assert.deepEqual(concurrent.inventoryBytes,captured.inventoryBytes,'parallel inventory ordering and hashes equal serial');assert.equal(active,0);assert.equal(peak,objectConcurrency,'bounded workers actually overlap');
 }
 ...
 for(const objectConcurrency of [4,8]){
  const concurrent={...options,objectConcurrency,target:path.join(temp,'http-parallel'+objectConcurrency)};concurrent.adapter=createStorageExportAdapter(concurrent);await exportObjects(concurrent);const dest=path.join(temp,'http-restored'+objectConcurrency);restoreEncrypted({...concurrent,packageDirectory:concurrent.target,target:dest});for(const [name,bytes] of files)assert.deepEqual(fs.readFileSync(path.join(dest,'synthetic',name)),bytes);
  behavior='metadata-drift';metadataReads=0;await assert.rejects(captureStorageInventory({...opts,objectConcurrency}),/METADATA_CHANGED/);behavior='normal';
  let observed=0;await assert.rejects(captureStorageInventory({...opts,objectConcurrency,readOnlyCapabilityQuery:async()=>({...capability,version_listing_migrations:observed++?1:0})}),/CAPABILITY_CHANGED/);
 }
 ...
 await assert.rejects(captureStorageInventory({...opts,maxPages:1}),/PAGE_LIMIT/);for(const patch of [{disabled_buckets:0},{archived_objects:1},{delete_markers:1},{versioned_objects:1},{null_flags:1},{object_count:103}])await assert.rejects(captureStorageInventory({...opts,readOnlyCapabilityQuery:async()=>({...capability,...patch})}));let observation=0;await assert.rejects(captureStorageInventory({...opts,readOnlyCapabilityQuery:async()=>({...capability,version_listing_migrations:observation++?1:0})}),/CAPABILITY_CHANGED/);assert.deepEqual(metadata.storage_version_capability,capability);assert.equal(metadata.database_api_project_binding_proven,false);
