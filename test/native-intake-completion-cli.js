 ...
 * This suite runs the ACTUAL CLI (scripts/native-intake-completion/run.js) as a
...
const lib = require('../scripts/native-intake-completion/completion-lib');
...
function runCli(port, extraArgs, extraEnv) {
  return new Promise(resolve => {
    const child = spawn(process.execPath, [path.resolve(__dirname, '../scripts/native-intake-completion/run.js'), ...extraArgs], {
      env: {
...
(async () => {
  ...
  ok(lib.boundReason(CANARY.nestedReason) === 'other' && lib.boundReason('card_slot_occupied') === 'card_slot_occupied'
    && lib.boundReason('child_terminal_receipt_missing') === 'child_terminal_receipt_missing'
  ...
  try {
    ...
    const keyed = await runCli(port, ['--limit=5'], { NATIVE_INTAKE_COMPLETION_HASH_KEY: 'fixture-hash-key' });
    ...
    const applied = await runCli(port, ['--apply', '--strict', '--limit=1'], { NATIVE_INTAKE_COMPLETION_CONFIRM: 'RECONCILE_NATIVE_INTAKE_APPLY' });
    ...
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nic-private-'));
  ...
  console.log(`\nnative-intake-completion-cli: ${pass} passed, ${fail} failed`);
