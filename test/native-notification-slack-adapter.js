...
(async()=>{try{
 ...
 {let seen;await postSlackChannelMessage('token',channel,text,'00000000-0000-4000-8000-000000000001',false,async(_u,o)=>{seen=JSON.parse(o.body);return new Response(JSON.stringify({ok:true,channel,ts:'1788800000.123456'}));},[{type:'section'}],[{color:'#F2994A',fallback:text,blocks:[{type:'section'}]}]);
  assert.equal(seen.attachments[0].color,'#F2994A');assert.equal(seen.blocks,undefined,'attachments replace top-level blocks');assert.equal(seen.text,undefined,'no visible duplicate line above the card');assert.equal(seen.attachments[0].fallback,text,'plain line kept as notification fallback');assert.equal(seen.link_names,false);}
