...
(async()=>{try{
 ...
 for(const[name,change]of[['inactive client',()=>tables.clients[0].active=false],['wrong client',()=>tables.deliverables[0].client_slug='other'],['wrong team',()=>tables.deliverables[0].team='graphics'],['provider assignment epoch',()=>epoch=''],['unresolved old assignee',()=>tables.deliverables[0].assignee_id='provider-id'],['inactive editor',()=>tables.team_members[1].active=false],['missing Slack identity',()=>delete tables.team_members[1].slack_user_id],['closed batch',()=>tables.batches[0].status='archived'],['changed round',()=>tables.calendar_posts[0].video_status_at='2026-09-02T00:00:00Z'],['changed status',()=>tables.deliverables[0].status='approved'],['lookup unavailable',()=>lookupError=true]])await check(name+' refuses before notification enqueue',async()=>{change();const r=await call();assert(r.status>=400);assert.equal(r.body.delivery,'not_sent');assert.equal(fetchCalls,0);assert.equal(enqueueArgs,null);});

 await check('real owner rows need no synthetic deleted_at columns',async()=>{for(const row of [tables.deliverables[0],tables.batches[0],tables.calendar_posts[0]])assert.equal(Object.hasOwn(row,'deleted_at'),false);assert.equal((await call()).status,202);});
 for(const marker of ['webhook_delete','deleted','delete','removed','archived','issue.archivedAt']){
  const set=value=>{tables.deliverables[0].linear_raw=marker==='issue.archivedAt'?{issue:{archivedAt:value}}:{[marker]:value};};
  for(const value of [false,0,null,'','  ','false',' FALSE ','0',' null '])await check(marker+' false-like '+JSON.stringify(value)+' remains eligible',async()=>{set(value);assert.equal((await call()).status,202);assert.ok(enqueueArgs);});
  for(const value of [true,1,'true','2026-09-01T00:00:00Z'])await check(marker+' live deletion marker refuses',async()=>{set(value);assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);});
 }
 await check('canceledAt is not a deletion marker',async()=>{tables.deliverables[0].linear_raw={issue:{canceledAt:stamp}};assert.equal((await call()).status,202);});
 await check('JSON encoded false-like marker remains eligible',async()=>{tables.deliverables[0].linear_raw=JSON.stringify({deleted:' false ',issue:{archivedAt:'0'}});assert.equal((await call()).status,202);});
 for(const raw of [undefined,null])await check('absent raw owner markers remain eligible',async()=>{tables.deliverables[0].linear_raw=raw;assert.equal((await call()).status,202);});
 for(const raw of ['{broken','null','false','[]','"text"',JSON.stringify(JSON.stringify({deleted:false})),[],true,0])await check('malformed or nonobject raw refuses',async()=>{tables.deliverables[0].linear_raw=raw;assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);});
 for(const marker of ['deleted','issue.archivedAt'])for(const value of [[],{},[false],{value:false}])await check('composite marker refuses despite false-like contents',async()=>{tables.deliverables[0].linear_raw=marker==='deleted'?{deleted:value}:{issue:{archivedAt:value}};assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);});
 await check('JSON encoded lifecycle marker is respected',async()=>{tables.deliverables[0].linear_raw=JSON.stringify({deleted:true});assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);});
 for(const [name,change] of [
  ['archived card',()=>tables.calendar_posts[0].status=' Archived '],
  ['batch client mismatch',()=>tables.batches[0].client_slug='other'],
  ['batch purpose mismatch',()=>tables.batches[0].purpose='samples'],
 ])await check(name+' refuses before enqueue',async()=>{change();assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);assert.equal(fetchCalls,0);});
 // Parent team is a summary label; mixed/imported parents do not change the video child owner.
 for(const team of ['video','graphics',null,undefined])await check('parent team summary does not restrict exact video child',async()=>{tables.batches[0].team=team;assert.equal((await call()).status,202);assert.ok(enqueueArgs);tables.deliverables[0].team='graphics';enqueueArgs=null;assert.equal((await call()).status,409);assert.equal(enqueueArgs,null);});
 ...
 await check('microsecond card rounds retain exact database identity after gateway normalization',async()=>{tables.calendar_posts[0].video_status_at='2026-09-01T00:00:00.123456Z';const r=await call({video_status_at:'2026-09-01T00:00:00.123456Z'});assert.equal(r.body.delivery,'pending');assert.equal(enqueueArgs.p_video_status_at,'2026-09-01T00:00:00.123456Z');});
