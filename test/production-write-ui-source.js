...
ok(/_prodIdbDeleteRecord\(\)/.test(extract('_prodCachePurge'))
  && /_prodCachePurge\(\)/.test(staffPurge)
  && /status === 401 \|\| status === 403\) _prodCachePurge\(\)/.test(extract('_prodLoadData')),
'the production first-paint cache (IndexedDB) is purged on staff sign-out and on a 401/403 live read (stale after the reland narrows grants)');
