...
'gate 3: graphics children only, and only where no PRIOR-ATTEMPT brief already exists — a caller-supplied note no longer excludes an item (2026-09-08)');
...
ok(/const generatedThumbnailTitle = team === "graphics" \? clean\(thumbnailText\.get\(index\)\) : "";\s*\n\s*const generatedLine = generatedThumbnailTitle \? `\$\{THUMBNAIL_TEXT_AI_LABEL\}\$\{generatedThumbnailTitle\}` : "";\s*\n\s*const brief = existingBrief \|\| \[sourceBrief, generatedLine\]\.filter\(Boolean\)\.join\("\\n"\);/.test(intake),
  'a prior attempt\'s brief wins outright; short of that the note and the labelled generated line are ADDITIVE (joined by a newline), reaching graphics items only, never one replacing the other');
ok(/const THUMBNAIL_TEXT_AI_LABEL = "Thumbnail title \(generated by AI\): ";/.test(edge),
  'the AI label is an explicit, reviewable in-repo constant, not assembled inline or read from a secret');
