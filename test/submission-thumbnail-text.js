...
ok(/const generatedThumbnailTitle = team === "graphics" \? clean\(thumbnailText\.get\(index\)\) : "";\s*\n\s*const brief = existingBrief \|\| sourceBrief\s*\n\s*\|\| \(generatedThumbnailTitle \? `\$\{THUMBNAIL_TEXT_AI_LABEL\}\$\{generatedThumbnailTitle\}` : ""\);/.test(intake),
  'generated text is LAST in the brief expression, reaches graphics items only, and is labelled so it cannot be mistaken for a human brief');
