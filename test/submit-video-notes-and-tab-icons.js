 ...
 *    (above the footage lines, because the editor reads the ask first) and
 *    the thumbnail sub-issue (alone -- a designer is not briefed by camera
 *    files).
 *
 *    THE NOTE NO LONGER BLOCKS AI THUMBNAIL GENERATION (fixed 2026-09-08,
 *    then refined the same day). The server used to treat any non-empty
 *    caller-supplied graphics brief as a human override and skip generation
 *    outright for that card, so an ordinary editing note silently disabled
 *    the generator (that is what the Chelsey Scaffidi batch hit). The note
 *    still reaches the thumbnail child exactly as before; what changed is
 *    server-side (production-write/index.ts): the note and a generated title
 *    are now ADDITIVE, never one replacing or blocking the other -- the note
 *    is a baseline the designer always sees, and a labelled generated line
 *    ("Thumbnail title (generated by AI): ...") is appended below it on its
 *    own line whenever generation succeeds.
 *
...
ok(thumbBrief(FULL) === 'Ranking things guys do on a first date p2',
  'the thumbnail brief is the note ALONE — no camera or audio links reach the designer (the server combines it with any generated title, not this function)');
ok(thumbBrief({ main_cam: 'https://x/1', audio: 'https://x/2' }) === '',
  'a card with links but no note gives the thumbnail nothing, so the server may still generate one with nothing to append it to');
...
ok(graphic1.brief === FULL.notes,
  'the planned THUMBNAIL item carries the note — the server (not this function) is what combines it with a generated title rather than letting it block one');
ok(graphic2.brief === '',
