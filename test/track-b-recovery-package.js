...
function manifestFor(preData, postData, data, overrides = {}) {
  ...
  return {
    format: recovery.RECOVERY_FORMAT, recovery_version: recovery.RECOVERY_VERSION, corpus: CORPUS, corpus_version: 7,
    ...
    schema: { fingerprint: 'f'.repeat(32), inventory: { tables: 38, sequences: 1 }, pre_data: { statements: pre.statements.length, skipped_platform_statements: pre.skipped },
    ...
    data: { table_count: 37, tables: inspected }, omitted_data_tables: ['synthetic_omitted'],
...
check('schema section validation counts statements, refuses disallowed classes and records egress-capable functions', () => {
  ...
  assert.equal(recovery.createdTables(section.statements).size, 37);
...
check('package round-trips with authenticated schema/data/sequence/callable binding', () => {
  ...
  assert.equal(recovery.summarize(pkg.manifest).data_table_count, 37);
...
check('reconstruction SQL is one transaction: prerequisites, pre-data, COPY, sequences, post-data, verification, commit', () => {
  ...
  assert.ok(at('COPY public."production_card_materialization_ingress"') < at("setval('public.synthetic_large_seq'"));
  ...
  assert.equal((sql.match(/COPY public\./g) || []).length, 37);
...
check('in-transaction verification checks fingerprint, per-table digests, sequences and ownership', () => {
  ...
  assert.equal((sql.match(/content digest mismatch/g) || []).length, 37);
});
...
check('canonical v3-v7 keep exact 14/21/33/35/37 identities and older readers refuse omitted evidence', () => {
  assert.equal(backup.TABLES.length, 14); assert.equal(backup.HISTORY_TABLES.length, 21);
  const expected = [['legacy-v3', 14], ['history-v4', 21], ['history-v5', 33], ['history-v6', 35], ['history-v7', 37]];
  for (let i = 0; i < expected.length; i++) {
    const [name, count] = expected[i], corpus = backup.resolveCorpus(name);
    assert.equal(corpus.tables.length, count);
    assert.ok(!corpus.tables.some(t => t.name === 'production_label_catalog_versions'));
    assert.doesNotThrow(() => restore.restoreSql(fixtureDump(name), name));
    if (i) {
      const [old] = expected[i - 1];
      // v5 orders referenced owners ahead of dependents; membership/PK identity
      // is retained even when the safe COPY order changes.
      for (const table of backup.resolveCorpus(old).tables)
        assert.deepEqual(corpus.tables.find(t => t.name === table.name), table);
      assert.throws(() => backup.parseStrictPgDump(fixtureDump(name), old), /Unexpected table/);
      assert.throws(() => backup.parseStrictPgDump(fixtureDump(old), name), /missing/);
    }
  }
  assert.equal(backup.resolveCorpus('history-v6').tables.at(-1).name, 'calendar_feedback_materializations');
  assert.equal(backup.resolveCorpus('history-v7').tables.at(-1).name, 'production_card_materialization_ingress');
  assert.match(restore.restoreSql(fixtureDump(CORPUS), CORPUS), /track_b_restore_set_history_v7_user_triggers/);
  assert.throws(() => recovery.readRecoveryPackage(resign(packed.bytes, m => {
    m.corpus = 'history-v6'; m.corpus_version = 6; m.data.table_count = 34;
  }), hmac), /Unexpected table|table count|manifest mismatch/);
});
check('dormant watcher evaluation emits public-safe alert codes and never schedules or sends', () => {
  ...
  const quiet = recovery.evaluateRecoveryWatch({ manifest: { ...manifest, package_sha256: 'p' }, liveFingerprint: manifest.schema.fingerprint, livePublicTableCount: 38, lastReconstruction: { ok: true, package_sha256: 'p' } });
...
check('reconstruct CLI guard refuses production, mismatched ref and missing confirmation; capture args pin the snapshot', () => {
  ...
  assert.equal(recovery.pgDumpSectionArgs('data', '/tmp/x.sql', CORPUS, 's').filter(a => a.startsWith('--table=')).length, 37);
