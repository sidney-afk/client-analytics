...
function backgroundHarness(options = {}) {
  ...
  const context = {
    ...
    wlWriteCache: (payload) => { counters.cacheWrite++; counters.lastCacheWrite = payload; },
...
async function run() {
  ...
  //
  // That is also why this no longer asserts cacheWrite === 0. The old rule --
  // "sensitive hydration must not renew the persisted issue-cache timestamp" --
  // guarded against stamping a NEW time onto an OLD issue population, because a
  // sensitiveOnly refresh used to re-read plans and metadata without re-reading
  // issues. On the native path there is one atomic snapshot and the population
  // is always re-read, so the premise is gone. Work item A6 then requires the
  // write: wlLoadSnapshot's catch still paints a cached board on a failed cold
  // start, and after the source swap nothing on this path filled that cache, so
  // the only cold-path net left had silently stopped being wired at one end.
  // The rule that survives is the one that actually mattered -- a renewed
  // timestamp must always travel WITH the issues it describes -- and it is
  // asserted directly below rather than by forbidding the write.
  {
    ...
    assert.strictEqual(sensitive.counters.cacheWrite, 1, 'A6: a complete native hydration refills the cold-start cache');
    assert.ok(Array.isArray(sensitive.counters.lastCacheWrite.issues) && sensitive.counters.lastCacheWrite.issues.length,
      'A6: the cache write carries the issue population, never a bare timestamp');
    assert.strictEqual(sensitive.counters.lastCacheWrite.fetchedAt, sensitive.context.wlState.fetchedAt,
      'A6: the persisted timestamp is exactly the one the newly read population was published under');
