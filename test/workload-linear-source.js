...
const earlyAuthorityCallAt = EDGE.indexOf('await requireLinearAuthority(db, target.team)');
const teamCallAt = EDGE.indexOf('const currentTeam = await requireCurrentLinearTeam(db, issueId, target.team)');
const authorityCallAt = EDGE.indexOf('await requireLinearAuthority(db, currentTeam)');
const mutationCallAt = EDGE.indexOf('const committed = await setLinearDueDate(db, issueId, dueDate)');
...
ok(teamQueryAt >= 0
  ...
  && earlyAuthorityCallAt > targetCallAt
  && teamCallAt > earlyAuthorityCallAt
 ...
 'set_due_date refuses native authority before provider lookup, then validates the current Linear team and rechecks authority before mutating');
